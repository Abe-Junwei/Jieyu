...
- **B12 MCP resources/prompts + AgentArtifactV0**: inbound `resources/list`/`read` (`jieyu://source-set/{id}`) and `prompts/list`/`get` (A12 vertical registry). Dexie v52 `agent_artifacts` with AdoptionQueue `artifactIds` and `buildB5bExportManifest`. Flag `aiMcpResourcesArtifactsEnabled` default false. Outbound HTTP MCP client is out of this slice.
