...
- **B5a-2 corpus project index**: `/corpus` lists all media units for the current text via a query-layer projection (`listCorpusIndexByTextId`); workset stays across media and clears on text change. Flag `corpusLibraryPageEnabled` still default false. No Dexie index table, no FTS, no ChatWindow.
- **B5b corpus workset clipboard export**: Copy current `corpusBasket` as `text/plain` or Markdown with unit/media/time codes and `/transcription?` deep links. Empty workset does not write the clipboard. Rides `corpusLibraryPageEnabled` (still default false). No HTML/bundle, no ChatWindow.
