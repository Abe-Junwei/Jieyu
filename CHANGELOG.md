...
- **B11 External MCP trust**: origin allowlist (`external_mcp_trust`, Dexie v51). Unregistered / disabled / flag-off origins never expose `tools/list` to the LLM. First enable with schema runs A9 `inspectInbound`. Settings AI section is flag-gated (`aiExternalMcpTrustEnabled`, default false). Outbound HTTP MCP client remains B12.
