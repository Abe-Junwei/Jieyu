...
- **Lexicon sense grammar note (B3w)**: `/lexicon` edits each sense `grammarNote` (the first form text of a sense-level LIFT `<note type="grammar">`). Save readback shows it on that sense and keeps the encyclopedic note. A blank value omits the key. Export writes one note with `lang="und"` after the encyclopedic note and before nested subsenses. Import ignores an entry-level note of the same type and an untyped General Note. Re-import replaces the whole `senses` array, so omitting the note drops the stored text. No DMLex, no new flag.
- **Lexicon sense encyclopedic note (B3v)**: `/lexicon` edits each sense `encyclopedicNote` (the first form text of a sense-level LIFT `<note type="encyclopedic">`). Save readback shows it on that sense and keeps the discourse note. A blank value omits the key. Export writes one note with `lang="und"` after the discourse note and before nested subsenses. Import ignores an entry-level note of the same type. Re-import replaces the whole `senses` array, so omitting the note drops the stored text. No DMLex, no new flag.
- **Lexicon sense discourse note (B3u)**: `/lexicon` edits each sense `discourseNote` (the first form text of a sense-level LIFT `<note type="discourse">`). Save readback shows it on that sense and keeps the anthropology note. A blank value omits the key. Export writes one note with `lang="und"` after the anthropology note and before nested subsenses. Import ignores an entry-level note of the same type. Re-import replaces the whole `senses` array, so omitting the note drops the stored text. No DMLex, no new flag.
- **Lexicon sense anthropology note (B3t)**: `/lexicon` edits each sense `anthropologyNote` (the first form text of a sense-level LIFT `<note type="anthropology">`). Save readback shows it on that sense and keeps the scientific name. A blank value omits the key. Export writes one note with `lang="und"` after the scientific name and before nested subsenses. Import ignores an entry-level note of the same type and a sense bibliography note. Re-import replaces the whole `senses` array, so omitting the note drops the stored text. No DMLex, no new flag.
