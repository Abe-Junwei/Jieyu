...
describe('Golden Round-trip: EAF', () => {
  it('minimal.eaf — 2 units, no translation tiers', () => {
    const raw = readGolden('eaf', 'minimal.eaf');
    const imported = importFromEaf(raw);

    expect(imported.units).toHaveLength(2);
    expect(imported.units[0]!.transcription).toBe('Hello world');
    expect(imported.units[0]!.startTime).toBeCloseTo(0, 3);
    expect(imported.units[0]!.endTime).toBeCloseTo(2, 3);
    expect(imported.units[1]!.transcription).toBe('Goodbye');
    expect(imported.units[1]!.startTime).toBeCloseTo(2, 3);
    expect(imported.units[1]!.endTime).toBeCloseTo(5.5, 3);
    expect(imported.translationTiers.size).toBe(0);

    // Round-trip: export minimal units then re-import
    const units = imported.units.map((u, i) => ({
      id: `u${i}`,
      textId: 'text1',
      mediaId: 'media1',
    ...
    const exported = exportToEaf({ units, layers: [], translations: [] });
    ...
    expect(reimported.units[1]!.transcription).toBe('Goodbye');
    expect(roundTime(reimported.units[0]!.startTime)).toBeCloseTo(0, 3);
    expect(roundTime(reimported.units[0]!.endTime)).toBeCloseTo(2, 3);
  });

  it('multi-tier-ipa.eaf — 3 units, 2 translation tiers, IPA text', () => {
    const raw = readGolden('eaf', 'multi-tier-ipa.eaf');
    const imported = importFromEaf(raw);

    expect(imported.units).toHaveLength(3);
    expect(imported.units[0]!.transcription).toBe('tɕʰa mo ɣa ra');
    expect(imported.units[1]!.transcription).toBe('ŋa joŋs pa yin');
    expect(imported.units[2]!.transcription).toBe('de ring dus tshod');
  ...
  it('xml-escaping.eaf — special characters survive import', () => {
    ...
    expect(imported.units[0]!.transcription).toBe('a < b & c > d "quoted"');
  });
});
...
describe('Golden Round-trip: TextGrid', () => {
  it('minimal.TextGrid — 2 units extracted from gap-padded intervals', () => {
    ...
    const nonEmpty = imported.units.filter((u) => u.transcription.trim() !== '');
    ...
    const exported = exportToTextGrid({ units, layers: [], translations: [] });
    const reimported = importFromTextGrid(exported);
    const nonEmptyAfter = reimported.units.filter((u) => u.transcription.trim() !== '');
...
describe('Golden Round-trip: TRS (Transcriber)', () => {
  it('minimal.trs — 1 turn, 1 speaker, 1 unit', () => {
    ...
    expect(imported.units[0]!.transcription).toBe('Hello world');
    expect(imported.units[0]!.startTime).toBeCloseTo(0, 3);
    expect(imported.units[0]!.endTime).toBeCloseTo(4.0, 3);
    expect(imported.units[0]!.speakerId).toBe('spk1');
  });

  it('two-speakers.trs — 2 speakers, 4 units across 2 turns', () => {
    ...
    expect(imported.units[0]!.transcription).toBe('First utterance by Alice');
    expect(imported.units[0]!.speakerId).toBe('spk1');
    expect(imported.units[1]!.transcription).toBe('Second utterance by Alice');
    expect(imported.units[1]!.speakerId).toBe('spk1');
    expect(imported.units[2]!.transcription).toBe('Bob responds here');
    expect(imported.units[2]!.speakerId).toBe('spk2');
    expect(imported.units[3]!.transcription).toBe('Bob continues speaking');
    expect(imported.units[3]!.speakerId).toBe('spk2');
  });
  ...
  it('two-speakers.trs — round-trip export → import preserves structure', () => {
    ...
    const exported = exportToTrs({ units, speakers: imported.speakers });
    const reimported = importFromTrs(exported);

    expect(reimported.units).toHaveLength(4);
    reimported.units.forEach((u, i) => {
      expect(u.transcription).toBe(imported.units[i]!.transcription);
      expect(u.speakerId).toBe(imported.units[i]!.speakerId);
      expect(roundTime(u.startTime)).toBeCloseTo(roundTime(imported.units[i]!.startTime), 2);
      expect(roundTime(u.endTime)).toBeCloseTo(roundTime(imported.units[i]!.endTime), 2);
    });
  });
});
...
describe('Golden Round-trip: Toolbox', () => {
  ...
  it('interlinear.txt — mb/ge/ps lines map into word+morpheme hierarchy', () => {
    ...
    expect(imported.units).toHaveLength(2);
    expect(imported.units[0]!.transcription).toBe('nga jongs');
    expect(imported.units[0]!.tokens).toHaveLength(2);
    expect(imported.units[0]!.tokens![0]!.morphemes![0]!.gloss!.eng).toBe('1SG');
    expect(imported.units[1]!.tokens![2]!.morphemes).toHaveLength(2);
  });

  it('interlinear.txt — round-trip export → import preserves unit text and timing', () => {
    const raw = readGolden('toolbox', 'interlinear.txt');
    const imported = importFromToolbox(raw);
    ...
    const tokenRows: import('../../src/db').UnitTokenDocType[] = [];
    const morphRows: import('../../src/db').UnitMorphemeDocType[] = [];
    imported.units.forEach((u, ui) => {
      const unitId = `u${ui}`;
    ...
    const exported = exportToToolbox({ units, layers: [], translations: [], tokens: tokenRows, morphemes: morphRows });
...
describe('Golden Round-trip: EAF (additional)', () => {
  it('empty.eaf — no annotations produces 0 units', () => {
    const raw = readGolden('eaf', 'empty.eaf');
    const imported = importFromEaf(raw);
    expect(imported.units).toHaveLength(0);
  });

  it('five-unit-thai.eaf — 5 units with Thai gloss tier', () => {
    const raw = readGolden('eaf', 'five-unit-thai.eaf');
    const imported = importFromEaf(raw);

    expect(imported.units).toHaveLength(5);
    expect(imported.units[0]!.transcription).toBe('kʰɔ̃ː tsʰeː pa reːt');
    expect(imported.units[4]!.transcription).toBe('sɯ́ː kʰɔ̌ŋ pàː');
    expect(imported.units[0]!.startTime).toBeCloseTo(0, 3);
    expect(imported.units[4]!.endTime).toBeCloseTo(10, 3);
  ...
  it('mvm-muya-real.eaf — real-style mvm tier keeps language labels and REF alignment', () => {
    ...
    expect(imported.units).toHaveLength(1);
    expect(imported.units[0]!.transcription).toBe('mu31 nji55');
    expect(imported.units[0]!.startTime).toBeCloseTo(0, 3);
...
describe('Golden Round-trip: TextGrid (additional)', () => {
  it('empty.TextGrid — 0 intervals produces 0 units', () => {
...
describe('Golden Round-trip: TRS (additional)', () => {
  it('empty.trs — no sync points produces 0 units', () => {
    const raw = readGolden('trs', 'empty.trs');
    const imported = importFromTrs(raw);
    expect(imported.units).toHaveLength(0);
  });

  it('three-speakers.trs — 3 speakers, 7 units across 4 turns', () => {
    ...
    expect(imported.units).toHaveLength(7);
    expect(imported.units[0]!.speakerId).toBe('spk2');
    expect(imported.units[0]!.transcription).toBe('请讲一下你的家乡');
    expect(imported.units[2]!.speakerId).toBe('spk1');
    expect(imported.units[2]!.transcription).toBe('ŋa bod ljongs nas yin');
    expect(imported.units[4]!.speakerId).toBe('spk3');
    expect(imported.units[6]!.speakerId).toBe('spk1');
    expect(imported.units[6]!.transcription).toBe('chu bo yaŋ yod red');
  });
});
...
describe('Golden Round-trip: Toolbox (additional)', () => {
  it('empty.txt — empty record produces 0 or 1 unit with empty text', () => {
    const raw = readGolden('toolbox', 'empty.txt');
    const imported = importFromToolbox(raw);
    // Either 0 units or 1 with empty transcription — both acceptable
    if (imported.units.length > 0) {
      expect(imported.units[0]!.transcription.trim()).toBe('');
    }
  });
  ...
  it('three-records.txt — 3 records with mb/ge/ps interlinear data', () => {
    ...
    expect(imported.units).toHaveLength(3);
    expect(imported.units[0]!.transcription).toBe('tɕʰa mo');
    expect(imported.units[1]!.transcription).toBe('ŋa joŋs pa yin');
    expect(imported.units[2]!.transcription).toBe('de ring');

    // Check word/morpheme structure
    expect(imported.units[0]!.tokens).toHaveLength(2);
    expect(imported.units[0]!.tokens![0]!.morphemes![0]!.gloss!.eng).toBe('tea');
    expect(imported.units[1]!.tokens).toHaveLength(4);
    expect(imported.units[1]!.tokens![0]!.morphemes![0]!.gloss!.eng).toBe('1SG');
...
describe('Golden Round-trip: FLExText', () => {
  it('minimal.flextext — 2 phrases, no words or glosses', () => {
    ...
    const exported = exportToFlextext({ units, layers: [], translations: [] });
    const reimported = importFromFlextext(exported);
    expect(reimported.units).toHaveLength(2);
    expect(reimported.units[0]!.transcription).toBe('tɕʰa mo ɣa ra');
    expect(reimported.units[1]!.transcription).toBe('ŋa joŋs pa yin');
  });
  ...
  it('interlinear-ipa.flextext — 3 phrases with word+morpheme glosses', () => {
    ...
    const words0 = imported.units[0]!.tokens;
  ...
  it('empty.flextext — empty phrases produces 0 units', () => {
