/**
 * PR-12: Segment QA Reflection — lightweight self-check.
...
export interface SegmentQaReflectionResult {
  reflectionFlagged: boolean;
  checks: SegmentQaReflectionCheck[];
  summary: string;
}
...
const MAX_ACCEPTABLE_CONFIDENCE = 1.0;
const MIN_ACCEPTABLE_CONFIDENCE = 0.0;
const LOW_CONFIDENCE_THRESHOLD = 0.5;

/**
 * Run lightweight reflection checks on a segment_qa turn.
 * Does not block user-visible output; intended to run after stream completion.
 */
export function runSegmentQaReflection(
  assistantContent: string,
  evidencePackets: readonly EvidencePacketV0[],
): SegmentQaReflectionResult {
  const checks: SegmentQaReflectionCheck[] = [];

  // Check 1: citation marker count vs evidence packet count
  const citationMarkers = assistantContent.match(CITATION_MARKER_RE) ?? [];
  ...
  const outOfBounds = evidencePackets.filter(
    (ep) => typeof ep.confidence === 'number' && (ep.confidence < MIN_ACCEPTABLE_CONFIDENCE || ep.confidence > MAX_ACCEPTABLE_CONFIDENCE),
  );
  checks.push({
 ...
 * Injects specific guidance so the LLM can correct the flagged issues
 * on the next attempt.
 */
export function buildReflectionRetryPrompt(result: SegmentQaReflectionResult): string {
