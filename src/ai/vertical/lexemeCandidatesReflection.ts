/**
 * PR-P4-2: Lexeme Candidates Reflection — lightweight self-check.
...
export interface LexemeCandidatesReflectionResult {
  reflectionFlagged: boolean;
  checks: LexemeCandidatesReflectionCheck[];
  summary: string;
}
...
const CANDIDATE_MARKER_RE = /\[\d+\]/g;
const LOW_CONFIDENCE_THRESHOLD = 0.5;

/**
 * Run lightweight reflection checks on a lexeme_candidates turn.
 * Does not block user-visible output; intended to run after stream completion.
 */
export function runLexemeCandidatesReflection(
  assistantContent: string,
  evidencePackets: readonly EvidencePacketV0[],
): LexemeCandidatesReflectionResult {
  const checks: LexemeCandidatesReflectionCheck[] = [];
  const evidenceCount = evidencePackets.length;

  // Check 1: citation marker count vs evidence packet count
  const citationMarkers = assistantContent.match(CANDIDATE_MARKER_RE) ?? [];
  ...
  // Check 6: lexeme candidates should contain structured entries
  const hasCandidateStructure = /(lexeme|gloss|pos|candidate|form)/i.test(assistantContent);
  checks.push({
    name: 'candidates_structure_present',
    passed: hasCandidateStructure || assistantContent.length < 100,
    detail: hasCandidateStructure
      ? 'candidate structure markers detected'
 ...
 */
export function buildLexemeCandidatesReflectionRetryPrompt(result: LexemeCandidatesReflectionResult): string {
  ...
  for (const check of failed) {
    switch (check.name) {
      ...
      case 'quote_nonempty':
        ...
        break;
      case 'candidates_structure_present':
        lines.push('- Please structure candidates using clear markers (lexeme/gloss/pos/candidate/form).');
        break;
