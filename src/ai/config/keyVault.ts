...
export interface KeyVaultBackend<TValue> {
  readonly kind: string;
  load(): Promise<TValue | null>;
  persist(value: TValue, options?: { isStale?: () => boolean }): Promise<void>;
  clear(): Promise<void>;
}
...
export class BrowserKeyVault implements KeyVaultBackend<AiChatSettings> {
  ...
  async persist(settings: AiChatSettings, options?: { isStale?: () => boolean }): Promise<void> {
...
export const browserAiChatKeyVault = new BrowserKeyVault();
