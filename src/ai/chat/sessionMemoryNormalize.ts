...
export function newSummaryEntryId(): string {
  return `sum_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;
}

export const SESSION_MEMORY_SUMMARY_WARNING_DEFAULT_THRESHOLD = SUMMARY_WARNING_DEFAULT_THRESHOLD;
export const SESSION_MEMORY_MAX_SUMMARY_CHAIN_LENGTH = MAX_SUMMARY_CHAIN_LENGTH;
...
export function normalizeSessionMemory(memory: AiSessionMemory): AiSessionMemory {
