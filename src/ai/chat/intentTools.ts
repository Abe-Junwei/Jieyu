...
function getUnits(context: AiPromptContext): TimelineUnitView[] {
  ...
  return rows.filter((row): row is TimelineUnitView =>
    Boolean(
      row &&
...
): Array<{ layerId: string; count: number }> {
...
function getScopedUnits(context: AiPromptContext, scope: QualityScope): TimelineUnitView[] {
  ...
  const currentMediaId =
    typeof context.shortTerm?.currentMediaId === 'string' ? context.shortTerm.currentMediaId : '';
  const trackScoped =
    currentMediaId.length > 0 ? units.filter((unit) => unit.mediaId === currentMediaId) : units;
  if (scope === 'current_track') return trackScoped;

  const selectedLayerId =
    typeof context.shortTerm?.selectedLayerId === 'string'
...
export function findIncompleteUnits(
  ...
  const items = incompleteUnits.slice(0, limit).map((unit) => ({
    id: unit.id,
    kind: unit.kind,
...
export function diagnoseQuality(
  context: AiPromptContext,
  args: Record<string, unknown> = {},
): IntentToolResult {
  ...
  const emptyText = units.filter(
    (unit) => (typeof unit.text === 'string' ? unit.text.trim().length : 0) === 0,
  );
