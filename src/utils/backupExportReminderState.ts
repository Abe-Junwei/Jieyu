...
const BACKUP_REMINDER_ENABLED_KEY = 'jieyu.settings.backupReminderEnabled';
const BACKUP_LAST_FULL_EXPORT_AT_KEY = 'jieyu.backup.lastFullExportAt';
const BACKUP_REMINDER_SNOOZE_UNTIL_KEY = 'jieyu.backup.reminderSnoozeUntil';
const BACKUP_FIRST_APP_OPEN_AT_KEY = 'jieyu.backup.firstAppOpenAt';
const BACKUP_LAST_REMINDER_TOAST_AT_KEY = 'jieyu.backup.lastReminderToastAt';
...
const BACKUP_DIRTY_SINCE_EXPORT_KEY = 'jieyu.backup.dirtySinceLastExport';

/** 已触发提醒后，同一条目下的最短间隔 | Min spacing between reminder toasts */
const BACKUP_REMINDER_TOAST_COOLDOWN_MS = 24 * 60 * 60 * 1000;
 ...
 */
const BACKUP_REMINDER_DIRTY_MIN_AFTER_EXPORT_MS = 24 * 60 * 60 * 1000;

/** 已有导出记录后的提醒间隔 | Interval once user has exported at least once */
const BACKUP_REMINDER_INTERVAL_AFTER_EXPORT_MS = 7 * 24 * 60 * 60 * 1000;
/** 从未导出时，自首次记录打开起的宽限期 | Grace period before first nag when never exported */
const BACKUP_REMINDER_FIRST_NAG_AFTER_MS = 14 * 24 * 60 * 60 * 1000;
