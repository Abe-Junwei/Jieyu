...
describe('transcriptionUrlDeepLink', () => {
  ...
  it('buildTranscriptionDeepLinkHref encodes textId and optional keys', () => {
    ...
    expect(buildTranscriptionDeepLinkHref({ textId: 't', unitId: 's', unitKind: 'segment' })).toBe(
      '/transcription?textId=t&unitId=s&unitKind=segment',
    );
    expect(
      buildTranscriptionDeepLinkHref({
        textId: 't',
    ...
    ).toBe('/transcription?textId=t&unitId=u1&lexiconReturn=lex-9');
  });

  it('keeps lexiconReturn when stripping transcription selection keys', () => {
    const prev = new URLSearchParams(
      'textId=t1&mediaId=m1&layerId=l1&unitId=u1&unitKind=segment&lexiconReturn=lex-9&keep=1',
    );
    const next = stripTranscriptionDeepLinkSearchParams(prev);
    expect(next.get('keep')).toBe('1');
    expect(next.get('lexiconReturn')).toBe('lex-9');
    expect(next.get('textId')).toBeNull();
    expect(next.get('unitId')).toBeNull();
  });
  ...
  it('round-trips workspace return hint via sessionStorage', () => {
    ...
    expect(buildTranscriptionWorkspaceReturnHref()).toBe(
      '/transcription?textId=tid-9&mediaId=mid-1',
    );
