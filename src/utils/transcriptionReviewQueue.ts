...
} from '../components/sidePaneSegmentListViewModel';
...
export function buildTranscriptionReviewItems(
  ...
  const assignedSpeakerCount = reviewableUnits.filter(
    (unit) => (unit.speakerId?.trim() ?? '').length > 0,
  ).length;
  const allowSpeakerPending =
    reviewableUnits.length > 0 &&
  ...
  return reviewableUnits.flatMap((unit, index, rows) => {
    ...
    const normalizedSpeakerId = unit.speakerId?.trim() ?? '';
    const flags = computeSegmentReviewIssueFlags(
      {
        ...
        speakerKeys: normalizedSpeakerId.length > 0 ? [normalizedSpeakerId] : [],
...
export function countTranscriptionReviewPresets(
  items: TranscriptionReviewItem[],
): Record<TranscriptionReviewPreset, number> {
  return {
    all: items.length,
    time: items.filter((item) => matchesReviewPreset(item.flags, 'time')).length,
    content_concern: items.filter((item) => matchesReviewPreset(item.flags, 'content_concern'))
      .length,
    content_missing: items.filter((item) => matchesReviewPreset(item.flags, 'content_missing'))
      .length,
    manual_attention: items.filter((item) => matchesReviewPreset(item.flags, 'manual_attention'))
      .length,
    pending_review: items.filter((item) => matchesReviewPreset(item.flags, 'pending_review'))
      .length,
  };
}

export function buildTranscriptionReviewQueue(
  unitsOnCurrentMedia: LayerUnitDocType[],
): LayerUnitDocType[] {
