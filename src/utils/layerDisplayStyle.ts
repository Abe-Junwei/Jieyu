...
function collectOrthographyExemplarCharacters(orthography?: OrthographyDocType): string[] {
  ...
  return dedupeNonEmptyStrings([...main, ...auxiliary, ...numbers, ...punctuation, ...index]);
}

function buildOrthographyCoverageSummary(
  orthography?: OrthographyDocType,
): OrthographyCoverageSummary {
...
type StoredFontCoverageVerification = {
  status: Extract<
    FontCoverageVerification['status'],
    'verified' | 'missing-glyphs' | 'shaping-risk'
  >;
  checkedAt: string;
  sampleText: string;
};
...
const SCRIPT_FONT_MAP: Record<string, FontPreset[]> = {
  ...
  Tibt: [
    {
      key: 'Noto Sans Tibetan',
      css: '"Noto Sans Tibetan", sans-serif',
      ipaSupport: false,
      license: 'OFL',
    },
    {
      key: 'Noto Serif Tibetan',
      css: '"Noto Serif Tibetan", serif',
      ipaSupport: false,
      license: 'OFL',
    },
  ],
  ...
  Beng: [
    {
      ...
      css: '"Noto Sans Bengali", sans-serif',
      ipaSupport: false,
      license: 'OFL',
    },
  ],
  ...
  Mymr: [
    {
      ...
      css: '"Noto Sans Myanmar", sans-serif',
      ipaSupport: false,
      license: 'OFL',
    },
  ...
  Ethi: [
    {
      key: 'Noto Sans Ethiopic',
      css: '"Noto Sans Ethiopic", sans-serif',
      ipaSupport: false,
      license: 'OFL',
    },
  ...
  Khmr: [
    {
      key: 'Noto Sans Khmer',
      css: '"Noto Sans Khmer", sans-serif',
      ipaSupport: false,
      license: 'OFL',
    },
  ...
  Guru: [
    {
      ...
      css: '"Noto Sans Gurmukhi", sans-serif',
      ipaSupport: false,
      license: 'OFL',
    },
  ],
  Mlym: [
    {
      ...
      css: '"Noto Sans Malayalam", sans-serif',
      ipaSupport: false,
      license: 'OFL',
    },
  ],
  Telu: [
    {
      ...
      css: '"Noto Sans Telugu", sans-serif',
      ipaSupport: false,
      license: 'OFL',
    },
  ],
  Knda: [
    {
      ...
      css: '"Noto Sans Kannada", sans-serif',
      ipaSupport: false,
      license: 'OFL',
    },
  ],
  Sinh: [
    {
      ...
      css: '"Noto Sans Sinhala", sans-serif',
      ipaSupport: false,
      license: 'OFL',
    },
  ],
  ...
  Geor: [
    {
      ...
      css: '"Noto Sans Georgian", sans-serif',
      ipaSupport: false,
      license: 'OFL',
    },
  ],
  Hebr: [
    {
      ...
      css: '"Noto Sans Hebrew", sans-serif',
      ipaSupport: false,
      license: 'OFL',
    },
  ],
...
const SCRIPT_DEFAULT_DIRECTION: Partial<
  Record<string, NonNullable<OrthographyDocType['direction']>>
> = {
  Arab: 'rtl',
  Hebr: 'rtl',
};
...
export function isIpaModeForLanguage(
  ...
  const orthography = resolveOrthographyForLanguage(
    languageId,
    orthographies,
 ...
 */
function getFontPresetsForScript(scriptTag: string, ipaMode = false): FontPreset[] {
  const scriptFonts = SCRIPT_FONT_MAP[scriptTag] ?? SCRIPT_FONT_MAP['Latn']!;
  const allFonts = [UNIVERSAL_FONTS[0]!, ...scriptFonts, ...UNIVERSAL_FONTS.slice(1)];
...
function resolveLocalizedLocalFontName(
  font: LocalFontEntry,
  scriptTag: string,
): string | undefined {
...
function isLocalFontCompatibleWithScript(font: LocalFontEntry, scriptTag: string): boolean {
  ...
  return getLocalFontScriptHints(scriptTag).some((hint) =>
    searchBlob.includes(normalizeFontSearchValue(hint)),
  );
}
...
function buildRuntimeFontKeys(
  ...
  const scriptPresetKeys = (SCRIPT_FONT_MAP[scriptTag] ?? SCRIPT_FONT_MAP['Latn'] ?? []).map(
    (preset) => preset.key,
  );
  ...
  return dedupeFontKeys([...preferredFontKeys, ...fallbackFontKeys, ...scriptPresetKeys]);
}

function buildSelectedFontStackCss(
  fontKey: string,
  renderPolicy?: OrthographyRenderPolicy,
): string {
  if (!renderPolicy) return resolveFontCss(fontKey);
  return buildFontStackCss([
    fontKey,
    ...renderPolicy.resolvedFontKeys.filter((key) => key !== fontKey),
  ]);
}
...
export function resolveOrthographyRenderPolicy(
  ...
  const resolvedFontKeys = buildRuntimeFontKeys(
    scriptTag,
    ipaMode,
    preferredFontKeys,
    fallbackFontKeys,
  ...
  const lineHeightScale =
    orthography?.fontPreferences?.lineHeightScale ?? getScriptLineHeightFactor(scriptTag);
  return {
...
export function describePresetFontCoverage(
  ...
  const baseLabel = renderPolicy.preferredFontKeys.includes(fontKey)
    ...
    : renderPolicy.fallbackFontKeys.includes(fontKey)
      ? '回退'
      : renderPolicy.coverageSummary.confidence === 'sample-backed'
        ? '推荐'
        : '脚本推荐';
...
export function describeLocalFontCoverage(
  ...
  const baseLabel = renderPolicy.preferredFontKeys.includes(font.family)
    ...
    : renderPolicy.fallbackFontKeys.includes(font.family)
      ? '回退'
      : isLocalFontCompatibleWithScript(font, renderPolicy.scriptTag)
        ? '脚本匹配'
        : '风险高';
  if (
    verification?.status === 'verified' &&
    !isShapingRiskLocalFont(font, renderPolicy, verification)
  ) {
