...
export function pushFieldIfPrimitive(
...
function collectPrimitivePayloadFields(
...
export function extractConflictFields(
  input: ExtractConflictFieldsInput,
): Record<string, FieldValue> {
  ...
  if (input.opType === 'upsert_relation') {
    ...
    const hasTranscriptionLayerKey =
      typeof transcriptionLayerKey === 'string'
        ? transcriptionLayerKey.length > 0
        : transcriptionLayerKey !== null && transcriptionLayerKey !== undefined;
    pushFieldIfPrimitive(
      fields,
      'hostTranscriptionLayerId',
      hostTranscriptionLayerId ?? (hasTranscriptionLayerKey ? undefined : entityHostToken),
    );
...
export function toPresencePersistKey(record: CollaborationPresenceRecord): string {
  return [record.state, record.focusedEntityType ?? '', record.focusedEntityId ?? ''].join('|');
}
