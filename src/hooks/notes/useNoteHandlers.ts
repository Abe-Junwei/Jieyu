...
import type { ActionableRecommendation } from '../ai/useAiPanelLogic';
...
export function useNoteHandlers(input: UseNoteHandlersInput) {
  ...
  const resolveTierAnnotationTargetId = useCallback(
    (uttId: string, layerId: string, scope: 'timeline' | 'waveform') => {
  ...
  const noteTarget = useMemo<NoteTarget | null>(() => {
    ...
    return notePopover.layerId
      ? {
          ...
          targetId: resolveTierAnnotationTargetId(notePopover.uttId, notePopover.layerId, scope),
        }
  ...
  const resolveNoteIndicatorTarget = useCallback(
    (unitId: string, layerId?: string, scope: 'timeline' | 'waveform' = 'timeline') => {
      if (layerId) {
        const layerScopedCount =
          noteCounts.get(resolveTierAnnotationTargetId(unitId, layerId, scope)) ?? 0;
  ...
  const handleOpenWordNote = useCallback(
    (unitId: string, wordId: string, event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  const handleUpdateTokenPos = useCallback(
    async (tokenId: string, pos: string | null) => {
      try {
        await updateTokenPos(tokenId, pos);
        setSaveState({
          kind: 'done',
          message: pos && pos.trim() ? messages.posUpdated : messages.posCleared,
        });
      } catch (error) {
        reportActionError({
  ...
  const handleBatchUpdateTokenPosByForm = useCallback(
    async (unitId: string, form: string, pos: string | null) => {
      try {
  ...
  const handleExecuteRecommendation = useCallback(
    async (item: ActionableRecommendation) => {
      if (item.actionType === 'batch_pos') {
        ...
        const updated = await handleBatchUpdateTokenPosByForm(
          item.targetUnitId,
          item.targetForm,
          item.targetPos,
        );
        if (updated > 0) {
          ...
          setSaveState({
            kind: 'done',
            message: messages.recommendationBatchApplied(updated, item.targetForm, item.targetPos),
          });
        }
        return;
      }
      ...
      if (item.actionType === 'risk_review') {
        const confidenceText =
          typeof item.targetConfidence === 'number'
