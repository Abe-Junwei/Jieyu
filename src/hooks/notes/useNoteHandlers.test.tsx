...
describe('useNoteHandlers error reporting', () => {
  it('resolves note indicator targets for segment-layer notes and unit notes', () => {
    ...
    expect(result.current.resolveNoteIndicatorTarget('seg-1', 'layer-1')).toEqual({
      count: 2,
      layerId: 'layer-1',
    });
  ...
  it('isolates waveform note indicators from timeline note indicators', () => {
    ...
    expect(result.current.resolveNoteIndicatorTarget('seg-1', 'layer-1', 'timeline')).toEqual({
      count: 2,
      layerId: 'layer-1',
    });
    expect(result.current.resolveNoteIndicatorTarget('seg-1', 'layer-1', 'waveform')).toEqual({
      count: 1,
      layerId: 'layer-1',
    });
    expect(result.current.resolveNoteIndicatorTarget('seg-1', 'layer-1')).toEqual({
  ...
  it('executes batch_pos recommendation and keeps selection/save-state in sync', async () => {
    ...
    expect(setSaveState).toHaveBeenCalledWith({
      kind: 'done',
      message: '已批量赋值 3 个 token（walk → VERB）',
    });
  });
});
