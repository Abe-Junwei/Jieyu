...
function makeSendTurnArgs(over: { setMessages?: Dispatch<SetStateAction<UiChatMessage[]>> } = {}): RunAiChatSendTurnArgs {
...
describe('runAiChatSendTurnPersistAndPrimaryStream failure paths', () => {
  ...
  it('propagates persistOpeningTurnAndBuildPromptContext failures without starting a stream', async () => {
    vi.mocked(persistOpeningTurnAndBuildPromptContext).mockRejectedValueOnce(new Error('persist failed'));
