...
export type LayerActionPanelKind =
  | 'speaker-management'
...
export function useLayerActionPanel({
  ...
  const [quickTranscriptionConstraint, setQuickTranscriptionConstraint] =
    useState<LayerConstraint>('symbolic_association');
  ...
  const [quickTranslationModality, setQuickTranslationModality] = useState<
    'text' | 'audio' | 'mixed'
  >('text');
  const [quickTranslationConstraint, setQuickTranslationConstraint] =
    useState<LayerConstraint>('symbolic_association');
  ...
  const handleCreateTranscriptionFromPanel = useCallback(async () => {
    ...
    const hasExistingTranscriptionLayer = deletableLayers.some(
  ...
  const handleCreateTranslationFromPanel = useCallback(async () => {
    const languageId = (
      quickTranslationLangId === '__custom__' ? quickTranslationCustomLang : quickTranslationLangId
    ).trim();
  ...
  const canConfigureTranscriptionConstraint = deletableLayers.some(
    (layer) => layer.layerType === 'transcription',
  );
