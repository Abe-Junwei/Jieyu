...
export function useLayerSegmentContents(
  ...
  const [segmentContentByLayer, setSegmentContentByLayer] = useState<SegmentContentByLayer>(
    () => new Map(),
  );
  ...
  const loadSegmentContents = useCallback(async () => {
    ...
    const segmentBackedLayers = layersRef.current
      .map((layer) => ({
        layer,
        sourceLayer: resolveSegmentTimelineSourceLayer(
          layer,
          layerById,
      ...
      .filter((item): item is { layer: LayerDocType; sourceLayer: LayerDocType } =>
        Boolean(item.sourceLayer),
      );
  ...
  }, [defaultLayerIdRef, layerLinks, layersRef, mediaId, segmentsRef]);
