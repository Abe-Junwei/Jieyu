...
export function useTranscriptionCloudSyncActions({
  ...
  const buildRemoteConflictRecord = useCallback(
    (change: CollaborationProjectChangeRecord): CollaborationRecord => {
      ...
      return {
        ...
        fields,
        ...(change.opType === 'delete_entity' || payload?.deleted === true
          ? { deleted: true }
          : {}),
      };
    },
  ...
  const buildLocalSnapshotFields = useCallback(
    (change: CollaborationProjectChangeRecord): Record<string, FieldValue> => {
      ...
      if (change.entityType === 'unit_relation') {
        ...
        const hostLayer = layersRef.current.find(
          (layer) =>
        ...
        const exists = layerLinksRef.current.some(
          (link) =>
  ...
  const applyRemoteMutation = useCallback(
    async (
      ...
      if (change.opType === 'upsert_unit_content') {
        ...
        if (unitId) {
          await runWithDbMutex(() =>
            rawActionsRef.current.saveUnitText(unitId, payloadValue, payloadLayerId ?? undefined),
          );
          mutated = true;
        }
      }
      ...
      if (change.opType === 'upsert_unit') {
        ...
        if (fullUnit) {
          await runWithDbMutex(() =>
            LinguisticService.saveUnit(
              fullUnit as unknown as import('../db').LayerUnitDocType,
            ).then(() => undefined),
          );
          mutated = true;
        } else {
          ...
          if (unitId && startTime !== null && endTime !== null) {
            await runWithDbMutex(() =>
              rawActionsRef.current.saveUnitTiming(unitId, startTime, endTime),
            );
            mutated = true;
          }
        }
      ...
      if (change.opType === 'batch_patch') {
        ...
        if (
          ...
          await runWithDbMutex(() =>
            rawActionsRef.current.saveUnitSelfCertainty(
              unitIds,
              certaintyValue as UnitSelfCertainty | undefined,
            ),
          );
          mutated = true;
        }
        ...
        if (action === 'layer-fields' && unitIds.length > 0 && patchRecord) {
          await runWithDbMutex(() =>
            rawActionsRef.current.saveUnitLayerFields(
              unitIds,
              patchRecord as PerLayerRowFieldPatch,
            ),
          );
          mutated = true;
        }
      ...
      if (change.opType === 'upsert_layer') {
        ...
        if (fullLayer) {
          await runWithDbMutex(() =>
            LinguisticService.upsertLayer(
              fullLayer as unknown as import('../db').LayerDocType,
            ).then(() => undefined),
          );
          mutated = true;
        }
      ...
      if (change.opType === 'upsert_relation') {
        ...
        if (resolvedHostKey && layerId && enabled !== undefined) {
          ...
          if (exists !== enabled) {
            await runWithDbMutex(() =>
              rawActionsRef.current.toggleLayerLink(resolvedHostKey, layerId),
            );
            mutated = true;
          }
        }
      ...
      if (change.opType === 'delete_entity') {
        ...
        if (change.entityType === 'layer') {
          ...
          if (layerId && layerId !== 'layer') {
            const keepUnits =
              typeof payload?.keepUnits === 'boolean' ? payload.keepUnits : undefined;
            await runWithDbMutex(() =>
              rawActionsRef.current.deleteLayer(
                layerId,
                keepUnits === undefined ? undefined : { keepUnits },
              ),
            );
            mutated = true;
          }
    ...
    [layerLinksRef, layersRef, loadSnapshot, runWithDbMutex],
  );
  ...
  const applyRemoteChangeToLocal = useCallback(
    ...
    [appendConflictLogs, applyRemoteMutation, buildLocalConflictRecord, buildRemoteConflictRecord],
  );
  ...
  const listAccessibleCloudProjects = useCallback(
    async (): Promise<CollaborationCloudDirectoryProject[]> => fetchAccessibleCloudProjects(),
    [],
  );
  ...
  const restoreProjectSnapshotToLocalById = useCallback(
    async (snapshotId: string): Promise<CollaborationProjectSnapshotRecord> => {
      ...
      await runWithDbMutex(() =>
        LinguisticService.importFromJSON(restored.payloadJson, 'replace-all').then(() => undefined),
      );
      await loadSnapshot();
          ...
          if (latestSnapshot) {
            try {
              ...
              if (isImportableDatabaseSnapshot(parsedPayload)) {
                await runWithDbMutex(() =>
                  LinguisticService.importFromJSON(restored.payloadJson, 'replace-all').then(
                    () => undefined,
                  ),
                );
                if (cancelled) return;
