...
interface UseVoiceAgentStartControllerOptions {
  ...
  commercialProviderKindRef: RefLike<
    'gemini' | 'openai-audio' | 'groq' | 'custom-http' | 'minimax' | 'volcengine'
  >;
  ...
  setSession: React.Dispatch<
    React.SetStateAction<import('../../services/IntentRouter').VoiceSession>
  >;
...
function resolveEffectiveLang(langOverride: string | null | undefined, corpusLang: string): string {
...
export function useVoiceAgentStartController({
  ...
  return useCallback(
    async (targetMode?: VoiceAgentMode) => {
      if (listening) return;
      ...
      const p = (async (): Promise<void> => {
        ...
        aliasMapRef.current = await loadAliasMap(
          loadIntentRouterRuntime,
          loadVoiceIntentRefineRuntime,
        );
        ...
        if (
          (runtimeEngine === 'commercial' && commercialProviderConfigRef.current) ||
          sttEnhancementKindRef.current !== 'none'
        ) {
          const { createCommercialProvider, createSttEnhancementProvider } = await loadSttRuntime();
          if (abortStaleMicStart(gen, voiceActivateGenerationRef, serviceRef, resetStartUi)) return;

          if (sttEnhancementKindRef.current !== 'none') {
            startConfig.sttEnhancement = createSttEnhancementProvider(
              sttEnhancementKindRef.current,
            );
        ...
        try {
          ...
          setAgentState('idle');
          setError(
            err instanceof Error ? err.message : t(locale, 'transcription.voice.error.startFailed'),
          );
          Earcon.playError();
          return;
        }
