...
export function loadCommercialSttConfig(): {
...
export function saveCommercialSttConfig(
  kind: CommercialProviderKind,
  config: CommercialProviderConfig,
): void {
  try {
    window.localStorage.setItem(
      ...
      JSON.stringify({ kind, config: sanitizeCommercialConfig(config) }),
    );
  } catch (error) {
...
export function saveSttEnhancementSelection(
  kind: SttEnhancementSelectionKind,
  config: VoiceSttEnhancementConfig,
): void {
  try {
    window.localStorage.setItem(
      VOICE_STT_ENHANCEMENT_STORAGE_KEY,
      JSON.stringify({ kind, config: sanitizeEnhancementConfig(config) }),
    );
  } catch (error) {
...
export function useVoiceDock({
  ...
  const [voiceDockPos, setVoiceDockPos] = useState<{ right: number; bottom: number }>({
    right: 16,
    bottom: 16,
  });
  const [voiceDockDragging, setVoiceDockDragging] = useState(false);
  const voiceDockDragRef = useRef<{
    pointerId: number;
    startX: number;
    startY: number;
    startRight: number;
  ...
  const [commercialProviderKind, setCommercialProviderKind] = useState<CommercialProviderKind>(
    () => loadCommercialSttConfig().kind,
  );
  ...
  const [sttEnhancementKind, setSttEnhancementKind] = useState<SttEnhancementSelectionKind>(
    () => loadSttEnhancementSelection().kind,
  );
  const [sttEnhancementConfig, setSttEnhancementConfig] = useState<VoiceSttEnhancementConfig>(
    () => loadSttEnhancementSelection().config,
  );
  ...
  useEffect(() => {
    ...
    fireAndForget(
      ...
      { context: 'src/hooks/voice/useVoiceDock.ts:L179', policy: 'background-quiet' },
    );
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    try {
      ...
      log.error('failed to read voice dock position from localStorage', { err });
    }
  }, []);
  ...
  const effectiveVoiceCorpusLang = (
  ...
  const handleVoiceDockDragStart = useCallback(
