...
export function useVoiceAgentProviderControls(input: UseVoiceAgentProviderControlsInput) {
  ...
  const [enhancementStatus, setEnhancementStatus] = useState<SttEnhancementReachability | null>(
    null,
  );
  ...
  const refreshProviderStatus = useCallback(async () => {
    ...
    const [results, nextEnhancementStatus] = await Promise.all([
      ...
      probeSelectedSttEnhancement(sttEnhancementKindState, sttEnhancementConfigState ?? {}),
    ]);
  ...
  const testCommercialProvider = useCallback(async (): Promise<{
    available: boolean;
    error?: string;
  }> => {
