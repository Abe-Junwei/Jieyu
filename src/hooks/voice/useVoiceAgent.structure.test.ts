...
function readUseVoiceAgentCode() {
  const filePath = path.resolve(process.cwd(), 'src/hooks/voice/useVoiceAgent.ts');
  return fs.readFileSync(filePath, 'utf8');
}

function readUseVoiceAgentRuntimeCode() {
  const filePath = path.resolve(process.cwd(), 'src/hooks/voice/useVoiceAgent.runtime.ts');
  return fs.readFileSync(filePath, 'utf8');
}

function readUseVoiceAgentWakeWordCode() {
  const filePath = path.resolve(process.cwd(), 'src/hooks/voice/useVoiceAgentWakeWord.ts');
  return fs.readFileSync(filePath, 'utf8');
}

function readUseVoiceAgentStartControllerCode() {
  const filePath = path.resolve(process.cwd(), 'src/hooks/voice/useVoiceAgentStartController.ts');
  return fs.readFileSync(filePath, 'utf8');
}

function readUseVoiceAgentTransportControlsCode() {
  const filePath = path.resolve(process.cwd(), 'src/hooks/voice/useVoiceAgentTransportControls.ts');
  return fs.readFileSync(filePath, 'utf8');
}
...
describe('useVoiceAgent structure invariants', () => {
  it('keeps heavy voice runtimes behind lazy imports (shared voiceRuntimeLoaders)', () => {
    ...
    expect(
      code.includes("import { VoiceInputService } from '../../services/VoiceInputService';"),
    ).toBe(false);
    expect(
      code.includes("import { WakeWordDetector } from '../../services/WakeWordDetector';"),
    ).toBe(false);
  });
  ...
  it('keeps service instantiation centralized to one runtime entry per service', () => {
    ...
    expect(code.includes('const serviceRef = useRef<VoiceInputServiceType | null>(null);')).toBe(
      true,
    );
    expect(
      code.includes('const wakeWordDetectorRef = useRef<WakeWordDetectorType | null>(null);'),
    ).toBe(true);
    expect(code.includes("import { useVoiceAgentWakeWord } from './useVoiceAgentWakeWord';")).toBe(
      true,
    );
    expect(
      code.includes(
  ...
  it('keeps runtime cleanup centralized for both voice services', () => {
    ...
    expect(
      code.includes('const exclusiveStartPromiseRef = useRef<Promise<void> | null>(null);'),
    ).toBe(true);
