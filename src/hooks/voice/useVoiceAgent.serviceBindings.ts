...
export function cleanupVoiceInputSubscriptions(
  unsubscribesRef: MutableRefObject<Array<() => void>>,
) {
...
export function bindVoiceInputService(options: BindVoiceInputServiceOptions) {
  ...
  options.unsubscribesRef.current.push(
    options.service.onError((error) => {
  ...
  if (
    'onVadStateChange' in options.service &&
    typeof options.service.onVadStateChange === 'function'
  ) {
  ...
  if ('onEnergyLevel' in options.service && typeof options.service.onEnergyLevel === 'function') {
    options.unsubscribesRef.current.push(
      options.service.onEnergyLevel((rms) => {
