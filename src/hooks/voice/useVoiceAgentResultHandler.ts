...
interface UseVoiceAgentResultHandlerOptions {
  ...
  resolveIntentWithLlmRef: RefLike<
    | ((input: {
        text: string;
        mode: VoiceAgentMode;
...
function updateDisambiguationOptions(
  ...
  if (
    ...
    setDisambiguationOptions(
      intentRouter.collectAlternativeIntents(sttText, intent.actionId, confidence),
    );
    return;
  }
  setDisambiguationOptions([]);
...
async function resolveVoiceIntentFromResult(options: {
  ...
  const { intent, llmFallbackFailed, nextAliasMap, errorMessage } = await resolveVoiceIntent(
    ...
    },
    {
      ...
      ...(options.resolveIntentWithLlm !== undefined && {
        resolveIntentWithLlm: options.resolveIntentWithLlm,
      }),
    },
  );
  if (errorMessage) {
...
export function useVoiceAgentResultHandler({
  ...
  return useCallback(
    async (result: SttResult) => {
      ...
      const { intentRouter, intent, llmFallbackFailed, nextAliasMap } =
        await resolveVoiceIntentFromResult({
          result,
          currentMode,
      ...
      await runVoiceFinalSttAfterIntentResolution({
        ...
        ...(insertDictationRef.current !== undefined
          ? { insertDictation: insertDictationRef.current }
          : {}),
        ...(executeVoiceToolCallRef.current !== undefined
          ? { executeVoiceToolCall: executeVoiceToolCallRef.current }
          : {}),
        queueAiThinking,
        setError,
        setAgentState,
