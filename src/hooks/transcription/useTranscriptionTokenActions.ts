...
export function useTranscriptionTokenActions({ runWithDbMutex, setUnits }: Params) {
  const updateTokenPos = useCallback(
    async (tokenId: string, pos: string | null) => {
      await runWithDbMutex(() => LinguisticService.units.updateTokenPos(tokenId, pos));
  ...
  const batchUpdateTokenPosByForm = useCallback(
    async (unitId: string, form: string, pos: string | null, orthographyKey = 'default') => {
      const updated = await runWithDbMutex(() =>
        LinguisticService.units.batchUpdateTokenPosByForm(unitId, form, pos, orthographyKey),
      );
      ...
      setUnits((prev) =>
        prev.map((unit) => {
          ...
          const nextWords = unit.words.map((word) => {
            const match =
              word.form[orthographyKey] === normalizedForm ||
  ...
  const updateTokenGloss = useCallback(
    async (tokenId: string, gloss: string | null, lang = 'eng') => {
      await runWithDbMutex(() => LinguisticService.units.updateTokenGloss(tokenId, gloss, lang));
