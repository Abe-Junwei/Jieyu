...
describe('useTranscriptionCloudSyncActions conflict governance', () => {
  ...
  it('auto-resolves low risk conflicts and applies remote mutation', async () => {
    ...
    await act(async () => {
      await triggerRemote(
        createRemoteContentChange('remote text', new Date(Date.now() - 10_000).toISOString()),
      );
    });
    ...
    await waitFor(() => {
      ...
      expect(
        result.current.conflictOperationLogs.some((item) => item.type === 'conflict_resolved'),
      ).toBe(true);
    });
  });
  ...
  it('persists conflict governance logs into audit storage', async () => {
    ...
    await act(async () => {
      await triggerRemote(
        createRemoteContentChange('remote-persist', new Date(Date.now() - 10_000).toISOString()),
      );
    });
  ...
  it('allows keeping local state and closing manual review ticket', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockRawSaveUnitText).toHaveBeenCalledTimes(0);
      expect(
        result.current.conflictOperationLogs.some((item) =>
          item.decisionId?.includes(':keep-local'),
        ),
      ).toBe(true);
    });
  });
  ...
  it('completes manual-review recovery flow from inbound conflict to later apply-remote', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        result.current.conflictOperationLogs.some((item) =>
          item.decisionId?.includes(':apply-remote'),
        ),
      ).toBe(true);
    });
  });
