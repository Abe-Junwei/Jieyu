 ...
 */
export function useTimelineUnitViewIndex(
  input: UseTimelineUnitViewIndexInput,
): TimelineUnitViewIndexWithEpoch {
  ...
  return useMemo(() => {
    ...
    return buildTimelineUnitViewIndex({
      ...
      ...(input.segmentsLoadComplete !== undefined
        ? { segmentsLoadComplete: input.segmentsLoadComplete }
        : {}),
      ...(input.transcriptionLaneReadScope
        ? { transcriptionLaneReadScope: input.transcriptionLaneReadScope }
        : {}),
    });
  }, [
    input.existingIndex,
