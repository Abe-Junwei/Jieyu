...
export function stripSpeakerAssociationFromTranslationText(
  doc: LayerUnitContentDocType,
): LayerUnitContentDocType {
  const {
    recordedBySpeakerId: _recordedBySpeakerId,
    speakerId: _speakerId,
    ...rest
  } = doc as LegacySpeakerLinkedUnitText;
  return rest as LayerUnitContentDocType;
}

export function formatRollbackFailureMessage(
  locale: Locale,
  actionKey: UndoLabelKey,
  error: unknown,
): string {
...
export function resolveProjectionLayerIdsForNewUnit(
  ...
  if (
    ...
    targetLayer.constraint === 'symbolic_association'
  ) {
    ...
    if (
      parentLayer &&
      parentLayer.layerType === 'transcription' &&
      parentLayer.constraint === 'independent_boundary'
    ) {
