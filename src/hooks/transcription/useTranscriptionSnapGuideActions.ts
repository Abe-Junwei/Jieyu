...
export function useTranscriptionSnapGuideActions({ unitsRef }: Params) {
  const getNeighborBounds = useCallback(
    (unitId: string, mediaId: string | undefined, probeStart: number) => {
      ...
      const timeline = [
        ...siblings,
        { id: unitId, startTime: probeStart, endTime: probeStart + 0.1 },
      ].sort((a, b) => a.startTime - b.startTime);
  ...
  const makeSnapGuide = useCallback(
    (
      ...
      const nearRight =
        typeof bounds.right === 'number' ? Math.abs(end - bounds.right) <= threshold : false;
      const nearSide =
        nearLeft && nearRight ? 'both' : nearLeft ? 'left' : nearRight ? 'right' : undefined;
      return {
        visible: true,
        left: bounds.left,
