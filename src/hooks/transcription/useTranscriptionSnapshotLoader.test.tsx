...
describe('useTranscriptionSnapshotLoader', () => {
  ...
  it('hydrates selectedTimelineUnit with non-empty layerId', async () => {
    ...
    const setTranslations =
      vi.fn<(next: React.SetStateAction<LayerUnitContentDocType[]>) => void>();
    ...
    const setSelectedTimelineUnit = vi.fn((next: React.SetStateAction<TimelineUnit | null>) => {
      const prev =
        selectedTimelineUnitCalls.length > 0
          ? selectedTimelineUnitCalls[selectedTimelineUnitCalls.length - 1]!
          : null;
    ...
    const mediaIdSetter =
      setSelectedMediaId.mock.calls[setSelectedMediaId.mock.calls.length - 1]![0];
  ...
  it('sets ready unifiedUnitCount using merged unit + segment semantics while unitCount remains unit rows', async () => {
    ...
    const setSelectedTimelineUnit =
      vi.fn<(next: React.SetStateAction<TimelineUnit | null>) => void>();
    const setSelectedMediaId = vi.fn<(next: React.SetStateAction<string>) => void>();
  ...
  it('scopes layers, media, and default selection to scopeTextId instead of globally first unit textId', async () => {
    const now = new Date().toISOString();
    ...
    const setUnits = vi.fn((next: React.SetStateAction<LayerUnitDocType[]>) => {
      capturedUnits = typeof next === 'function' ? next(capturedUnits) : next;
    });
    const setSelectedUnitIds = vi.fn((next: React.SetStateAction<Set<string>>) => {
      capturedSelectedUnitIds = typeof next === 'function' ? next(capturedSelectedUnitIds) : next;
    });
