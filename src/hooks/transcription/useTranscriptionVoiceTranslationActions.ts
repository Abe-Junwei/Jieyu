...
function sanitizeCommercialConfig(
  config: CommercialProviderConfig | undefined,
...
function readJsonFromLocalStorage<T>(
  key: string,
...
function loadCommercialSttConfigFromStorage(): {
...
export function useTranscriptionVoiceTranslationActions({
  ...
  const listSegmentTranslations = useCallback(
    async (
      db: Awaited<ReturnType<typeof getDb>>,
      segmentId: string,
      layerId: string,
  ...
  const resolveVoiceTranslationTargets = useCallback(
    async (
      db: Awaited<ReturnType<typeof getDb>>,
  ...
  const saveVoiceTranslation = useCallback(
    async (blob: Blob, targetUnit: LayerUnitDocType, targetLayer: LayerDocType) => {
      ...
      const { standaloneSegmentTarget, existingAudioTranslation, newestSegmentRowForLayer } =
      ...
      const translationId =
        ...
        (standaloneSegmentTarget ? `segc_${targetLayer.id}_${targetUnit.id}` : newId('utr'));
      const newTranslation: LayerUnitContentDocType = existingTranslation
        ? {
            ...existingTranslation,
            modality:
              existingTranslation.modality === 'mixed' || Boolean(existingTranslation.text)
                ? 'mixed'
                : 'audio',
  ...
  const deleteVoiceTranslation = useCallback(
    async (targetUnit: LayerUnitDocType, targetLayer: LayerDocType) => {
      if (!targetUnit || !targetLayer) {
        throw new Error(t(locale, 'transcription.error.validation.voiceTranslationTargetRequired'));
      }
      ...
      const db = await getDb();
      const now = new Date().toISOString();
      const { standaloneSegmentTarget, existingAudioTranslation } =
        await resolveVoiceTranslationTargets(db, targetUnit, targetLayer);
      ...
      if (!existingAudioTranslation || !mediaId) {
        setSaveState({
          kind: 'done',
          message: t(locale, 'transcription.voiceTranslation.done.noAudioToDelete'),
        });
        return;
      }
      ...
      if (normalizedText) {
        ...
        setTranslations((prev) =>
          prev.map((item) => (item.id === existingAudioTranslation.id ? updatedTranslation : item)),
        );
      } else {
        if (standaloneSegmentTarget) {
      ...
      setSaveState({
        kind: 'done',
        message: t(locale, 'transcription.voiceTranslation.done.deleted'),
      });
    },
    [
      locale,
   ...
   */
  const transcribeVoiceTranslation = useCallback(
    async (
      targetUnit: LayerUnitDocType,
      targetLayer: LayerDocType,
      ...
      if (
        ...
        (targetLayer.modality !== 'mixed' && targetLayer.modality !== 'audio')
      ) {
        setSaveState({
          kind: 'error',
      ...
      if (!existingAudioTranslation || !mediaId) {
        setSaveState({
          kind: 'done',
          message: t(locale, 'transcription.voiceTranslation.done.noAudioToTranscribe'),
        });
        return;
      }
      ...
      let blob: Blob | null =
        options?.audioBlob && options.audioBlob.size > 0 ? options.audioBlob : null;
      if (!blob) {
      ...
      try {
        ...
        if (isLikelyAbortError(e, sttSignal)) {
          setSaveState({
            kind: 'done',
            message: t(locale, 'transcription.voiceTranslation.done.transcribeAborted'),
          });
          return;
        }
      ...
      if (!transcript) {
        ...
        try {
          ...
          if (!available) {
            throw new Error(
              t(locale, 'transcription.voiceTranslation.error.commercialUnavailable'),
            );
          }
      ...
      setSaveState({
        kind: 'done',
        message: t(locale, 'transcription.voiceTranslation.done.transcribed'),
      });
    },
    [
      locale,
