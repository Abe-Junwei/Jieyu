...
export function useTranscriptionLayerActions({
  ...
  const syncTranslationHostLinksByOrder = useCallback(
    async (nextLayers: LayerDocType[]): Promise<LayerLinkDocType[] | null> => {
      ...
      const getEffectiveConstraint = (
        layer: LayerDocType,
      ): NonNullable<LayerDocType['constraint']> =>
        layer.constraint ??
        (layer.layerType === 'translation' ? 'symbolic_association' : 'independent_boundary');
      const isIndependentTranscriptionRoot = (layer: LayerDocType): boolean =>
        layer.layerType === 'transcription' &&
        getEffectiveConstraint(layer) === 'independent_boundary';
      ...
      for (const link of nextLayerLinks) {
        const hostIdFromKey = resolveLayerLinkHostTranscriptionLayerId(
          link,
          transcriptionIdByKeyForLinks,
        );
      ...
      const translationsToRewire = [...expectedHostByTranslationId.entries()]
        .filter(
          ([translationLayerId, expectedHost]) =>
            currentHostByTranslationId.get(translationLayerId) !== expectedHost.id,
        )
      ...
      for (const translationLayerId of translationsToRewire) {
        ...
        if (removableLinks.length > 0) {
          await Promise.all(
            removableLinks.map((link) => db.collections.layer_links.remove(link.id)),
          );
        ...
        const alreadyLinked = nextLayerLinks.some(
          (link) =>
            link.layerId === translationLayerId && link.transcriptionLayerKey === nextParentKey,
        );
        if (alreadyLinked) {
          continue;
        }
  ...
  const persistLayerState = useCallback(
    async (previousLayers: LayerDocType[], nextLayers: LayerDocType[]) => {
      ...
      for (const layer of nextLayers) {
        ...
        const parentChanged =
          (layerTranscriptionTreeParentId(previous) ?? '') !==
  ...
  const createLayer = useCallback(
    async (
      ...
      const hasExistingTranscriptionLayer = layers.some(
        (layer) => layer.layerType === 'transcription',
      );
      const resolvedConstraint =
        input.constraint ??
        (layerType === 'transcription' && !hasExistingTranscriptionLayer
      ...
      const requestedHostTranscriptionLayerIds = [
        ...new Set(
      ...
      const requestedPreferredHostTranscriptionLayerId =
        input.preferredHostTranscriptionLayerId?.trim() ?? '';

      const resolvedTranslationHostTranscriptionLayerIds =
        layerType === 'translation'
          ? (() => {
              const explicitHostIds = requestedHostTranscriptionLayerIds.filter((id) =>
                independentParentById.has(id),
              );
          ...
          : [];
      const resolvedTranslationPreferredHostTranscriptionLayerId =
        layerType === 'translation'
          ? requestedPreferredHostTranscriptionLayerId &&
            resolvedTranslationHostTranscriptionLayerIds.includes(
              requestedPreferredHostTranscriptionLayerId,
            )
            ? requestedPreferredHostTranscriptionLayerId
          ...
          : resolvedConstraint !== 'independent_boundary'
            ? requestedParentLayerId
              ...
              : independentParentCandidates.length === 1
                ...
                : undefined
            : undefined;
      const resolvedParentLayerId =
        resolvedParent?.id ?? (layerType === 'translation' ? '' : requestedParentLayerId);
      ...
      const createGuard = getLayerCreateGuard(layers, layerType, {
        ...
        ...(layerType === 'transcription' && resolvedParentLayerId
      ...
      try {
        ...
        const textId =
          input.textId?.trim() || unitsRef.current[0]?.textId || layers[0]?.textId || '';
        if (!textId) {
          setLayerCreateMessage(
            '\u672a\u627e\u5230\u5f53\u524d\u9879\u76ee\u4e0a\u4e0b\u6587\uff0c\u8bf7\u5148\u8fdb\u5165\u76ee\u6807\u9879\u76ee\u540e\u518d\u521b\u5efa\u5c42\u3002',
          );
          return false;
        }
        ...
        if (layerType === 'transcription' && !hasExistingTranscriptionLayer) {
          ...
          for (const u of pendingUnits) {
            try {
              await LinguisticService.units.save(u);
              ...
              log.warn(
                'Adopt pending units / mirror segments after first transcription layer failed',
                {
                  unitId: u.id,
                  textId: u.textId,
        ...
        const linksForOrdering = autoLinks.length > 0 ? [...layerLinks, ...autoLinks] : layerLinks;
        ...
        setLayerCreateMessage(
          `\u5df2\u521b\u5efa${typeLabel}\u5c42\uff1a${autoName}\uff08${languageId}\uff09`,
        );
        return true;
      } catch (error) {
        setLayerCreateMessage(
          error instanceof Error ? error.message : '\u521b\u5efa\u5c42\u5931\u8d25',
        );
        return false;
      }
    },
  ...
  const performLayerDelete = useCallback(
    async (targetLayerId: string, options?: PerformLayerDeleteOptions) => {
      ...
      try {
        if (!options?.skipUndo) {
          pushUndo(
            targetLayer.layerType === 'translation'
        ...
        if (targetLayer.layerType === 'transcription') {
          if (deleteCheck.orphanedTranslationIds && deleteCheck.relinkTargetKey) {
            ...
            const relinkTargetLayer = layers.find(
              (layer) =>
                layer.layerType === 'transcription' && layer.key === deleteCheck.relinkTargetKey,
            );
          ...
          await db.collections.layer_links.removeBySelector({
            transcriptionLayerKey: targetLayer.key,
          });
          nextLayerLinks = nextLayerLinks.filter(
            (link) => link.transcriptionLayerKey !== targetLayer.key,
          );
        } else {
        ...
        if (!keepUnits && affectedUnitIds.length > 0) {
          ...
          if (orphanIds.length > 0) {
            await LinguisticService.cleanup.removeUnitsBatch(orphanIds);
        ...
        if (!options?.silent) {
          setLayerCreateMessage(
            error instanceof Error ? error.message : '\u5220\u9664\u5c42\u5931\u8d25',
          );
        }
      }
    },
  ...
  const deleteLayerWithoutConfirm = useCallback(
  ...
  const deleteLayer = useCallback(
    async (targetLayerId?: string, options?: DeleteLayerOptions) => {
      ...
      if (targetLayer.layerType === 'transcription') {
        ...
        if (transcriptionLayers.length <= 1) {
          try {
            const dependentTranslationIdsFromLinks = [
              ...new Set(
                layerLinks
                  .filter((link) => {
                    const hostId =
    ...
    [layerLinks, layerToDeleteId, layers, performLayerDelete, setLayerCreateMessage],
  );

  const toggleLayerLink = useCallback(
    async (transcriptionLayerKey: string, layerId: string) => {
      const targetParent = listIndependentBoundaryTranscriptionLayers(layers).find(
        (layer) => layer.key === transcriptionLayerKey,
      );
      if (!targetParent) {
        setLayerCreateMessage(
          '\u8bf7\u9009\u62e9\u6709\u6548\u7684\u72ec\u7acb\u8f6c\u5199\u5c42\u4f5c\u4e3a\u4f9d\u8d56\u76ee\u6807\u3002',
      ...
      const alreadyLinkedToTarget = layerLinks.some(
        (link) =>
          link.layerId === layerId &&
          resolveLayerLinkHostTranscriptionLayerId(link, transcriptionIdByKey) === targetParent.id,
      );
      ...
      if (alreadyLinkedToTarget) {
        const preferredOnTarget = existingLinksForTranslation.some(
          (link) =>
            resolveLayerLinkHostTranscriptionLayerId(link, transcriptionIdByKey) ===
              targetParent.id && link.isPreferred,
        );
      ...
      if (existingLinksForTranslation.length > 0) {
        ...
        const newLink: LayerLinkDocType = !hasPreferred
          ? { ...baseLink, isPreferred: true }
          : baseLink;
        await db.collections.layer_links.insert(newLink);
      ...
      const nextLayers = computeCanonicalLayerOrder(
        layers.map((layer) => (layer.id === layerId ? updatedTranslationBase : layer)),
      );
      const updatedTranslation =
        nextLayers.find((layer) => layer.id === layerId) ?? updatedTranslationBase;
      ...
      if (changedSortLayers.length > 0) {
        await Promise.all(
          changedSortLayers.map((layer) =>
            LayerTierUnifiedService.updateLayerSortOrder(layer.id, layer.sortOrder ?? 0, db),
          ),
        );
      }
      ...
      setLayers(nextLayers);
      setLayerLinks((prev) => [...prev.filter((link) => link.layerId !== layerId), newLink]);
    },
    [
      layerLinks,
      layers,
  ...
  const rebindTranslationLayerHost = useCallback(
    async (input: {
      ...
      const translationLayer = layers.find(
        (layer) => layer.id === input.translationLayerId && layer.layerType === 'translation',
      );
      const fallbackParent = layers.find(
        (layer) =>
          layer.key === input.fallbackTranscriptionLayerKey && layer.layerType === 'transcription',
      );
      if (!translationLayer || !fallbackParent) return;

      const transcriptionIdByKey = buildTranscriptionIdByKeyMap(layers);
      const linksForTranslation = layerLinks.filter(
        (link) => link.layerId === input.translationLayerId,
      );
      const removeIds = new Set(
        linksForTranslation
          .filter(
            (link) =>
              resolveLayerLinkHostTranscriptionLayerId(link, transcriptionIdByKey) ===
              input.removeTranscriptionLayerId,
          )
          .map((link) => link.id),
      );
      ...
      const existingFallback = remainingLinks.find(
        (link) =>
  ...
  const reorderLayers = useCallback(
    async (draggedLayerId: string, targetIndex: number) => {
      ...
      try {
        ...
        const message =
          error instanceof Error ? error.message : '\u5c42\u7ea7\u91cd\u6392\u5931\u8d25';
    ...
    [
      ...
      setLayers,
      setSaveState,
      syncTranslationHostLinksByOrder,
    ],
  );
