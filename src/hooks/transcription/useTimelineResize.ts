...
export function useTimelineResize({
  ...
  const startTimelineResizeDrag = useCallback(
    (
      ...
      const effectiveZoom =
        typeof options?.zoomPxPerSec === 'number' &&
        Number.isFinite(options.zoomPxPerSec) &&
        options.zoomPxPerSec > 0
      ...
      const layerSegments =
        segmentMapKey && segmentsByLayer ? segmentsByLayer.get(segmentMapKey) : undefined;
      ...
      const onMove = (ev: PointerEvent) => {
        ...
        const rightBound =
          typeof bounds.right === 'number' ? bounds.right : Number.POSITIVE_INFINITY;
