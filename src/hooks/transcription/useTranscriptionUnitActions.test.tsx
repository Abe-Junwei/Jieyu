...
import {
  ...
  setCommercialSttRuntimeSnapshot,
} from '../../services/stt/voiceCommercialSttRuntime';
...
describe('useTranscriptionUnitActions - batch operations', () => {
  ...
  it('offsetSelectedTimes should block overlap and not push undo', async () => {
    ...
    let unitsState = [makeUnit('u1', 0, 1), makeUnit('u2', 1.1, 2)];

    const setUnits = vi.fn(
      (updater: ((prev: LayerUnitDocType[]) => LayerUnitDocType[]) | LayerUnitDocType[]) => {
  ...
  it('scaleSelectedTimes should push undo once and update selected ranges', async () => {
    const pushUndo = vi.fn();
    const setSaveState = vi.fn();
    const saveBatchSpy = vi.spyOn(LinguisticService.units, 'saveBatch').mockResolvedValue();

    let unitsState = [makeUnit('u1', 1, 2), makeUnit('u2', 2.2, 3.2), makeUnit('u3', 6, 7)];
  ...
  it('saveUnitLayerText should strip legacy speaker linkage fields', async () => {
    ...
    const v2Content = (await db.layer_unit_contents.get('utr-legacy')) as
  ...
  it('deleteVoiceTranslation should remove saved audio translation and media item', async () => {
    ...
    const setTranslations = vi.fn(
      (
        updater:
          | LayerUnitContentDocType[]
          | ((prev: LayerUnitContentDocType[]) => LayerUnitContentDocType[]),
      ) => {
        translationsState = typeof updater === 'function' ? updater(translationsState) : updater;
      },
    );
    const setMediaItems = vi.fn(
      (updater: MediaItemDocType[] | ((prev: MediaItemDocType[]) => MediaItemDocType[])) => {
    ...
    await act(async () => {
      await result.current.saveVoiceTranslation(
    ...
    expect(await db.layer_unit_contents.where('layerId').equals(translationLayer.id).count()).toBe(
      0,
    );
    expect(setSaveState).toHaveBeenCalledWith(
      expect.objectContaining({ kind: 'done', message: '录音翻译已删除' }),
    );
  });
  ...
  it('saveVoiceTranslation should not create projected child segment for standalone segment targets', async () => {
    ...
    const projectedChildren = await LayerSegmentQueryService.listSegmentsByParentUnitIds([
      segment.id,
    ]);
  ...
  it('saveVoiceTranslation upgrades existing standalone-segment text row in place', async () => {
    ...
    expect(translationsState).toHaveLength(1);
    expect(translationsState[0]).toEqual(
      expect.objectContaining({
        id: 'segc_existing_text',
        modality: 'mixed',
        text: '已有文本',
  ...
  it('saveUnitText should write translation text through canonical LayerUnit path', async () => {
    ...
    await act(async () => {
      await result.current.saveUnitText(
        'utt-stop-write',
        'translation via layerunit only',
        'trl-en',
      );
    });
  ...
  it('offsetSelectedTimes should rollback when persistence fails after pushUndo', async () => {
    ...
    const setSaveState = vi.fn();
    vi.spyOn(LinguisticService.units, 'saveBatch').mockRejectedValue(new Error('db write failed'));

    let unitsState = [makeUnit('u1', 0, 1), makeUnit('u2', 1.2, 2)];
  ...
  it('scaleSelectedTimes should log both operation error and rollback warning when rollback also fails', async () => {
    ...
    let unitsState = [makeUnit('u1', 1, 2), makeUnit('u2', 2.2, 3.2)];
  ...
  it('mergeSelectedUnits should log both operation error and rollback warning when rollback also fails', async () => {
    ...
    const setSaveState = vi.fn();
    vi.spyOn(LinguisticService.units, 'save').mockRejectedValue(new Error('db write failed'));
  ...
  it('createUnitFromSelection should select created unit with non-empty layerId', async () => {
    ...
    const setSelectedTimelineUnit = vi.fn(
      (next: { layerId: string; unitId: string; kind: 'unit' | 'segment' } | null) => {
    ...
    const nonNullSelections = selectedTimelineUnits.filter(
      (item): item is { layerId: string; unitId: string; kind: 'unit' | 'segment' } =>
        item !== null,
    );
  ...
  it('createUnitFromSelection keeps current selection when selectionBehavior is keep-current', async () => {
    ...
    expect(setSelectedTimelineUnit).not.toHaveBeenCalledWith(
  ...
  it('projects a new unit to both dependent transcription layer and its parent root', async () => {
    ...
    const dependentSegments = await LayerSegmentQueryService.listSegmentsByLayerId(
      dependentLayer.id,
    );
  ...
  it('transcribeVoiceTranslation merges text into existing audio row as mixed (no second text row)', async () => {
    ...
    const sttBlob = mediaItemsState.find(
      (m) => m.id === translationsState[0]!.translationAudioMediaId,
    );
    const blobFromState =
      sttBlob?.details &&
      typeof sttBlob.details === 'object' &&
      (sttBlob.details as { audioBlob?: unknown }).audioBlob instanceof Blob
        ? (sttBlob.details as { audioBlob: Blob }).audioBlob
        : new Blob(['a'], { type: 'audio/webm' });

    await act(async () => {
      await result.current.transcribeVoiceTranslation(unit, translationLayer, {
        audioBlob: blobFromState,
      });
    });
    ...
    const rows = await db.layer_unit_contents
      .where('layerId')
  ...
  it('transcribeVoiceTranslation uses runtime-injected commercial credentials after whisper fallback', async () => {
    ...
    window.localStorage.setItem(
      'jieyu.voiceAgent.commercialStt',
      JSON.stringify({
        kind: 'groq',
        config: {
    ...
    vi.spyOn(LocalWhisperSttProvider.prototype, 'transcribe').mockRejectedValue(
    ...
    const createCommercialProviderSpy = vi
      .spyOn(sttModule, 'createCommercialProvider')
      .mockReturnValue({
    ...
    expect(createCommercialProviderSpy).toHaveBeenCalledWith(
      'groq',
      expect.objectContaining({
        apiKey: 'sk-runtime-secret',
        baseUrl: 'https://api.example.com',
