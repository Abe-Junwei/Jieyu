...
import {
  mergeUnitSelfCertaintyConservative,
  type UnitSelfCertainty,
} from '../../utils/unitSelfCertainty';
import {
...
export function useTranscriptionUnitActions({
  ...
  const saveUnitText = useCallback(
  ...
  const saveUnitTiming = useCallback(
    async (unitId: string, startTime: number, endTime: number) => {
      const db = await getDb();
      ...
      const normalizedStart = Math.max(
        0,
        Math.min(boundedStart, maxAllowedStart, endTime - minSpan),
      );
  ...
  const saveUnitLayerText = useCallback(
    async (unitId: string, value: string, layerId: string) => {
      ...
      const candidates = allTexts.filter(
        (item) =>
          item.layerId === layerId && (item.modality === 'text' || item.modality === 'mixed'),
      );
  ...
  const createAdjacentUnit = useCallback(
    async (base: LayerUnitDocType, playerDuration: number) => {
      const db = await getDb();
  ...
  const createUnitFromSelection = useCallback(
    async (
      start: number,
      end: number,
      options?: {
      ...
      const lowerBound = allowOverlapInTranscription
        ? 0
        : Math.max(0, prev ? prev.endTime + gap : 0);
  ...
  const reassignTranslations = useCallback(
    async (
      survivorId: string,
      removedId: string,
      db: Awaited<ReturnType<typeof getDb>>,
  ...
  const mergeWithPrevious = useCallback(
    async (unitId: string) => {
      ...
      const { newTranslations, updatedTranslations } = await reassignTranslations(
        prev.id,
        curr.id,
        db,
        now,
      ...
      setUnits((p) =>
        p.filter((u) => u.id !== curr.id).map((u) => (u.id === prev.id ? updated : u)),
      );
      setTranslations((p) => {
  ...
  const mergeWithNext = useCallback(
    async (unitId: string) => {
      ...
      setUnits((p) =>
        p.filter((u) => u.id !== next.id).map((u) => (u.id === curr.id ? updated : u)),
      );
      setTranslations((p) => {
        const updatedIds = new Set(updatedTranslations.map((t) => t.id));
        return [
  ...
  const splitUnit = useCallback(
    async (unitId: string, splitTime: number) => {
      const target = unitsRef.current.find((u) => u.id === unitId);
      if (!target) {
        reportValidationError({
  ...
  const deleteSelectedUnits = useCallback(
