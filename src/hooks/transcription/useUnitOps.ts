...
export function useUnitOps(input: UseUnitOpsInput) {
  ...
  const unitHasText = useCallback(
    (uttId: string): boolean => {
  ...
  const runDeleteSelection = useCallback(
    (primaryId: string, ids: Set<string>) => {
      const plan = resolveDeletePlan(primaryId, ids);
      if (plan.kind === 'none') return;
      if (plan.kind === 'multi') {
        requestDeleteUnits(plan.ids, () => {
          fireAndForget(deleteSelectedUnits(plan.ids), {
            context: 'src/hooks/transcription/useUnitOps.ts:L52',
            policy: 'user-visible',
          });
        });
        return;
      }
      requestDeleteUnits(plan.id, () => {
        fireAndForget(deleteUnit(plan.id), {
          context: 'src/hooks/transcription/useUnitOps.ts:L55',
          policy: 'user-visible',
        });
      });
    },
  ...
  const runMergeSelection = useCallback(
    (ids: Set<string>) => {
      if (ids.size <= 1) return;
      fireAndForget(mergeSelectedUnits(ids), {
        context: 'src/hooks/transcription/useUnitOps.ts:L64',
        policy: 'user-visible',
      });
    },
    [mergeSelectedUnits],
  ...
  const runMergePrev = useCallback(
    (id: string) => {
      if (!id) {
        onMergeTargetMissing?.();
        return;
      ...
      fireAndForget(mergeWithPrevious(id), {
        context: 'src/hooks/transcription/useUnitOps.ts:L72',
        policy: 'user-visible',
      });
    },
  ...
  const runMergeNext = useCallback(
    (id: string) => {
      if (!id) {
        onMergeTargetMissing?.();
        return;
      ...
      fireAndForget(mergeWithNext(id), {
        context: 'src/hooks/transcription/useUnitOps.ts:L80',
        policy: 'user-visible',
      });
    },
  ...
  const runSplitAtTime = useCallback(
    (id: string, splitTime: number) => {
      if (!id) return;
      fireAndForget(splitUnit(id, splitTime), {
        context: 'src/hooks/transcription/useUnitOps.ts:L85',
        policy: 'user-visible',
      });
    },
    [splitUnit],
  ...
  const runSelectBefore = useCallback(
    (id: string) => {
      if (!id) return;
      selectAllBefore(id);
    },
  ...
  const runSelectAfter = useCallback(
    (id: string) => {
      if (!id) return;
      selectAllAfter(id);
    },
