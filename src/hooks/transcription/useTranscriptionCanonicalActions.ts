...
export function useTranscriptionCanonicalActions({ runWithDbMutex, setUnits }: Params) {
  const getCanonicalTokensForUnit = useCallback(async (unitId: string) => {
    try {
      return await LinguisticService.units.listTokensByUnitId(unitId);
    } catch (err) {
  ...
  const getCanonicalMorphemesForToken = useCallback(async (tokenId: string) => {
    try {
      return await LinguisticService.units.listMorphemesByTokenId(tokenId);
    } catch (err) {
  ...
  const { updateTokenPos, batchUpdateTokenPosByForm, updateTokenGloss } =
