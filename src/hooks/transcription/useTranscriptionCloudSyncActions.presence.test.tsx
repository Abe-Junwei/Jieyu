...
const {
  ...
  const mockUpsert = vi
    .fn<(row: unknown, options: unknown) => Promise<{ error: null }>>()
  ...
  const mockPresenceUpdate = vi
    .fn<(patch: { state: 'online' | 'idle' | 'offline' }) => Promise<void>>()
...
vi.mock('../../collaboration/cloud/CollaborationPresenceService', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('../../collaboration/cloud/CollaborationPresenceService')>();
  class PresenceServiceMock {
    private connection: { projectId: string; userId: string; displayName?: string } | null = null;

    async connect(
      input: { projectId: string; userId: string; displayName?: string },
      onMembersChanged?: (members: unknown[]) => void,
    ): Promise<void> {
...
describe('useTranscriptionCloudSyncActions presence visibility sync', () => {
  ...
  it('switches to idle when page becomes hidden and returns online when visible', async () => {
    ...
    await waitFor(() => {
      expect(mockUpsert).toHaveBeenCalledWith(expect.objectContaining({ state: 'idle' }), {
        onConflict: 'project_id,user_id',
      });
    });
    ...
    await waitFor(() => {
      ...
      expect(mockUpsert).toHaveBeenLastCalledWith(expect.objectContaining({ state: 'online' }), {
        onConflict: 'project_id,user_id',
      });
    });
