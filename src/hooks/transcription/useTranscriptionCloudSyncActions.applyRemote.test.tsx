...
function buildParams(
  overrides?: Partial<UseTranscriptionCloudSyncActionsParams>,
...
function createRemoteRelationChange(
...
describe('useTranscriptionCloudSyncActions applyRemote + conflict audit chain', () => {
  ...
  it('records resolutionTraceId on conflict_resolved logs after auto LWW merge', async () => {
    ...
    await waitFor(() => {
      const resolved = result.current.conflictOperationLogs.filter(
        (item) => item.type === 'conflict_resolved',
      );
