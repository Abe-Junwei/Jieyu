...
export function useTranscriptionDataTextTimeMapping(input: {
  ...
  const applyTextTimeMapping = useCallback(
    async (params: {
      textId?: string;
      offsetSec?: number;
      scale?: number;
  ...
  const previewTextTimeMapping = useCallback(
    (previewInput: { startTime: number; endTime: number; offsetSec?: number; scale?: number }) =>
      transcriptionAppService.previewTextTimeMapping(previewInput),
