...
export function useTranscriptionPersistence({ unitsRef, translationsRef, speakersRef }: Params) {
  // Async mutex: serializes syncToDb / undo / redo to prevent interleaving
  const dbMutexRef = useRef(createAsyncMutex());

  /** Sync a snapshot of units + translations + speakers back to IndexedDB (caller must hold db mutex). */
  const syncToDbCore = useCallback(
    async (
      ...
      if (options?.conflictGuard) {
        const assertNoConflict = async <T extends { id: string; updatedAt?: string }>(
          ...
          const expectedById = new Map(
            baseRows.map((row) => [row.id, row.updatedAt ?? ''] as const),
          );
          ...
          for (const [id, expectedUpdatedAt] of expectedById) {
            if (!persistedById.has(id)) {
              throw new TranscriptionPersistenceConflictError(
                `${label} conflict: missing persisted row ${id}`,
              );
            }
        ...
        try {
          ...
          await assertNoConflict('translations', translationsRef.current, async () => {
            ...
            return ids.flatMap((id) => {
              ...
              return content
                ? [{ toJSON: () => content as unknown as LayerUnitContentDocType }]
                : [];
            });
          });
          await assertNoConflict('speakers', speakersRef.current, async () =>
            db.collections.speakers.findByIndexAnyOf(
              'id',
              speakersRef.current.map((row) => row.id),
            ),
          );
        } catch (error) {
      ...
      for (const id of currentUttIds) {
        if (!targetUttIds.has(id)) await LinguisticService.cleanup.removeUnit(id);
      }
      // Upsert target units
      for (const u of targetUnits) await LinguisticService.units.save(u);
  ...
  const syncToDb = useCallback(
    async (
      ...
      await dbMutexRef.current.run(() =>
        syncToDbCore(targetUnits, targetTranslations, targetSpeakers, options),
      );
    },
    [syncToDbCore],
  );
