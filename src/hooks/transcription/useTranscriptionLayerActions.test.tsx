...
vi.mock('../../services/LayerSegmentGraphService', () => ({
  ...
  listUnitUnitPrimaryKeysByTextId: (_db: unknown, _textId: string) =>
    mockUnitsWhereTextIdPrimaryKeys(),
}));
...
describe('useTranscriptionLayerActions v2 cleanup', () => {
  ...
  it('removes segment_links via source/target indexes when deleting a layer', async () => {
    ...
    expect(mockDeleteLayerSegmentGraphByLayerId).toHaveBeenCalledWith(
      expect.anything(),
      'layer_trl_1',
    );
  ...
  it('replaces an existing media item with the same id instead of appending a duplicate', () => {
    ...
    const updater = setMediaItems.mock.calls[0]?.[0] as (
      items: Array<typeof existing | typeof other>,
    ) => Array<typeof existing | typeof other>;
  ...
  it('creates host link when creating a translation layer', async () => {
    ...
    await act(async () => {
      await result.current.createLayer(
        'translation',
        { languageId: 'eng', textId: 'text_1' },
        'text',
      );
    });

    expect(mockCreateLayer).toHaveBeenCalled();
    expect(mockInsertLayerLink).toHaveBeenCalledWith(
  ...
  it('passes through supported translation constraints that remain enabled', async () => {
    ...
    for (const { constraint, languageId } of constraintInputs) {
      await act(async () => {
        await result.current.createLayer(
          'translation',
          { languageId, textId: 'text_1', constraint },
          'text',
        );
      });
    }
    ...
    expect(createdConstraints).toEqual(
      expect.arrayContaining(['symbolic_association', 'time_subdivision']),
    );
  });
  ...
  it('blocks translation independent_boundary creation in the action layer', async () => {
    ...
    expect(setLayerCreateMessage).toHaveBeenCalledWith(
      expect.stringContaining('翻译层不支持独立边界'),
    );
  });
  ...
  it('requires explicit parent selection when multiple independent transcription parents exist', async () => {
    ...
    expect(mockCreateLayer).not.toHaveBeenCalled();
    expect(
      setLayerCreateMessage.mock.calls[setLayerCreateMessage.mock.calls.length - 1]?.[0],
    ).toMatch(/存在多个独立边界转写层|请先为译文选择宿主转写/);
  ...
  it('creates first transcription layer with explicit independent_boundary constraint', async () => {
    ...
    const created = (lastCallIndex >= 0 ? createCalls[lastCallIndex]?.[0] : undefined) as
      | { constraint?: string }
      | undefined;
  ...
  it('still updates React layer list when adopting pending units fails (DB layer already inserted)', async () => {
    ...
    await act(async () => {
      const ok = await result.current.createLayer(
        'transcription',
        { languageId: 'zho', textId: 'text_1' },
        'text',
      );
  ...
  it('moves an independent root together with its bundle when reordered', async () => {
    ...
    const reordered = setLayers.mock.calls[setLayers.mock.calls.length - 1]?.[0] as
      | LayerDocType[]
      | undefined;
  ...
  it('rewires host links when moving a dependent translation layer into another root bundle', async () => {
    ...
    const linkedLayers = setLayerLinks.mock.calls[setLayerLinks.mock.calls.length - 1]?.[0] as
  ...
  it('re-canonicalizes sort order after toggling translation host link', async () => {
    ...
    const nextLayers = setLayers.mock.calls[setLayers.mock.calls.length - 1]?.[0] as
      | LayerDocType[]
      | undefined;
    // 主宿主仍在 trc_a 时 canonical 顺序保持译文紧邻首选根；link-only 下由 computeCanonicalLayerOrder 决定 |
    expect(nextLayers?.map((layer) => layer.id)).toEqual([
      'layer_trc_a',
      'layer_trl_a',
      'layer_trc_b',
      'layer_trl_b',
    ...
    const lastSetLayerLinksArg = setLayerLinks.mock.calls[
      setLayerLinks.mock.calls.length - 1
    ]?.[0] as Array<{ transcriptionLayerKey: string; layerId: string }> | undefined;
  ...
  it('toggleLayerLink appends a second host link without removing existing host links', async () => {
    ...
    const nextLinks = setLayerLinks.mock.calls[setLayerLinks.mock.calls.length - 1]?.[0] as
      | Array<{ id: string; hostTranscriptionLayerId?: string }>
      | undefined;
    expect(nextLinks).toHaveLength(2);
    expect(nextLinks?.some((l) => l.id === 'link_to_a')).toBe(true);
    expect(nextLinks?.map((l) => l.hostTranscriptionLayerId).sort()).toEqual([
      'layer_trc_a',
      'layer_trc_b',
    ]);
  });
  ...
  it('toggleLayerLink switches preferred host when a host link already exists', async () => {
    ...
    const nextLinks = setLayerLinks.mock.calls[setLayerLinks.mock.calls.length - 1]?.[0] as
      | Array<{ id: string; isPreferred?: boolean }>
      | undefined;
  ...
  it('cascade-deletes dependent translation layers when deleting the last transcription layer', async () => {
    ...
    expect(mockDeleteLayer).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ id: 'layer_trl_1' }),
    );
    expect(mockDeleteLayer).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ id: 'layer_trc_1' }),
    );
    expect(setLayerCreateMessage).toHaveBeenCalledWith(
      expect.stringContaining('自动级联删除 1 个依赖翻译层'),
    );
  });
  ...
  it('blocks creating translation when same-language transcription exists', async () => {
    ...
    expect(mockCreateLayer).not.toHaveBeenCalled();
    expect(setLayerCreateMessage).toHaveBeenCalledWith(
      expect.stringContaining('禁止与翻译层同语言'),
    );
  });
  ...
  it('blocks creating transcription when same-language translation exists', async () => {
    ...
    expect(mockCreateLayer).not.toHaveBeenCalled();
    expect(setLayerCreateMessage).toHaveBeenCalledWith(
      expect.stringContaining('禁止与转写层同语言'),
    );
  });
  ...
  it('blocks creating same-language transcription without alias (case-insensitive)', async () => {
    ...
    expect(mockCreateLayer).not.toHaveBeenCalled();
    expect(setLayerCreateMessage).toHaveBeenCalledWith(
      expect.stringContaining('该语言已存在转写层'),
    );
  });
});
