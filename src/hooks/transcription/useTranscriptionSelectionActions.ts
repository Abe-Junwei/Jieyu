...
export function useTranscriptionSelectionActions({
  ...
  const setUnitSelection = useCallback(
    (primaryId: string, ids: Iterable<string>) => {
      applyUnitSelection({
        primaryId,
        ids,
  ...
  const selectTimelineUnit = useCallback(
    (unit: TimelineUnit | null) => {
      if (!unit) {
        clearSelectionState();
        return;
      }
      ...
      if (
        prev &&
        prev.unitId === unit.unitId &&
        prev.layerId === unit.layerId &&
        prev.kind === unit.kind
      ) {
        return;
      }
  ...
  const selectUnitRange = useCallback(
  ...
  const selectAllBefore = useCallback(
    (id: string) => {
      const sorted = unitsOnCurrentMediaRef.current;
  ...
  const toggleSegmentSelection = useCallback(
    (id: string) => {
      ...
      if (next.has(id)) {
        ...
        const primary =
          selectedUnitIdRef.current === id ? (remaining[0] ?? '') : selectedUnitIdRef.current;
        if (!primary) {
          clearSelectionState();
          return;
        }
  ...
  const selectSegmentRange = useCallback(
    (anchorId: string, targetId: string, orderedItems: Array<{ id: string }>) => {
