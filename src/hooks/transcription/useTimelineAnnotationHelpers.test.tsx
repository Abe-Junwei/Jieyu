...
describe('useTimelineAnnotationHelpers', () => {
  it('preserves existing multi-selection when context-clicking a selected segment', () => {
    ...
    render(
      result.current.renderAnnotationItem(
  ...
  it('renders self-certainty badge on segment row from host unit', () => {
    ...
    const { container } = render(
      result.current.renderAnnotationItem(seg, makeLayer('layer-seg'), 'segment text', {
        onChange: vi.fn(),
        onBlur: vi.fn(),
      }),
    );
  ...
  it('keeps certainty isolated to the display lane when a dependent row reuses a source segment', () => {
    ...
    const { result } = renderHook(() =>
      useTimelineAnnotationHelpers({
        ...
        resolveSelfCertaintyForUnit: (_unitId, layerId) =>
          layerId === 'layer-source' ? 'certain' : undefined,
      }),
    );

    const { container } = render(
      result.current.renderAnnotationItem(seg, makeLayer('layer-display'), 'segment text', {
        onChange: vi.fn(),
        onBlur: vi.fn(),
      }),
    );
