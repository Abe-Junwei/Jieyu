...
import type { TimelineUnitView } from '../transcription/timelineUnitView';
import type { TimelineUnit } from '../transcription/transcriptionTypes';
import {
  DEFAULT_KEYBINDINGS,
  getEffectiveKeymap,
  matchKeyEvent,
...
export function useKeybindingActions(input: UseKeybindingActionsInput) {
        ...
        if (segMarkStart === null) {
          ...
          if (end - s >= 0.05) {
            ...
            fireAndForget(
              ...
              { context: 'src/hooks/ui/useKeybindingActions.ts:L142', policy: 'user-visible' },
            );
          }
  ...
  useEffect(() => {
    const globalActions: Record<string, () => void> = {
      undo: () =>
        fireAndForget(undo(), {
          context: 'src/hooks/ui/useKeybindingActions.ts:L267',
          policy: 'user-visible',
        }),
      redo: () =>
        fireAndForget(redo(), {
          context: 'src/hooks/ui/useKeybindingActions.ts:L268',
          policy: 'user-visible',
        }),
  ...
  const executeAction = useCallback(
    (actionId: string, params?: { segmentIndex?: number }) => {
      ...
      switch (actionId) {
        case 'undo':
          fireAndForget(undo(), {
            context: 'src/hooks/ui/useKeybindingActions.ts:L342',
            policy: 'user-visible',
          });
          break;
        case 'redo':
          fireAndForget(redo(), {
            context: 'src/hooks/ui/useKeybindingActions.ts:L343',
            policy: 'user-visible',
          });
          break;
        case 'search':
