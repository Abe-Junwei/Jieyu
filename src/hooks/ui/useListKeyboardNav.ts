    ...
    (event: React.KeyboardEvent) => {
      ...
      switch (event.key) {
        case 'ArrowDown': {
          ...
          const next =
            activeIndexRef.current < currentItems.length - 1 ? activeIndexRef.current + 1 : 0;
          setActiveIndex(next);
          scrollToIndex(next);
          break;
        }
        case 'ArrowUp': {
          event.preventDefault();
          const prevIdx =
            activeIndexRef.current > 0 ? activeIndexRef.current - 1 : currentItems.length - 1;
        ...
        case 'Enter': {
          event.preventDefault();
          const idx =
            activeIndexRef.current >= 0 && activeIndexRef.current < currentItems.length
