...
function resolvePrimaryLanguageId(text: TextDocType): string | null {
  const metadataLang = (text.metadata as { primaryLanguageId?: unknown } | undefined)
    ?.primaryLanguageId;
...
function resolvePrimaryOrthographyId(text: TextDocType): string | null {
  const metadataOrthography = (text.metadata as { primaryOrthographyId?: unknown } | undefined)
    ?.primaryOrthographyId;
...
function parseTextTimeMappingSummary(rawMapping: unknown): TextTimeMappingSummaryItem | null {
  ...
  const offsetSec =
    typeof mapping.offsetSec === 'number' && Number.isFinite(mapping.offsetSec)
      ? mapping.offsetSec
      : undefined;
  const scale =
    typeof mapping.scale === 'number' && Number.isFinite(mapping.scale) ? mapping.scale : undefined;
  if (offsetSec === undefined || scale === undefined) return null;
  return {
    offsetSec,
    scale,
    revision:
      typeof mapping.revision === 'number' && Number.isFinite(mapping.revision)
...
export function useDialogs(units: DialogUnit[]) {
  const unitTextIdSignature = useMemo(() => units.map((unit) => unit.textId).join('|'), [units]);
  ...
  useEffect(() => {
    ...
    if (!targetId) return;
    let cancelled = false;
    void LinguisticService.timeline.getTextById(targetId).then((activeText) => {
