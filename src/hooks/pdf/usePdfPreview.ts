...
export function usePdfPreview() {
  ...
  const [pdfPreviewPos, setPdfPreviewPos] = useState<{ right: number; bottom: number }>({
    right: 16,
    bottom: 16,
  });
  ...
  const buildPdfPreviewUrl = useCallback(
    (sourceUrl: string, page: number | null, hashSuffix = '', searchSnippet?: string): string => {
  ...
  const handlePdfPreviewPageChange = useCallback(
    (delta: number) => {
      setPdfPreview((current) => {
  ...
  const handlePdfPreviewDragStart = useCallback(
