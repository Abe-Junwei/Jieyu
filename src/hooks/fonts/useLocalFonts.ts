...
export function useLocalFonts() {
  ...
  const getCoverage = useCallback(
    (fontFamily: string, renderPolicy: OrthographyRenderPolicy) => {
      const cacheKey = [
        fontFamily,
        renderPolicy.scriptTag,
        renderPolicy.coverageSummary.exemplarSample,
        renderPolicy.coverageSummary.exemplarCharacterCount,
        renderPolicy.ipaMode ? 'ipa' : 'plain',
      ].join('\u0000');
  ...
  const ensureCoverage = useCallback(
    async (fontFamily: string, renderPolicy: OrthographyRenderPolicy) => {
      ...
      const existing =
        coverageByKey[cacheKey] ?? getCachedFontCoverageVerification(fontFamily, renderPolicy);
      if (existing) {
