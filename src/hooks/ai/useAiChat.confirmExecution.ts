 ...
 */
async function finalizeHumanToolCallConfirmOutcome(
  params: FinalizeHumanToolCallConfirmOutcomeParams,
): Promise<void> {
  const {
    assistantMessageId,
    call,
...
function appendProposeRollbackStatus(
  ...
  if (rb.ok) {
    return locale === 'zh-CN'
      ? `${detail}（已回滚已应用的修改）`
      : `${detail} (rolled back applied changes)`;
  }
  const joined = rb.errors.join('; ');
  return locale === 'zh-CN'
    ? `${detail}（回滚失败：${joined}）`
    : `${detail} (rollback failed: ${joined})`;
}
...
export async function executeConfirmedProposedChangeBatch({
  ...
  const blockedByDirective =
    ...
    (sessionMemory.safetyPreferences?.denyDestructive === true &&
      childCalls.some((call) => isDestructiveToolCall(call.name)));
  if (blockedByDirective) {
    const reason =
      sessionMemory.toolPreferences?.autoExecute === 'never'
        ? 'user_directive_never_execute'
