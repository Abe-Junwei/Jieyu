...
} from '../../utils/citationFootnoteUtils';
import {
  isSearchFusionScenario,
  type SearchFusionScenario,
} from '../../ai/embeddings/searchFusionProfiles';
...
export async function enrichContextWithRag({
  ...
  try {
    ...
    const ragResult = await withTimeout(
      embeddingSearchService.searchMultiSourceHybrid(queryText, ['unit', 'note', 'pdf'], {
        topK: searchTopK,
        fusionScenario: scenario,
        ...hybridOptsBase,
      }),
      ragContextTimeoutMs,
    ...
    const settledResults = await Promise.allSettled(
      activeMatches.map(async (match): Promise<RagSourceRow> => {
        ...
        if (match.sourceType === 'note') {
          ...
          if (noteDoc?.content) {
            ...
            snippet = (
              contentByLang['und'] ??
              contentByLang['en'] ??
              Object.values(contentByLang).find((value) => value.trim()) ??
              ''
            ).trim();
          }
        ...
        const label =
          match.sourceType === 'note'
            ? '笔记参考'
            : match.sourceType === 'unit'
              ? '句段参考'
              : '文档参考';
        const contextTag =
          match.sourceType === 'note'
            ...
            : match.sourceType === 'unit'
              ? 'UNIT_CONTEXT'
              : 'PDF_CONTEXT';
        const safeSnippet = normalizedSnippet.replace(/[\[\]]/g, (char) =>
          char === '[' ? '【' : '】',
        );
        const validCitationTypes: Array<'note' | 'unit' | 'pdf' | 'schema'> = [
          'note',
          'unit',
          'pdf',
          'schema',
        ];
        if (!validCitationTypes.includes(match.sourceType as (typeof validCitationTypes)[number]))
          return null;
    ...
    if (rejectedCount > 0) {
      log.warn(
        `RAG enrichment: ${rejectedCount}/${activeMatches.length} lookups failed, continuing with successful results`,
      );
    }

    const rawRagSources = rawRagResults.filter(
