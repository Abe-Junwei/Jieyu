...
import {
  deactivateSessionDirective as deactivateSessionDirectiveFromMemory,
...
export function useAiChatDirectiveSessionControls(options: {
  ...
  const writeDirectiveMutationAuditLog = useCallback(
    (mutationType: 'deactivate' | 'prune_source', payload: Record<string, unknown>) => {
  ...
  const toggleMessagePinned = useCallback(
    (messageId: string) => {
      ...
      const activeConversationId = conversationIdRef.current;
      const currentlyPinned = (sessionMemoryRef.current.pinnedMessageIds ?? []).includes(
        normalizedMessageId,
      );
  ...
  const deactivateSessionDirective = useCallback(
    (directiveId: string) => {
      ...
      const nextMemory = deactivateSessionDirectiveFromMemory(
        previousMemory,
  ...
  const pruneSessionDirectivesBySourceMessage = useCallback(
    (sourceMessageId: string) => {
      const normalizedSourceMessageId = sourceMessageId.trim();
      if (!normalizedSourceMessageId) return;
      const previousMemory = sessionMemoryRef.current;
      const matchedDirectiveCount = (previousMemory.sessionDirectives ?? []).filter(
        (directive) => directive.sourceMessageId === normalizedSourceMessageId,
      ).length;
      const matchedLedgerCount = (previousMemory.directiveLedger ?? []).filter(
        (entry) => entry.sourceMessageId === normalizedSourceMessageId,
      ).length;
      if (matchedDirectiveCount === 0 && matchedLedgerCount === 0) return;
      const nextMemory = pruneDirectiveLedgerBySourceMessage(
        previousMemory,
