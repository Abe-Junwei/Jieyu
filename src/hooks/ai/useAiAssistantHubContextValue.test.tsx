...
import { pickVoiceAgentContextValue } from '../voice/useVoiceAgentContextValue';
import { DEFAULT_VOICE_MODE } from '../../services/voiceMode';

function makeAiChatSource(
  overrides: Partial<AiChatContextValue> = {},
): Partial<AiChatContextValue> {
  return {
    selectedUnit: null,
...
function makeVoiceSource(
  overrides: Partial<VoiceAgentContextValue> = {},
): Partial<VoiceAgentContextValue> {
  return {
    voiceEnabled: false,
...
describe('useAiAssistantHubContextValue', () => {
  it('maps ai panel context values for assistant hub consumption', () => {
    ...
    const { result } = renderHook(() =>
      useAiAssistantHubContextValue(
        pickAiChatContextValue(chatSource),
        pickVoiceAgentContextValue(voiceSource),
      ),
    );
