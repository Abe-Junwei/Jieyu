...
function makeDeps(overrides?: {
  ...
  const onToolCall =
    ...
    vi.fn(async (): Promise<AiChatToolResult> => ({ ok: true, message: 'ok' }));
...
describe('executeConfirmedProposedChangeBatch', () => {
  ...
  it('marks parent request executed only after all children succeed and does not invoke rollbacks', async () => {
    ...
    const confirmedLog = d.writeToolDecisionAuditLog.mock.calls.find((c) =>
      auditLogNewValue(c)?.startsWith('confirmed:propose_changes'),
    );
