...
import { useVadCacheEntry } from '../voice/useVadCachedSegments';
import { useVadCacheWarmupStatus } from '../voice/useVadCacheWarmupStatus';
...
export function useAiPanelLogic({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void LinguisticService.lexemes
        .search(token)
        .then((items) => {
