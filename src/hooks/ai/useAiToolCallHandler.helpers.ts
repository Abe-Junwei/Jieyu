...
function buildAiToolLanguageTokens(query: string): string[] {
  ...
  if (code) {
    tokens.add(code);
    const entry = SUPPORTED_VOICE_LANGS.flatMap((group) => group.langs).find(
      (lang) => lang.code === code,
    );
    if (entry) {
...
export function parseLayerHintFromOpaqueId(
  value: string,
): { layerType: 'translation' | 'transcription'; languageQuery: string } | null {
  const normalized = normalizeAiToolCallText(value);
  if (!normalized) return null;
  const layerType = /translation|译|翻译/.test(normalized)
    ? 'translation'
    : /transcription|转写|转录|听写/.test(normalized)
      ? 'transcription'
      : null;
...
export function layerMatchesLanguage(layer: LayerDocType, languageQuery: string): boolean {
  ...
  const tokens = buildAiToolLanguageTokens(languageQuery).map((token) =>
    normalizeAiToolCallText(token),
  );
...
export function formatVoiceLayerKinds(
...
export function formatLayerTypeLabel(
