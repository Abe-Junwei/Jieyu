...
import type {
  ExecutionContext,
  UseAiToolCallHandlerParams as Params,
  CompensationEntry,
...
export function useAiToolCallHandler({
  ...
  return useCallback(
    async (call: AiChatToolCall): Promise<AiChatToolResult> => {
      ...
      const hasRequestedUnitTarget = (): boolean => {
        ...
        if (typeof segmentIndex === 'number' && Number.isInteger(segmentIndex) && segmentIndex >= 1)
          return true;
        return (
          typeof call.arguments.segmentPosition === 'string' &&
          call.arguments.segmentPosition.length > 0
        );
      };
      ...
      const resolveRequestedUnit = (): LayerUnitDocType | null => {
        ...
        if (
          call.arguments.segmentPosition === 'previous' ||
          call.arguments.segmentPosition === 'next'
        ) {
      ...
      const resolveTranscriptionLayerForLink = (): LayerDocType | null => {
        ...
        if (requestedLayerKey) {
          return (
            currentTranscriptionLayers.find((layer) => layer.key === requestedLayerKey) ?? null
          );
        }
        return null;
      };
      ...
      if (!adapter) {
        return {
          ok: false,
          message: tf(locale, 'transcription.aiTool.unsupportedTool', { toolName: call.name }),
        };
      }
