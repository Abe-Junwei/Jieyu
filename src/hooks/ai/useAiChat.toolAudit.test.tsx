...
describe('useAiChatToolAudit', () => {
  ...
  it('returns false for propose_changes partial-failure audit when metadata.executed is false (retry same parent requestId)', async () => {
    ...
    await expect(
      result.current.hasPersistedExecutionForRequest('req-parent-propose'),
    ).resolves.toBe(false);
  });
  ...
  it('compact newValue without metadata treats propose_changes child_failed as not persisted (retry same parent requestId)', async () => {
    ...
    await expect(
      result.current.hasPersistedExecutionForRequest('req-legacy-compact'),
    ).resolves.toBe(false);
  });
  ...
  it('compact newValue without metadata treats invalid_child_args as not persisted', async () => {
    ...
    await expect(
      result.current.hasPersistedExecutionForRequest('req-compact-invalid-child'),
    ).resolves.toBe(false);
  });
  ...
  it('compact auto_failed with gate reason is not persisted', async () => {
    ...
    await expect(result.current.hasPersistedExecutionForRequest('req-auto-gate')).resolves.toBe(
      false,
    );
  });
});
