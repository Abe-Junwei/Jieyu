  ...
  async execute(ctx) {
    ...
    const executeMappedAction = (
      ...
      if (!ctx.executeAction) {
        return {
          ok: false as const,
          message: t(locale, 'transcription.aiTool.voice.actionUnsupported'),
        };
      }
    ...
    const executeTargetedMerge = async (
      ...
      if (requestedSegmentId.length > 0) {
        ...
        if (!targetSegment) {
          return {
            ok: false as const,
            message: tf(locale, 'transcription.aiTool.voice.segmentNotFound', {
    ...
    if (call.name === 'play_pause') {
      return executeMappedAction(
        'playPause',
        t(locale, 'transcription.aiTool.voice.playPauseDone'),
      );
    }
    if (call.name === 'undo') {
    ...
    if (call.name === 'search_segments') {
      ...
      const rawLayers = Array.isArray(call.arguments.layers)
        ? call.arguments.layers.filter(
            (item): item is 'transcription' | 'translation' | 'gloss' =>
      ...
      if (ctx.openSearch) {
        ctx.openSearch({
          query,
          scope: 'global',
          ...(rawLayers.length > 0 ? { layerKinds: rawLayers } : {}),
        });
        return {
          ok: true,
          message:
      ...
      if (!ctx.executeAction) {
        return {
          ok: false,
          message: tf(locale, 'transcription.aiTool.voice.searchUnsupportedManual', { query }),
        };
      }
      ctx.executeAction('search');
      return {
        ok: true,
        message: tf(locale, 'transcription.aiTool.voice.searchManual', { query }),
      };
    }
    if (call.name === 'toggle_notes') {
      return executeMappedAction(
        'toggleNotes',
        t(locale, 'transcription.aiTool.voice.toggleNotesDone'),
      );
    }
    if (call.name === 'mark_segment') {
      return executeMappedAction(
        'markSegment',
        t(locale, 'transcription.aiTool.voice.markSegmentDone'),
      );
    }
    if (call.name === 'delete_segment') {
      const hasExplicitTarget =
        call.arguments.allSegments === true ||
      ...
      return executeMappedAction(
        'deleteSegment',
        t(locale, 'transcription.aiTool.voice.deleteSegmentDone'),
      );
    }
    ...
    if (call.name === 'nav_to_segment') {
      ...
      if (!Number.isFinite(idx) || idx < 1)
        return {
          ok: false,
          message: t(locale, 'transcription.aiTool.voice.navSegmentInvalidIndex'),
        };
      ...
      if (!target)
        return {
          ok: false,
          message: tf(locale, 'transcription.aiTool.voice.navSegmentOutOfRange', {
            index: idx,
            total: segments.length,
          }),
        };
      if (!ctx.navigateTo)
        return { ok: false, message: t(locale, 'transcription.aiTool.voice.navigateUnsupported') };
      ...
      return {
        ok: true,
        message: tf(locale, 'transcription.aiTool.voice.navSegmentDone', {
          index: idx,
          total: segments.length,
        }),
    ...
    if (call.name === 'nav_to_time') {
      ...
      if (!Number.isFinite(timeSeconds) || timeSeconds < 0)
        return { ok: false, message: t(locale, 'transcription.aiTool.voice.navTimeInvalid') };
      if (!ctx.seekToTime)
        return {
          ok: false,
          message: tf(locale, 'transcription.aiTool.voice.navTimeUnsupported', { timeSeconds }),
        };
      ctx.seekToTime(timeSeconds);
      return {
        ok: true,
        message: tf(locale, 'transcription.aiTool.voice.navTimeDone', {
    ...
    if (call.name === 'split_at_time') {
      const timeSeconds = Number(call.arguments.timeSeconds);
      if (!Number.isFinite(timeSeconds) || timeSeconds < 0)
        return { ok: false, message: t(locale, 'transcription.aiTool.voice.splitAtTimeInvalid') };
      const targetSegment = ctx.units.find(
        (unit) => unit.startTime <= timeSeconds && unit.endTime >= timeSeconds,
      );
      if (targetSegment && ctx.splitTranscriptionSegment) {
        ...
        if (!Number.isFinite(start) || !Number.isFinite(end) || end <= start) {
          return {
            ok: false,
            message: t(locale, 'transcription.aiTool.segment.splitInvalidRange'),
          };
        }
        const minSpan = 0.05;
        ...
        return {
          ok: true,
          message: tf(locale, 'transcription.aiTool.voice.splitAtTimeDone', {
            timeSeconds: timeSeconds.toFixed(2),
          }),
          ...(rollback ? { rollback } : {}),
        };
      ...
      if (!ctx.splitAtTime)
        return {
          ok: false,
          message: tf(locale, 'transcription.aiTool.voice.splitAtTimeUnsupported', { timeSeconds }),
        };
      const ok = ctx.splitAtTime(timeSeconds);
      if (!ok)
        return {
          ok: false,
          message: tf(locale, 'transcription.aiTool.voice.splitAtTimeNoSegment', {
            timeSeconds: timeSeconds.toFixed(2),
          }),
        };
      return {
    ...
    if (call.name === 'merge_prev') {
      return executeTargetedMerge(
        ctx.mergeWithPrevious,
        t(locale, 'transcription.aiTool.voice.mergePrevDone'),
      );
    }
    if (call.name === 'merge_next') {
      return executeTargetedMerge(
        ctx.mergeWithNext,
        t(locale, 'transcription.aiTool.voice.mergeNextDone'),
      );
    }
    if (call.name === 'focus_segment') {
      ...
      if (!segId)
        return {
          ok: false,
          message: t(locale, 'transcription.aiTool.voice.focusSegmentMissingId'),
        };
      const found = ctx.units.find((u) => u.id === segId);
      if (!found)
        return {
          ok: false,
          message: tf(locale, 'transcription.aiTool.voice.segmentNotFound', { segmentId: segId }),
        };
      if (!ctx.navigateTo)
        return {
          ok: false,
          message: tf(locale, 'transcription.aiTool.voice.focusSegmentUnsupported', {
            segmentId: segId,
          }),
        };
      const previousFocusId = ctx.selectedUnit?.id;
    ...
    if (call.name === 'zoom_to_segment') {
      const segId = String(call.arguments.segmentId ?? '').trim();
      const zoomLevel =
        typeof call.arguments.zoomLevel === 'number' ? call.arguments.zoomLevel : undefined;
      if (!segId)
        return { ok: false, message: t(locale, 'transcription.aiTool.voice.zoomSegmentMissingId') };
      const found = ctx.units.find((u) => u.id === segId);
      if (!found)
        return {
          ok: false,
      ...
      if (ctx.zoomToSegment) {
        ...
        if (!ok)
          return {
            ok: false,
            message: tf(locale, 'transcription.aiTool.voice.zoomSegmentFailed', {
              segmentId: segId,
            }),
          };
        return {
          ...
          message:
            typeof zoomLevel === 'number'
              ? tf(locale, 'transcription.aiTool.voice.zoomSegmentDoneWithLevel', {
                  segmentId: segId,
                  zoomLevel,
                })
      ...
      if (!ctx.navigateTo)
        return {
          ok: false,
          message: tf(locale, 'transcription.aiTool.voice.zoomSegmentNavigateFallbackUnsupported', {
            segmentId: segId,
          }),
        };
      ...
      return {
        ok: true,
        message: tf(locale, 'transcription.aiTool.voice.zoomSegmentNavigateFallbackDone', {
          segmentId: segId,
        }),
      };
    }
    if (call.name === 'get_current_segment') {
      ...
      if (!utt)
        return { ok: false, message: t(locale, 'transcription.aiTool.voice.currentSegmentNone') };
      const dur = (utt.endTime - utt.startTime).toFixed(2);
      const status = utt.annotationStatus ?? 'raw';
      const speaker = utt.speaker
        ? tf(locale, 'transcription.aiTool.voice.currentSegmentSpeakerSuffix', {
            speaker: utt.speaker,
          })
        : '';
      return {
    ...
    if (call.name === 'get_recent_history') {
      try {
        ...
        if (sessions.length === 0)
          return { ok: true, message: t(locale, 'transcription.aiTool.voice.historyNone') };
        const lines = sessions.flatMap((s) => s.entries.slice(-2)).slice(-8);
        if (lines.length === 0)
          return { ok: true, message: t(locale, 'transcription.aiTool.voice.historyEmpty') };
        const entries = lines
          .map((e, i) => {
        ...
        return {
          ok: true,
          message: tf(locale, 'transcription.aiTool.voice.historyDone', { entries }),
        };
      } catch (err) {
    ...
    if (call.name === 'get_project_summary') {
      ...
      const done = ctx.units.filter(
        (u) => u.annotationStatus && u.annotationStatus !== 'raw',
      ).length;
    ...
    return {
      ok: false,
      message: tf(locale, 'transcription.aiTool.voice.unknownTool', { toolName: call.name }),
    };
  },
};
