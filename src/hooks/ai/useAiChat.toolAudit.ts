...
export function useAiChatToolAudit() {
   ...
   */
  const writeToolDecisionAuditLog = useCallback(
    async (
  ...
  const writeToolIntentAuditLog = useCallback(
    async (
      assistantMessageId: string,
  ...
  const hasPersistedExecutionForRequest = useCallback(
      ...
      const hasExecuted = rows.some((row) => {
        if (typeof row.metadataJson === 'string' && row.metadataJson.trim().length > 0) {
          try {
            ...
            log.error('failed to parse tool decision metadata, falling back to compact parsing', {
              err,
            });
          }
        }
        ...
        if (decision === 'confirm_failed' || decision === 'auto_failed') {
          return !NON_PERSISTED_TOOL_DECISION_REASONS.has(reason);
        }
        return false;
      });
