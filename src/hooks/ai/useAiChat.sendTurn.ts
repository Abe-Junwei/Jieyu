...
export async function runAiChatSendTurn(args: RunAiChatSendTurnArgs): Promise<void> {
  ...
  logSendTurnPhase(correlationId, 'preflight_ok', {
    assistantId,
    ...(verticalWorkflowSelection
      ? { verticalWorkflowId: verticalWorkflowSelection.workflowId }
      : {}),
  });
  ...
  try {
    ...
    const { opening, sendTurnConversationId, stream, generationSource } =
    ...
    await runAiChatSendTurnStreamPhase({
      ...
      verticalOutputEnvelopeSeed:
        opening.verticalOutputEnvelopeSeed ?? preflight.verticalOutputEnvelopeSeed,
      userMsg,
      assistantId,
      shouldTrackRemoteStatus,
      timeoutHandle,
    ...
    if (
      ...
      setLastError(
        tf(toolFeedbackLocaleRef.current, 'ai.chat.persistLayerRecoveryHint', {
