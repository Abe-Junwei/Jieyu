...
export async function runAiChatSendTurnPreflight(
  ...
  const currentSessionTokens =
    metricsRef.current.totalInputTokens + metricsRef.current.totalOutputTokens;
  ...
  const effectiveTimeoutMs =
    provider.id === 'deepseek' || provider.id === 'minimax'
      ...
      : provider.id === 'ollama'
        ...
        : firstChunkTimeoutMs;
  const timeoutHandle =
    typeof window !== 'undefined' && effectiveTimeoutMs > 0
      ? window.setTimeout(() => {
      ...
      : null;
  const { queueFlushAssistantDraft, awaitQueuedPersistence, finalizeAssistantMessage } =
  ...
  if (composedTemplate) {
    ...
    sessionMemoryRef.current = {
      ...sessionMemoryRef.current,
      composedWorkflowState: createInitialComposedWorkflowState(
        composedTemplate,
        finalEffectiveUserText,
      ),
    };
    persistSessionMemory(sessionMemoryRef.current);
    const stepWorkflow = resolveComposedStepWorkflowSelection(
      sessionMemoryRef.current.composedWorkflowState!,
    );
    if (stepWorkflow) {
  ...
  const verticalOutputEnvelopeSeed = verticalWorkflowSelection
    ...
    : null;
  const immediateDirectives = extractUserDirectives({
  ...
  if (immediateDirectives.length > 0) {
    ...
    if (decision.action === 'allow') {
      sessionMemoryRef.current = applyUserDirectivesToSessionMemory(
        sessionMemoryRef.current,
        immediateDirectives,
      ).nextMemory;
      persistSessionMemory(sessionMemoryRef.current);
