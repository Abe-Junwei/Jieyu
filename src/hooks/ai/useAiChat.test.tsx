    ...
    sendMessage(input: {
      ...
      async function* stream() {
        if (userText.includes('请解释删除层命令是什么意思')) {
          yield {
            delta:
              '{"tool_call":{"name":"delete_layer","arguments":{"layerType":"transcription","languageQuery":"中文"}}}',
          };
        ...
        if (userText.includes('set_translation_text和set_transcription_text有什么区别')) {
          yield {
            delta:
              '{"tool_call":{"name":"set_translation_text","arguments":{"unitId":"u1","text":"x"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        ...
        if (userText.includes('__TOOL_PROPOSE_CHANGES__')) {
          yield {
            delta:
              '{"tool_call":{"name":"propose_changes","arguments":{"description":"demo","changes":[{"tool":"set_transcription_text","arguments":{"segmentId":"u1","text":"hi"}}]}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        if (userText.includes('__TOOL_PROPOSE_CHANGES_MULTI__')) {
          yield {
            delta:
              '{"tool_call":{"name":"propose_changes","arguments":{"description":"demo","changes":[{"tool":"set_transcription_text","arguments":{"segmentId":"u1","text":"first"}},{"tool":"set_transcription_text","arguments":{"segmentId":"u2","text":"second"}}]}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        ...
        if (userText.includes('__TOOL_DELETE_SEGMENT__')) {
          yield {
            delta:
              '{"tool_call":{"name":"delete_transcription_segment","arguments":{"unitId":"u1"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        ...
        if (userText.includes('把第五个句段转写改为你好')) {
          yield {
            delta: '{"tool_call":{"name":"set_transcription_text","arguments":{"text":"你好"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        if (userText.includes('set the fifth segment transcription to hello')) {
          yield {
            delta: '{"tool_call":{"name":"set_transcription_text","arguments":{"text":"hello"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        ...
        if (userText.includes('__LEGACY_TOOL_MERGE_SELECTED_SEGMENTS__')) {
          yield {
            delta:
              '{"tool_call":{"name":"merge_transcription_segments","arguments":{"unitIds":["utt-legacy-1","utt-legacy-2"]}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        ...
        if (userText.includes('__LEGACY_PENDING_SEGMENT_TEXT__')) {
          yield {
            delta:
              '我识别到你想执行“delete_transcription_segment”。这个操作风险较高，我先暂停，等你确认后再继续。',
          };
          yield { delta: '', done: true };
          return;
        }
        ...
        if (userText.includes('__TOOL_RENAME_LAYER__')) {
          yield {
            delta:
              '{"tool_call":{"name":"set_transcription_text","arguments":{"unitId":"u1","text":"hello"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        if (userText.includes('__TOOL_INVALID_ARGS__')) {
          yield {
            delta: '{"tool_call":{"name":"set_transcription_text","arguments":{"unitId":"u1"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        if (userText.includes('__TOOL_SET_TRANSLATION_MISSING_LAYER__')) {
          yield {
            delta:
              '{"tool_call":{"name":"set_translation_text","arguments":{"unitId":"u1","text":"译文"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        if (userText.includes('__TOOL_CLEAR_TRANSLATION_MISSING_LAYER__')) {
          yield {
            delta: '{"tool_call":{"name":"clear_translation_segment","arguments":{"unitId":"u1"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        ...
        if (userText.includes('__TOOL_CREATE_TRANSCRIPTION_LAYER_UND__')) {
          yield {
            delta:
              '{"tool_call":{"name":"create_transcription_layer","arguments":{"languageId":"und"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        if (userText.includes('__TOOL_CREATE_TRANSCRIPTION_LAYER__')) {
          yield {
            delta:
              '{"tool_call":{"name":"create_transcription_layer","arguments":{"languageId":"eng"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        ...
        if (userText.includes('合并句段')) {
          yield {
            delta:
              '{"tool_call":{"name":"merge_transcription_segments","arguments":{"segmentId":"seg_1775124896485_puuzw8"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        ...
        if (
          userText.includes('__LOCAL_CONTEXT_TOOL_FENCED__') &&
          !userText.includes('__LOCAL_TOOL_RESULT__')
        ) {
          yield {
            delta: [
              '```json',
        ...
        if (
          userText.includes('__LOCAL_CONTEXT_TOOL_CALLS_SINGLE__') &&
          !userText.includes('__LOCAL_TOOL_RESULT__')
        ) {
          yield {
        ...
        if (userText.includes('可以把当前句段转写改为你好么')) {
          yield {
            delta:
              '{"tool_call":{"name":"set_transcription_text","arguments":{"unitId":"u1","text":"你好"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        if (userText.includes('这个句段呢？')) {
          yield {
            delta:
              '{"tool_call":{"name":"create_transcription_segment","arguments":{"unitId":"u1"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        ...
        if (userText.includes('__TOOL_DELETE_LAYER_HALLUCINATED_ID__')) {
          yield {
            delta:
              '{"tool_call":{"name":"delete_layer","arguments":{"layerId":"transcription_layer_1","layerType":"transcription"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        if (userText.includes('__TOOL_SET_TEXT_HALLUCINATED_UNIT__')) {
          yield {
            delta:
              '{"tool_call":{"name":"set_transcription_text","arguments":{"unitId":"utt_fake_999","text":"hello"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
        if (userText.includes('__TOOL_SET_TRANSLATION_HALLUCINATED_LAYER__')) {
          yield {
            delta:
              '{"tool_call":{"name":"set_translation_text","arguments":{"layerId":"translation_layer_fake","unitId":"u1","text":"hi"}}}',
          };
          yield { delta: '', done: true };
          return;
        }
...
describe('useAiChat abort and recovery', () => {
  ...
  it('should queue propose_changes for confirmation with child tool calls', async () => {
    ...
    expect(result.current.pendingToolCall?.proposedChildCalls?.[0]?.name).toBe(
  ...
  it('should surface partial execution details when propose_changes fails mid-batch', async () => {
    ...
    const decisionLog = auditRows.find(
      (row) =>
        row.field === 'ai_tool_call_decision' &&
        row.newValue === 'confirm_failed:propose_changes:child_failed',
    );
  ...
  it('should resolve deleting the first segment without requiring manual selection', async () => {
    const onToolCall = vi.fn();
    const onToolRiskCheck = vi
      .fn()
      .mockReturnValue({ requiresConfirmation: true, riskSummary: '将删除第 1 个句段' });
    ...
    const { result } = renderHook(() =>
      useAiChat({ onToolCall, onToolRiskCheck, preparePendingToolCall }),
    );
  ...
  it('should resolve selected-segment merge command to merge_transcription_segments', async () => {
    ...
    await waitFor(() => {
      expect(Boolean(result.current.pendingToolCall || onToolCall.mock.calls.length > 0)).toBe(
        true,
      );
    });
  ...
  it('should persist structured replay metadata for auto tool decisions', async () => {
    ...
    const metadata = JSON.parse(decisionLog?.metadataJson ?? '{}') as {
      ...
      toolCall?: {
        name?: string;
        arguments?: { segmentId?: string; text?: string };
        requestId?: string;
      };
  ...
  it('should persist assistant generation metadata across remounts', async () => {
    ...
    const persistedAssistant = (await db.ai_messages.toArray()).find(
    ...
    const restoredAssistant = second.result.current.messages.find(
  ...
  it('should block send when ADR-0028 voice disambiguation dialogue is primary (hook path)', async () => {
    ...
    publishAssistantDialogueVoiceLayer({
      pendingConfirm: null,
      disambiguationOptions: [{ type: 'action', actionId: 'undo', raw: 'u', confidence: 0.9 }],
    });
  ...
  it('should retry once with upgraded output cap when first attempt hits cap', async () => {
    const { result } = renderHook(() =>
      useAiChat({ outputTokenCap: 120, outputTokenRetryCap: 240 }),
    );
  ...
  it('should treat split-segment command as actionable intent', async () => {
    const onToolCall = vi
      .fn()
      .mockResolvedValue({ ok: true, message: '已在 12.50s 处切分当前句段。' });
    const { result } = renderHook(() =>
      useAiChat({
        onToolCall,
        getContext: () => ({
  ...
  it('should clarify split position when cursor location is unavailable, then split after replying 这里', async () => {
    const onToolCall = vi
      .fn()
      .mockResolvedValue({ ok: true, message: '已在 12.00s 处切分当前句段。' });
  ...
  it('should use concise natural-language prompt when create_transcription_layer fails by conflict', async () => {
    const onToolCall = vi
      .fn()
      .mockResolvedValue({ ok: false, message: '创建转写层失败，请检查语言或别名是否冲突。' });
    const { result } = renderHook(() => useAiChat({ onToolCall }));
  ...
  it('should count output tokens using actual model output text during local-tool agent loop', async () => {
    ...
    expect(result.current.metrics.currentTurnTokens).toBeGreaterThanOrEqual(
      result.current.metrics.totalOutputTokens,
    );
  ...
  it('should reset task session to idle after loop budget warning', async () => {
    ...
    expect(assistant?.content).toMatch(
      /如需我继续完成这项查询|If you want me to continue this lookup/,
    );
    expect(result.current.taskSession.status).toBe('idle');
  });
  ...
  it('should resume from the saved loop checkpoint when user replies continue', async () => {
    ...
    expect(warnedAssistant?.content).toMatch(
      /如需我继续完成这项查询|If you want me to continue this lookup/,
    );
  ...
  it('should resume from latest durable loop checkpoint after full restart without session memory', async () => {
    ...
    const resumedAssistant = restartedRuntime.result.current.messages.find(
      (item) => item.role === 'assistant',
    );
    expect(resumedAssistant?.content).toContain('已从持久任务检查点继续。');
  ...
  it('hydrates sessionMemory.pendingAgentLoopCheckpoint from ai_tasks on mount when local session is empty (T1-c)', async () => {
    ...
    await waitFor(() => {
      ...
      expect(result.current.sessionMemory?.pendingAgentLoopCheckpoint?.originalUserText).toBe(
        'hydrate-me',
      );
    });
  ...
  it('should bump cancelCount when user cancels a pending tool call', async () => {
    ...
    const onToolRiskCheck = vi
      .fn()
      .mockReturnValue({ requiresConfirmation: true, riskSummary: '将删除' });
    const { result } = renderHook(() =>
      useAiChat({
        onToolCall,
        onToolRiskCheck,
  ...
  it('should resolve delete-all request to allSegments without any selection', async () => {
    ...
    const onToolRiskCheck = vi
      .fn()
      .mockReturnValue({ requiresConfirmation: true, riskSummary: '将删除全部句段' });
    const preparePendingToolCall = vi.fn((call: AiChatToolCall) => ({
      ...call,
      arguments: {
    ...
    expect(result.current.pendingToolCall?.executionCall?.arguments.segmentIds).toEqual([
      'seg1',
      'seg2',
      'seg3',
    ]);
  ...
  it('should resolve ordinal delete request to segmentIndex without any selection', async () => {
    ...
    const onToolRiskCheck = vi
      .fn()
      .mockReturnValue({ requiresConfirmation: true, riskSummary: '将删除第 5 个句段' });
    const preparePendingToolCall = vi.fn((call: AiChatToolCall) => ({
      ...call,
      arguments: {
        segmentId: 'seg5',
  ...
  it('should resolve ordinal delete request to last segment selector without any selection', async () => {
    ...
    const onToolRiskCheck = vi
      .fn()
      .mockReturnValue({ requiresConfirmation: true, riskSummary: '将删除最后一个句段' });
    const preparePendingToolCall = vi.fn((call: AiChatToolCall) => ({
      ...call,
      arguments: {
        segmentId: 'seg8',
  ...
  it('should resolve English delete request to last segment selector without any selection', async () => {
    ...
    const onToolRiskCheck = vi
      .fn()
      .mockReturnValue({ requiresConfirmation: true, riskSummary: 'Will delete the last segment' });
    const preparePendingToolCall = vi.fn((call: AiChatToolCall) => ({
      ...call,
      arguments: {
        segmentId: 'seg8',
  ...
  it('should resolve previous-segment delete when there is a current anchor', async () => {
    ...
    const onToolRiskCheck = vi
      .fn()
      .mockReturnValue({ requiresConfirmation: true, riskSummary: '将删除前一个句段' });
    const preparePendingToolCall = vi.fn((call: AiChatToolCall) => ({
      ...call,
      arguments: {
        segmentId: 'seg1',
...
describe('useAiChat — deactivateSessionDirective integration', () => {
  ...
  it('clears ledger-applied response preference and persists when only directiveLedger matches (no sessionDirectives)', async () => {
    ...
    const ledgerEntry = result.current.sessionMemory.directiveLedger?.find(
      (e) => e.id === 'dir-ledg-hook-1',
    );
