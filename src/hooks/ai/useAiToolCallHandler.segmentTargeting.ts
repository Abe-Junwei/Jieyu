...
function materializeSegmentTargetCall(
  ...
  const hasSelectorTarget =
    typeof call.arguments.segmentIndex === 'number' ||
  ...
  const target =
    resolveRequestedTarget(call, segmentTargets, selectedTargetId) ??
...
function materializeDeleteLayerCall(
  ...
  if (requestedLayerId) {
    ...
    if (!hint) return call;
    const pool =
      hint.layerType === 'translation' ? context.translationLayers : context.transcriptionLayers;
