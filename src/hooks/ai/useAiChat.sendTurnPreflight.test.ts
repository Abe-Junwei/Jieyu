...
function makeArgs(over: Partial<RunAiChatSendTurnArgs> = {}): RunAiChatSendTurnArgs {
  ...
  return {
    ...
    taskSessionRef: {
      current: { id: 't1', status: 'idle', updatedAt: '2026-01-01T00:00:00.000Z' } as AiTaskSession,
    },
...
describe('runAiChatSendTurnPreflight', () => {
  ...
  it('selects vertical workflow and builds envelope seed when keywords match', async () => {
    ...
    expect(result?.verticalOutputEnvelopeSeed).toEqual(
  ...
  it('skips send-preflight user directives when session sidecar sandbox denies writes', async () => {
    ...
    const args = makeArgs({
      ...
      } as unknown as typeof featureFlags,
  ...
  it('writes session sidecar sandbox audit when send-preflight directives are blocked', async () => {
    ...
    const row = auditInsert.mock.calls.find(
      (call) => (call[0] as { field?: string }).field === 'ai_session_sidecar_sandbox',
    )?.[0] as { field: string; documentId: string; metadataJson: string } | undefined;
