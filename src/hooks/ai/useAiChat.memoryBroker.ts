...
export function buildSessionMemoryDigestSuppressionRefs(
...
export async function maybeAppendMemoryBrokerContext(input: {
  ...
  if (input.maxContextChars !== undefined && input.contextBlock.length >= input.maxContextChars)
  ...
  if (input.maxContextChars === undefined || nextBlock.length <= input.maxContextChars)
    return nextBlock;
