...
function isResponseFormat(value: unknown): value is ResponseFormat {
  return (
    value === 'bullets' || value === 'prose' || value === 'steps' || value === 'evidence_first'
  );
}
...
function rebuildDirectivePreferencesFromLedger(memory: AiSessionMemory): AiSessionMemory {
  ...
  for (const entry of entries) {
    ...
    if (entry.targetPath === 'responsePreferences.language' && isResponseLanguage(entry.value))
      response.language = entry.value;
    if (entry.targetPath === 'responsePreferences.style' && isResponseStyle(entry.value))
      response.style = entry.value;
    if (entry.targetPath === 'responsePreferences.format' && isResponseFormat(entry.value))
      response.format = entry.value;
    if (
      entry.targetPath === 'responsePreferences.evidenceRequired' &&
      typeof entry.value === 'boolean'
    )
      response.evidenceRequired = entry.value;
    if (entry.targetPath === 'toolPreferences.defaultScope' && isToolScope(entry.value))
      tool.defaultScope = entry.value;
    if (entry.targetPath === 'toolPreferences.autoExecute' && isToolAutoExecute(entry.value))
      tool.autoExecute = entry.value;
    if (entry.targetPath === 'toolPreferences.preferLocalReads' && typeof entry.value === 'boolean')
      tool.preferLocalReads = entry.value;
    if (
      entry.targetPath === 'safetyPreferences.denyDestructive' &&
      typeof entry.value === 'boolean'
    )
      safety.denyDestructive = entry.value;
    if (entry.targetPath === 'safetyPreferences.denyBatch' && typeof entry.value === 'boolean')
      safety.denyBatch = entry.value;
    if (
      entry.targetPath === 'safetyPreferences.requireImpactPreview' &&
      typeof entry.value === 'boolean'
    )
      safety.requireImpactPreview = entry.value;
    if (entry.targetPath === 'terminologyPreferences' && typeof entry.value === 'string') {
      ...
      if (source && target) {
        ...
        if (sourceTrimmed && targetTrimmed) {
          const existingIdx = terminology.findIndex(
            (item) => item.source.toLocaleLowerCase() === sourceTrimmed.toLocaleLowerCase(),
          );
          const nextItem = {
...
function stripPinnedMessageDirectiveEffects(memory: AiSessionMemory): AiSessionMemory {
  const directiveLedger = (memory.directiveLedger ?? []).filter(
    (entry) => entry.source !== 'pinned_message',
  );
  const sessionDirectives = (memory.sessionDirectives ?? []).filter(
    (entry) => entry.source !== 'pinned_message',
  );
  const {
...
export function resolvePinnedMessageSessionMemory(
  ...
  if (!currentlyPinned) {
    ...
    nextMemory = {
      ...
      pinnedMessageDigests: [
        ...(nextMemory.pinnedMessageDigests ?? []).filter(
    ...
    if (
      !allowSessionSidecarDirectives(
        sessionSidecarSandbox,
        AI_CHAT_SESSION_SIDECAR_WRITE_PATH.pinnedMessageDirective,
      )
    ) {
      return nextMemory;
    }
    const application = applyUserDirectivesToSessionMemory(
      nextMemory,
  ...
  const memoryWithoutPinned = stripPinnedMessageDirectiveEffects({
    ...nextMemory,
    pinnedMessageDigests: (nextMemory.pinnedMessageDigests ?? []).filter(
      (item) => item.messageId !== normalizedMessageId,
    ),
  });
