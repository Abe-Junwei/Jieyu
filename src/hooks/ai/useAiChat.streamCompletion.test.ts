...
> {
  ...
  const { assistantId: _a, assistantContent: _c, userText: _u, aiContext: _ctx, ...env } = full;
  return env;
}
...
describe('resolveAiChatStreamCompletion local tools JIT context', () => {
  it('uses resolveFreshAiContext for a single local tool when provided', async () => {
    ...
    const fresh: AiPromptContext = {
      longTerm: {
        projectStats: { unitCount: 99, translationLayerCount: 1, aiConfidenceAvg: null },
      },
    };
    ...
    const payload = result.localToolResults![0]!.result as {
      unitCount: number;
  ...
  it('asks for clarification instead of executing ambiguous bare metric query', async () => {
    ...
    try {
      ...
      expect(
        metricEvents.some(
          (event) =>
            event.id === 'ai.local_tool_clarification_needed' &&
            event.reason === 'metric_ambiguous',
        ),
      ).toBe(true);
    } finally {
      dispose();
  ...
  it('formats successful local stats results as a five-part evidence answer', async () => {
    const result = await resolveAiChatStreamCompletion(
      baseParams({
        assistantContent:
          '{"tool_call":{"name":"get_project_stats","arguments":{"metric":"speaker_count","scope":"current_scope"}}}',
        userText: '当前范围有多少说话人？',
        aiContext: {
          shortTerm: {
  ...
  it('records a unified local-tool trace on task session updates', async () => {
    ...
    await resolveAiChatStreamCompletion(
      baseParams({
        ...
        assistantContent:
          '{"tool_call":{"name":"get_project_stats","arguments":{"metric":"unit_count","scope":"project"}}}',
        userText: '整个项目有多少语段？',
        aiContext: {
          shortTerm: {
            projectUnitCount: 12,
    ...
    expect(
      taskUpdates.some(
        (session) =>
          ...
          session.trace.some(
            (item: { phase?: string; toolName?: string; requestId?: string }) =>
              item.phase === 'local_tool' &&
              item.toolName === 'get_project_stats' &&
              item.requestId === 'ast-trace_local_1',
          ),
      ),
    ).toBe(true);
  });
