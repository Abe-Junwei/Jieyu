...
function getMaterializedDeleteBatchIds(call?: AiChatToolCall | null): string[] {
  ...
  const collect = (value: unknown): string[] =>
    Array.isArray(value)
      ...
      : [];
  return Array.from(
    new Set([...collect(call.arguments.segmentIds), ...collect(call.arguments.unitIds)]),
  );
}
...
function getMaterializedDeleteSingleId(call?: AiChatToolCall | null): string {
  if (!call) return '';
  const segmentId =
    typeof call.arguments.segmentId === 'string' ? call.arguments.segmentId.trim() : '';
...
function hasConcreteDeleteSegmentTarget(call?: AiChatToolCall | null): boolean {
  return (
    getMaterializedDeleteBatchIds(call).length > 0 || getMaterializedDeleteSingleId(call).length > 0
  );
}
...
function hasConcreteWriteTarget(call?: AiChatToolCall | null): boolean {
  ...
  return (
    getMaterializedDeleteSingleId(call).length > 0 || getMaterializedDeleteBatchIds(call).length > 0
  );
}
...
export async function resolveDestructiveGate({
  ...
  const writeTargetSensitive =
    isAiToolSegmentExecutionWithExplicitTarget(toolCall.name) &&
  ...
  if (
    (destructiveBlocked || writeTargetSensitive) &&
    riskCheck?.riskSummary &&
    isAmbiguousTargetRiskSummary(riskCheck.riskSummary)
  ) {
  ...
  const executionCall =
    (destructiveBlocked || writeTargetSensitive) && preparePendingToolCall
  ...
  if (
    destructiveBlocked &&
    requiresDeleteSegmentMaterialization(toolCall) &&
    !hasConcreteDeleteSegmentTarget(materializedCall)
  ) {
    const fallbackMessage =
      toolCall.arguments.allSegments === true
  ...
  if (
    writeTargetSensitive &&
    requiresWriteTargetMaterialization(toolCall) &&
    !hasConcreteWriteTarget(materializedCall)
  ) {
    const finalErrorMessage =
      riskCheck?.riskSummary ?? getWriteTargetFallbackMessage(locale, toolCall);
    const finalContent = toNaturalToolFailure(
      locale,
      toolCall.name,
      finalErrorMessage,
