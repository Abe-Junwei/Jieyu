...
export function normalizeOutputTokenRetryCap(
  ...
  return Math.min(
    24_000,
    Math.max(outputTokenCap + 1, Math.floor(input ?? DEFAULT_OUTPUT_TOKEN_RETRY_CAP)),
  );
}
...
export function readDevStreamPersistIntervalMs(): number | undefined {
  ...
  const globalValue = (window as unknown as { __JIEYU_AI_STREAM_PERSIST_MS__?: unknown })
    .__JIEYU_AI_STREAM_PERSIST_MS__;
...
export function readDevAutoProbeIntervalMs(): number | undefined {
  ...
  const globalValue = (window as unknown as { __JIEYU_AI_AUTO_PROBE_MS__?: unknown })
    .__JIEYU_AI_AUTO_PROBE_MS__;
  if (typeof globalValue === 'number' && Number.isFinite(globalValue)) {
    return globalValue;
  }
...
export function readDevRagContextTimeoutMs(): number | undefined {
  ...
  const globalValue = (window as unknown as { __JIEYU_AI_RAG_TIMEOUT_MS__?: unknown })
    .__JIEYU_AI_RAG_TIMEOUT_MS__;
  if (typeof globalValue === 'number' && Number.isFinite(globalValue)) {
    return globalValue;
  }
...
export function readDevSessionTokenBudget(): number | undefined {
  ...
  const globalValue = (window as unknown as { __JIEYU_AI_SESSION_TOKEN_BUDGET__?: unknown })
    .__JIEYU_AI_SESSION_TOKEN_BUDGET__;
  if (typeof globalValue === 'number' && Number.isFinite(globalValue)) {
    return globalValue;
  }
...
export function readDevOutputTokenCap(): number | undefined {
  ...
  const globalValue = (window as unknown as { __JIEYU_AI_OUTPUT_TOKEN_CAP__?: unknown })
    .__JIEYU_AI_OUTPUT_TOKEN_CAP__;
  if (typeof globalValue === 'number' && Number.isFinite(globalValue)) {
    return globalValue;
  }
...
export function readDevOutputTokenRetryCap(): number | undefined {
  ...
  const globalValue = (window as unknown as { __JIEYU_AI_OUTPUT_TOKEN_RETRY_CAP__?: unknown })
    .__JIEYU_AI_OUTPUT_TOKEN_RETRY_CAP__;
  if (typeof globalValue === 'number' && Number.isFinite(globalValue)) {
    return globalValue;
  }
