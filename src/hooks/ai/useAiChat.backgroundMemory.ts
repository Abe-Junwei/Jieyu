import type {
  ...
  BackgroundMemoryExtractionInput,
  ExtractedMemoryFact,
} from '../../ai/memory/backgroundMemoryExtractor';
...
import {
  applyUserDirectivesToSessionMemory,
  summarizeDirectiveApplication,
...
export const AI_CHAT_BACKGROUND_MEMORY_SANDBOX_PROFILE: BackgroundToolSandboxProfile =
  resolveBackgroundMemorySandboxProfileFromEnv();
export const AI_CHAT_BACKGROUND_MEMORY_SANDBOX_AUTHORIZED_DIRS: readonly string[] = [
  'session-memory',
];
...
export function extractBackgroundMemoryFacts(
  input: BackgroundMemoryExtractionInput,
): readonly ExtractedMemoryFact[] {
  const directiveLines = new Set(
    extractUserDirectives({ userText: input.userText, source: 'background_extracted' }).map(
      (directive) => directive.text.toLocaleLowerCase(),
    ),
  );
...
export function appendBackgroundFactsToSessionMemory(
  ...
  const seen = new Set(
    existingFacts.map((item) => normalizeFactText(item.fact).toLocaleLowerCase()),
  );
...
export function createAiChatBackgroundMemoryRuntime(
  params: CreateAiChatBackgroundMemoryRuntimeParams,
  ...
  const sandboxDecision = resolveAiChatBackgroundMemorySandboxPolicy({
    ...
    authorizedWriteDirs:
      params.sandboxAuthorizedWriteDirs ?? AI_CHAT_BACKGROUND_MEMORY_SANDBOX_AUTHORIZED_DIRS,
  });
  const quotaMax = params.flushQuotaMaxCompletedWriteFlushesPerConversation ?? 0;
  const quotaState =
    params.flushQuotaEnabled === true && quotaMax > 0
      ? createBackgroundMemoryFlushQuotaState()
      : null;
...
export function buildBackgroundMemoryAuditLog(
  ...
  const directiveSummary = directiveApplication
    ? summarizeDirectiveApplication(directiveApplication)
    : null;
  return {
    id: newAuditLogId(),
