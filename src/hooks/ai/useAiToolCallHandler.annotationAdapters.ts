...
function collectTokenPosSnapshotsByFormFromUnits(
  ...
  for (const w of words) {
    ...
    if (
      direct !== normalizedForm &&
      !Object.values(w.form ?? {}).some((v) => v === normalizedForm)
    ) {
      continue;
    }
    const raw = w.pos;
      ...
      ...(canRollbackAutoGloss && updateGloss
        ? {
            rollback: async () => {
              ...
              await LinguisticService.units.removeTokenLexemeLinksByIds(linkIds);
...
export const tokenAdapter: ToolObjectAdapter = {
  ...
  async execute(ctx) {
    ...
    if (call.name === 'set_token_pos') {
      ...
      if (tokenId.length > 0) {
        if (!ctx.updateTokenPos) {
          return {
            ok: false,
            message: t(locale, 'transcription.aiTool.token.setPosCallbackMissing'),
          };
        }
      ...
      if (!ctx.batchUpdateTokenPosByForm) {
        return {
          ok: false,
          message: t(locale, 'transcription.aiTool.token.batchSetPosCallbackMissing'),
        };
      }
    ...
    if (call.name === 'set_token_gloss') {
      ...
      const gloss =
        glossRaw === null || glossRaw === '' ? null : String(glossRaw ?? '').trim() || null;
      const lang = String(call.arguments.lang ?? 'eng').trim() || 'eng';

      if (!tokenId) {
        return {
          ok: false,
          message: t(locale, 'transcription.aiTool.token.setGlossMissingTokenId'),
        };
      }
      if (!ctx.updateTokenGloss) {
        return {
          ok: false,
          message: t(locale, 'transcription.aiTool.token.setGlossCallbackMissing'),
        };
      }
    ...
    return {
      ok: false,
      message: tf(locale, 'transcription.aiTool.token.unsupportedTool', { toolName: call.name }),
    };
  },
};
