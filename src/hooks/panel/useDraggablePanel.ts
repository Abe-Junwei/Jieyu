...
export function useDraggablePanel({
  ...
  const clampSizeToViewport = useCallback(
    (candidate: PanelSize): PanelSize => {
      ...
      return {
        width: Math.min(
          Math.max(Math.round(candidate.width), effectiveMinWidth),
          effectiveMaxWidth,
        ),
        height: Math.min(
          Math.max(Math.round(candidate.height), effectiveMinHeight),
          effectiveMaxHeight,
        ),
      };
    },
  ...
  const clampPositionToViewport = useCallback(
    (candidate: PanelPosition, withSize: PanelSize): PanelPosition => {
      if (typeof window === 'undefined') return candidate;
    ...
    const onPointerMove = (event: PointerEvent): void => {
      ...
      if (resizeRef.current) {
        ...
        const maxWidthByViewport = Math.max(
          1,
          window.innerWidth - currentPositionRef.current.x - margin,
        );
        const maxHeightByViewport = Math.max(
          1,
          window.innerHeight - currentPositionRef.current.y - margin,
        );
        setSize(
          clampSizeToViewport({
