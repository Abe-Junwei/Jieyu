...
function catalogQueryKey(locale: Locale) {
  return [LANGUAGE_CATALOG_QUERY_KEY, locale] as const;
}
...
export function useLanguageCatalogLabelMap(
  locale: Locale,
  options?: UseLanguageCatalogLabelMapOptions,
): {
  ...
  const entries = useMemo(() => {
    ...
    return allEntries.filter(
      (entry) =>
        idSet.has(entry.id.trim().toLowerCase()) ||
        idSet.has(entry.languageCode.trim().toLowerCase()),
    );
  ...
  const resolveLabel = useCallback(
    (languageId: string | undefined): string => {
      const normalizedLanguageId = languageId?.trim().toLowerCase() ?? '';
      if (!normalizedLanguageId) {
        return '';
      }
      return (
        labelById.get(normalizedLanguageId) ?? getLanguageDisplayName(normalizedLanguageId, locale)
      );
    },
    [labelById, locale],
  );

  const resolveLanguageCode = useCallback(
    (languageId: string | undefined): string => {
      const normalizedLanguageId = languageId?.trim().toLowerCase() ?? '';
      if (!normalizedLanguageId) {
        return '';
  ...
  const resolveLanguageDisplayName = useCallback<ResolveLanguageDisplayName>(
    (languageId, targetLocale) => {
      const normalizedLanguageId = languageId?.trim().toLowerCase() ?? '';
      if (!normalizedLanguageId) {
        return '';
      }
      if (targetLocale === locale) {
        return (
          labelById.get(normalizedLanguageId) ??
          getLanguageDisplayName(normalizedLanguageId, targetLocale)
        );
      }
      if (targetLocale === 'en-US') {
        return (
          englishById.get(normalizedLanguageId) ??
          getLanguageDisplayName(normalizedLanguageId, targetLocale)
        );
      }
