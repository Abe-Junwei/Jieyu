...
import {
  resolveVerticalReorderTargetIndex,
  type VerticalDragDirection,
...
export function useSidePaneSidebarDrag({
  ...
  const updateRailShiftVisual = useCallback(
    (sourceIndex: number, sourceSpan: number, dropIndex: number) => {
      ...
      if (
        ...
        (insertionIndex > sourceIndex && insertionIndex <= sourceIndex + sourceSpan)
      )
        return;

      const dragBlockHeight = getDraggedRailBlockHeight(
        rows.slice(sourceIndex, sourceIndex + sourceSpan),
      );
  ...
  const updateRailBoundaryHighlightVisual = useCallback(
    (draggedId: string, sourceIndex: number, dropIndex: number) => {
  ...
  const resolveRailDropTargetIndex = useCallback(
    (clientY: number): number | null => {
      ...
      const items = Array.from(
        overview.querySelectorAll<HTMLElement>('.transcription-side-pane-item-row'),
      );
      ...
      if (
        ...
        targetIndex < activeDrag.sourceIndex + activeDrag.sourceSpan
      ) {
        targetIndex = activeDrag.sourceIndex;
      }
  ...
  const resolveTargetBundleRange = useCallback(
    (draggedId: string, dropIndex: number) => {
      ...
      const targetRange = bundleRanges.find(
        (range) => clampedProbeIndex >= range.start && clampedProbeIndex < range.end,
      );
  ...
  const commitRailDragReorder = useCallback(
    (clientY?: number) => {
      const activeDrag = dragStateRef.current;
      ...
      const resolvedIntent =
        typeof clientY === 'number' && activeDrag
      ...
      if (activeDrag && finalTarget !== null && finalTarget !== activeDrag.sourceIndex) {
        fireAndForget(onReorderLayers(activeDrag.draggedId, finalTarget), {
          context: 'src/hooks/sidePane/useSidePaneSidebarDrag.ts:L268',
          policy: 'user-visible',
        });
      }
    ...
    [
      ...
      clearRailShiftVisual,
      onReorderLayers,
      resolveRailDropTargetIndexForActiveDrag,
      updateRailDragDirection,
      updateRailDragVisual,
    ],
  );

  const handleDragStart = useCallback(
    (event: React.MouseEvent, layer: LayerDocType) => {
      ...
      timer = setTimeout(() => {
        timer = null;
        const currentIndex = sidePaneRows.findIndex(
          (candidateLayer) => candidateLayer.id === layer.id,
        );
  ...
  useEffect(() => {
    ...
    clearRailDragVisual,
    clearRailShiftVisual,
    commitRailDragReorder,
    dragState,
    resolveRailDropTargetIndexForActiveDrag,
    updateRailBoundaryHighlightVisual,
    updateRailDragDirection,
    updateRailDragVisual,
    updateRailShiftVisual,
  ]);
