...
describe('mcp_tool_call_audits Dexie table (v48)', () => {
  ...
  it('can insert and retrieve an MCP tool call audit doc', async () => {
    ...
    const retrieved = await jieyuDb.collections.mcp_tool_call_audits
      .findOne({ selector: { id: 'mcp_aud_test_001' } })
      .exec();
  ...
  it('persists optional agentRunId and readbacks after reload', async () => {
    const doc = {
      id: 'mcp_aud_test_run',
      schemaVersion: 1 as const,
      timestamp: new Date().toISOString(),
      ...
      toolName: 'tools/list',
      argumentsJson: '{}',
      runtimeContextJson: '{"agentRunId":"run_a8"}',
      outcome: 'success' as const,
      durationMs: 4,
      agentRunId: 'run_a8',
    };
    ...
    await jieyuDb.collections.mcp_tool_call_audits.insert(doc);
    const retrieved = await jieyuDb.collections.mcp_tool_call_audits
      .findOne({ selector: { id: 'mcp_aud_test_run' } })
      .exec();

    expect(retrieved!.toJSON().agentRunId).toBe('run_a8');
  });
});
