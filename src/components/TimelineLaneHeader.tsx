...
function renderLaneLinkConnectorSvg(segment: {
  column: number;
  colorIndex: number;
    ...
    >
      {segment.role === 'bundle-root' ? (
        <>
          ...
          />
          <line
            className="lane-link-connector-svg-elbow"
            x1={LANE_LINK_MARKER_X + 1}
            x2={LANE_LINK_TRUNK_X}
            y1={50}
            y2={50}
          />
        ...
        <>
          ...
          <line
            className="lane-link-connector-svg-elbow"
            x1={LANE_LINK_ELBOW_START_X}
            x2={LANE_LINK_TRUNK_X}
            y1={50}
            y2={50}
          />
          <polygon
            className="lane-link-connector-svg-arrow"
            points={`${LANE_LINK_MARKER_X},50 ${LANE_LINK_ELBOW_START_X},46 ${LANE_LINK_ELBOW_START_X},54`}
          />
        </>
      )}
    </g>
...
export function TimelineLaneHeader({
  ...
  const resolvedSpeakerQuickActions = allowGlobalTimelineHeaderChrome
    ? speakerQuickActions
    : undefined;
  ...
  const { bundleBoundaryIndexes, bundleRootIds, bundleRanges, bundleRoleByLayerId } =
    useMemo(() => {
      const boundaries = new Set<number>([0, allLayers.length]);
      const rootIds = new Set<string>();
      const ranges: Array<{ rootId: string; start: number; end: number }> = [];
      const roleByLayerId = new Map<
        string,
        'bundle-root' | 'bundle-child-middle' | 'bundle-child-end' | 'bundle-detached-root'
      >();
      let cursor = 0;
  ...
  const openLaneLockDialog = useCallback(
    (selectedSpeakerHint: string, initialLaneIndex: number) => {
  ...
  if (resolvedSpeakerQuickActions) {
    ...
    } = resolvedSpeakerQuickActions;
    const topSpeakers = speakerOptions.slice(0, 3);
    const speakerMenuItems: ContextMenuItem[] = [
      {
        label:
          selectedCount > 0
            ? tf(locale, 'transcription.laneHeader.speaker.clearWithCount', {
      ...
      speakerMenuItems.push({
        label:
          selectedCount > 0
            ...
            : tf(locale, 'transcription.laneHeader.speaker.assignToSpeaker', {
                name: speaker.name,
              }),
        disabled: selectedCount === 0,
        onClick: () => {
    ...
    contextMenuItems.push({
      ...
      meta:
        selectedCount > 0
          ? tf(locale, 'transcription.laneHeader.speaker.metaSelected', {
              count: String(selectedCount),
            })
  ...
  if (resolvedTrackModeControl) {
    ...
    if (!resolvedTrackModeControl.onSetMode) {
      trackMenuItems.push({
        label:
          resolvedTrackModeControl.mode === 'single'
    ...
    if (
      resolvedTrackModeControl.mode !== 'multi-speaker-fixed' &&
      resolvedTrackModeControl.onLockSelectedToLane
    ) {
      trackMenuItems.push({
        label: tf(locale, 'transcription.laneHeader.track.lockSpeakersToLane', {
          hint: selectedSpeakerHint,
        }),
    ...
    if (lockConflictCount > 0) {
      trackMenuItems.push({
        label:
          resolvedTrackModeControl.mode === 'multi-speaker-fixed'
            ? tf(locale, 'transcription.laneHeader.track.conflictOnePerSpeaker', {
                count: String(lockConflictCount),
              })
            : tf(locale, 'transcription.laneHeader.track.conflictLockedReverted', {
                count: String(lockConflictCount),
              }),
        disabled: true,
      });
  ...
  return (
    <div className="timeline-lane-header-wrapper">
      ...
      {isDropAbove && (
        <div className="timeline-lane-drop-indicator timeline-lane-drop-indicator-top" />
      )}
      {isDropBelow && (
        <div className="timeline-lane-drop-indicator timeline-lane-drop-indicator-bottom" />
      )}
      ...
      {laneLockDialog && (
        <ModalPanel
          ...
          layoutStyle={{
            width: laneLockDialogWidth,
            maxWidth: 'calc(100vw - 32px)',
            height: 'auto',
          }}
          footer={
            <>
              <PanelButton variant="ghost" onClick={closeLaneLockDialog}>
                {t(locale, 'transcription.dialog.cancel')}
              </PanelButton>
              <PanelButton variant="primary" onClick={confirmLaneLockDialog}>
                {t(locale, 'transcription.laneHeader.laneLock.confirm')}
              </PanelButton>
            </>
          }
        >
          <div className="speaker-rail-batch-panel">
            ...
            />
            <PanelSection
              className="speaker-rail-form-section"
              title={t(locale, 'transcription.laneHeader.laneLock.targetLaneOrdinalTitle')}
            >
