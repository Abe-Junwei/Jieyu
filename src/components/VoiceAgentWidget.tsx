...
export interface VoiceAgentWidgetProps {
  ...
  onCommercialConfigChange: (config: {
    apiKey?: string;
    baseUrl?: string;
    model?: string;
    appId?: string;
...
function getEnhancementFailureKindLabel(
  locale: ReturnType<typeof useLocale>,
  errorKind: SttEnhancementFailureKind | undefined,
): string | null {
  switch (errorKind) {
    case 'missing-config':
...
export const VoiceAgentWidget = memo(function VoiceAgentWidget(props: VoiceAgentWidgetProps) {
  ...
  const confidenceStyle: CSSProperties | undefined = confidenceColor
    ? ({
        ...
        '--voice-agent-confidence-color': confidenceColor,
      } as CSSProperties)
    : undefined;
  ...
  const [providerAvailability, setProviderAvailability] = useState<
    'idle' | 'testing' | 'available' | 'unavailable'
  >('idle');
  ...
  useEffect(() => {
    ...
    if (lastIntent?.type === 'action') {
      const label = getActionLabel(
        lastIntent.actionId as Parameters<typeof getActionLabel>[0],
        locale,
      );
  ...
  const lastTestRef = useRef<{ result: { available: boolean; error?: string }; ts: number } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    setLocalEnhancementEndpointUrl(
      sttEnhancementConfig?.endpointUrl ? String(sttEnhancementConfig.endpointUrl) : '',
    );
    setLocalEnhancementModel(sttEnhancementConfig?.model ? String(sttEnhancementConfig.model) : '');
    setLocalEnhancementLanguage(
      sttEnhancementConfig?.language ? String(sttEnhancementConfig.language) : '',
    );
  ...
  const nonDictationMode: Exclude<VoiceAgentMode, 'dictation'> =
    mode === 'analysis' ? 'analysis' : 'command';
  ...
  const sessionHint = listening
    ? t(
        locale,
        isNonDictationMode ? MODE_HINT_KEYS.analysis.active : MODE_HINT_KEYS.dictation.active,
      )
  ...
  const sessionTextPreviewProps =
    mode === 'dictation' && !isSessionEmpty ? dictationPreviewTextProps : undefined;
  ...
  const workspaceLabel = t(
    locale,
    isNonDictationMode ? MODE_WORKSPACE_LABEL_KEYS.analysis : MODE_WORKSPACE_LABEL_KEYS.dictation,
  );
  const processSummary = t(
    locale,
    isNonDictationMode ? MODE_PROCESS_LABEL_KEYS.analysis : MODE_PROCESS_LABEL_KEYS.dictation,
  );
  ...
  const showProviderConfig =
    showSettings && (engine === 'commercial' || engine === 'whisper-local');
  ...
  const selectedEnhancementDefinition =
    sttEnhancementKind === 'none'
      ? null
      : (compatibleEnhancements.find((definition) => definition.kind === sttEnhancementKind) ??
        null);
  ...
  return (
    <div className="voice-agent-widget">
      <div aria-live="polite" aria-atomic="true" className="voice-agent-aria-live" role="status">
        {ariaAnnouncement}
      </div>
      <div className="voice-agent-shell">
        <div className="voice-agent-header">
          <div className="voice-agent-header-main">
            <button
              ...
              onPointerLeave={
                engine === 'whisper-local' && isRecording ? onMicPointerUp : undefined
              }
              onClick={engine !== 'whisper-local' ? () => onToggle() : undefined}
              title={
                engine === 'whisper-local'
                  ? isRecording
                    ? t(locale, 'transcription.voiceWidget.mic.releaseToStop')
                    : t(locale, 'transcription.voiceWidget.mic.holdToTalk')
                  : listening
                    ? t(locale, 'transcription.voiceWidget.mic.stopHotkey')
                    : t(locale, 'transcription.voiceWidget.mic.startHotkey')
              }
              aria-label={
                listening
                  ? t(locale, 'transcription.voiceWidget.mic.stopAria')
                  : t(locale, 'transcription.voiceWidget.mic.startAria')
              }
              aria-pressed={listening}
            >
              {isRecording ? (
                <MaterialSymbol name="mic" className={JIEYU_MATERIAL_VOICE_MIC} />
              ) : listening ? (
                <MaterialSymbol name="mic" className={JIEYU_MATERIAL_VOICE_MIC} />
              ) : (
                <MaterialSymbol name="mic_off" className={JIEYU_MATERIAL_VOICE_MIC} />
              )}
            </button>
            <div className="voice-agent-header-copy">
              <div
                className="voice-agent-status-line"
                role="status"
                aria-label={t(locale, 'transcription.voiceWidget.status.current')}
              >
                <span className={`voice-agent-state-badge voice-agent-state-badge-${agentState}`}>
                  {agentStateLabel}
                </span>
                {statusSummary}
              </div>
          ...
          <div
            className="voice-agent-mode-bar"
            role="radiogroup"
        ...
        <div
          className={`voice-agent-session-card voice-agent-session-card-${mode} ${listening ? 'is-live' : 'is-idle'} ${isSessionEmpty ? 'is-empty' : 'has-content'}`}
          {...confidenceStyleProps}
        >
          <div className="voice-agent-session-heading">
            <span className="voice-agent-session-kicker">{workspaceLabel}</span>
            <span className="voice-agent-session-note">
              {listening ? sessionHint : processSummary}
            </span>
          </div>
          ...
          <div className="voice-agent-session-meta">
            ...
            {listening && engine === 'web-speech' && (
              <div className="voice-agent-session-signal-group">
                <span
                  className={`voice-agent-vad-dot ${speechActive ? 'voice-agent-vad-dot-active' : ''}`}
                  title={
                    speechActive
                      ? t(locale, 'transcription.voiceWidget.signal.speaking')
                      : t(locale, 'transcription.voiceWidget.signal.silent')
                  }
                />
                <div
                  className="voice-agent-energy-bar"
                  title={tf(locale, 'transcription.voiceWidget.signal.volume', {
                    percent: Math.round(energyLevel * 500),
                  })}
                  aria-label={tf(locale, 'transcription.voiceWidget.signal.volume', {
                    percent: Math.round(energyLevel * 500),
                  })}
                >
                  <svg
                    className="voice-agent-energy-meter"
                    viewBox="0 0 100 6"
            ...
            {langOverride === '__auto__' && detectedLang && (
              <span
                className="voice-agent-session-chip"
                title={tf(locale, 'transcription.voiceWidget.detectedLanguage', {
                  lang: detectedLang,
                })}
              >
                {detectedLang}
            ...
            {confidence > 0 && (
              <span className="voice-agent-confidence">{Math.round(confidence * 100)}%</span>
            )}
          </div>

          <div
            className={`voice-agent-session-body ${interimText ? 'voice-agent-interim' : 'voice-agent-final'} ${isSessionEmpty ? 'is-placeholder' : ''}`}
          >
            <span
          ...
          <div className="voice-agent-notice-stack">
            {disambiguationOptions.length > 0 && (
              <DialogShell
                ...
                actions={
                  <PanelChip variant="warning" className="voice-agent-confirm-fuzzy">
                    {t(locale, 'transcription.voiceWidget.disambiguation.badge')}
                  </PanelChip>
                }
                footer={
                  <PanelButton
              ...
              >
                <p className="voice-agent-confirm-label">
                  {t(locale, 'transcription.voiceWidget.disambiguation.label')}
                </p>
                <div className="voice-agent-disambiguation-options">
                  {disambiguationOptions.map((option) => (
                    ...
                    >
                      ...
                      <span className="voice-agent-disambiguation-score">
            ...
            {pendingConfirm && (
              <DialogShell
                ...
                actions={
                  pendingConfirm.fromFuzzy ? (
                    <PanelChip variant="warning" className="voice-agent-confirm-fuzzy">
                      {t(locale, 'ai.assistantHub.fuzzyMatch')}
                    </PanelChip>
                  ) : undefined
                }
                footer={
              ...
              >
                <p className="voice-agent-confirm-label">
                  {tf(locale, 'transcription.voiceWidget.confirm.prompt', {
          ...
          <div className="voice-agent-disclosure-group">
            <section className={`voice-agent-disclosure ${showSettings ? 'is-open' : ''}`}>
              <button
                ...
                aria-expanded={showSettings}
              >
                <span className="voice-agent-disclosure-title">
                  {t(locale, 'transcription.voiceWidget.settings.title')}
                </span>
                <MaterialSymbol name="expand_more" className={JIEYU_MATERIAL_INLINE} />
              </button>
                ...
                <div className="voice-agent-disclosure-panel voice-agent-settings-panel">
                  <div className="voice-agent-settings-grid">
                    <label className="voice-agent-field">
                      <span className="voice-agent-field-label">
                        {t(locale, 'transcription.voiceWidget.settings.recognitionLanguage')}
                      </span>
                      <select
                      ...
                      >
                        <option value="__auto__">
                          {t(locale, 'transcription.voiceWidget.settings.autoDetectOption')}
                        </option>
                    ...
                    <label className="voice-agent-field">
                      <span className="voice-agent-field-label">
                        {t(locale, 'transcription.voiceWidget.settings.engine')}
                      </span>
                      <select
                      ...
                      >
                        <option value="web-speech">
                          {t(locale, 'transcription.voiceWidget.engine.webSpeech')}
                        </option>
                        <option value="whisper-local">
                          {t(locale, 'transcription.voiceWidget.engine.whisperLocal')}
                        </option>
                        <option value="commercial">
                          {t(locale, 'transcription.voiceWidget.engine.commercial')}
                        </option>
                      </select>
                    </label>
                    {engine === 'commercial' &&
                      !hasCommercialCredentials(commercialProviderConfig) && (
                  ...
                  <div className="voice-agent-toggle-row">
                    <label
                      className="voice-agent-safe-toggle"
                      title={t(locale, 'transcription.voiceWidget.settings.safeModeTitle')}
                    >
                      <input
                        type="checkbox"
                        checked={safeMode}
                      ...
                      />
                      <span className="voice-agent-safe-label">
                        {t(locale, 'ai.assistantHub.safeMode')}
                      </span>
                    </label>

                    <label
                      className="voice-agent-wakeword-toggle"
                      title={t(locale, 'transcription.voiceWidget.settings.wakeWordTitle')}
                    >
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="voice-agent-wakeword-label">
                        {t(locale, 'transcription.voiceWidget.settings.wakeWord')}
                      </span>
                    </label>
                      ...
                      <label
                        ...
                        title={
                          assistantTtsSupported
                            ...
                            : t(
                                locale,
                                'transcription.voiceWidget.settings.assistantTtsUnsupported',
                              )
                        }
                      >
                        <input
                        ...
                        />
                        <span className="voice-agent-wakeword-label">
                          {t(locale, 'transcription.voiceWidget.settings.assistantTts')}
                        </span>
                      </label>
                    )}
                    ...
                    {wakeWordEnabled && !listening && (
                      <div
                        className="voice-agent-wakeword-energy"
                        title={tf(locale, 'transcription.voiceWidget.signal.wakeEnergy', {
                          percent: Math.round(wakeWordEnergyLevel * 500),
                        })}
                      >
                        <svg
                          className="voice-agent-energy-meter"
                        ...
                        >
                          <defs>
                            ...
                            >
                              ...
                              <stop
                                offset="100%"
                                stopColor="color-mix(in srgb, var(--state-info-solid) 70%, var(--surface-panel))"
                              />
                            </linearGradient>
                          </defs>
                          <rect
                            ...
                            x={0}
                            y={0}
                            width={wakeWordEnergyFillPercent}
                            height={6}
                            rx={3}
                            ry={3}
                            fill={`url(#${wakeWordEnergyGradientId})`}
                          />
                        </svg>
                      </div>
                    )}
                  ...
                  <div className="voice-agent-provider-overview">
                    <div className="voice-agent-provider-overview-header">
                      <span className="voice-agent-field-label">
                        {t(locale, 'transcription.voiceWidget.settings.providerHealth')}
                      </span>
                      <button
                        type="button"
                      ...
                      >
                        <MaterialSymbol
                          name="refresh"
                          className={jieyuMaterialClass(
                            JIEYU_MATERIAL_MICRO,
                            providerStatusRefreshing
                              ? 'voice-agent-provider-refresh-icon spinning'
                              : 'voice-agent-provider-refresh-icon',
                          )}
                        />
                        <span>
                          {providerStatusRefreshing
                            ? t(locale, 'ai.chat.testing')
                            : t(locale, 'transcription.voiceWidget.settings.refreshProviders')}
                        </span>
                      </button>
                    </div>
                    <div className="voice-agent-provider-status-list">
                      {providerStatuses.map((provider) => {
                        ...
                        return (
                          <div
                            key={provider.kind}
                            className="voice-agent-provider-status-item"
                            title={provider.error ?? provider.description}
                          >
                            <span
                              className={`voice-agent-provider-status-dot voice-agent-provider-status-dot-${statusTone}`}
                              aria-hidden="true"
                            />
                            <span className="voice-agent-provider-status-name">
                              {provider.label}
                            </span>
                            <span className="voice-agent-provider-status-text">
                              {getProviderStatusLabel(locale, provider.available)}
                            </span>
                          </div>
                        );
                      })}
                  ...
                  {showProviderConfig && (
                    <div className="voice-agent-commercial-config">
                      ...
                      <div className="voice-agent-config-actions">
                        ...
                        >
                          {providerAvailability === 'testing'
                            ? t(locale, 'ai.chat.testing')
                            : t(locale, 'ai.chat.testConnection')}
                        </button>
                        {providerAvailability === 'available' && (
                          <span className="voice-agent-config-ok">
                            <MaterialSymbol name="check" className={JIEYU_MATERIAL_MICRO} />{' '}
                            {t(locale, 'transcription.voiceWidget.provider.available')}
                          </span>
                        )}
                        {providerAvailability === 'unavailable' && (
                          <span
                            className="voice-agent-config-error"
                            title={providerError ?? undefined}
                          >
                            {t(locale, 'transcription.voiceWidget.provider.unavailable')}
                        ...
                        {engine === 'whisper-local' ? (
                          ...
                          <>
                            <input
                              ...
                              placeholder={
                                commercialProviderKind === 'minimax' ? 'eyJ...' : 'sk-...'
                              }
                  ...
                  {engine !== 'web-speech' && (
                    <div className="voice-agent-enhancement-config">
                      <label className="voice-agent-field">
                        <span className="voice-agent-field-label">
                          {t(locale, 'transcription.voiceWidget.settings.enhancement')}
                        </span>
                        <select
                          className="voice-agent-commercial-provider-select"
                          ...
                          onChange={(e) =>
                            onSttEnhancementKindChange?.(
                              e.currentTarget.value as SttEnhancementSelectionKind,
                            )
                          }
                          aria-label={t(locale, 'transcription.voiceWidget.settings.enhancement')}
                        >
                          <option value="none">
                            {t(locale, 'transcription.voiceWidget.settings.enhancementNone')}
                          </option>
                          ...
                          {selectedEnhancementDefinition && (
                            <div className="voice-agent-provider-overview voice-agent-enhancement-overview">
                              ...
                              <div className="voice-agent-provider-status-list">
                                <div
                                  ...
                                  title={
                                    ...
                                    selectedEnhancementDefinition.description
                                  }
                                >
                                  <span
                                    className={`voice-agent-provider-status-dot voice-agent-provider-status-dot-${enhancementStatusTone}`}
                                    aria-hidden="true"
                                  />
                                  <span className="voice-agent-provider-status-name">
                                    {selectedEnhancementDefinition.label}
                                  </span>
                                  <span className="voice-agent-provider-status-text">
                              ...
                              <div className="voice-agent-provider-status-note">
                                <span>
                                  {t(locale, 'transcription.voiceWidget.enhancement.externalHint')}
                                </span>
                                {enhancementFailureKindLabel ? (
                                  <span>{enhancementFailureKindLabel}</span>
                                ) : null}
                                {enhancementStatus?.error ? (
                                  <span>{enhancementStatus.error}</span>
                                ) : null}
                              </div>
                            </div>
                          )}
                          <div className="voice-agent-commercial-fields">
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                notifyEnhancementConfigChange(
                                  value,
                                  localEnhancementModel,
                                  localEnhancementLanguage,
                                );
                              }}
                              placeholder={t(
                                locale,
            ...
            <section className={`voice-agent-disclosure ${showInsights ? 'is-open' : ''}`}>
              ...
              >
                <span className="voice-agent-disclosure-title voice-agent-disclosure-title-with-icon">
                  <MaterialSymbol name="history" className={JIEYU_MATERIAL_INLINE} />{' '}
                  {t(locale, 'transcription.voiceWidget.insights.title')}
                </span>
                {insightCount > 0 && (
              ...
              {showInsights && (
                <div className="voice-agent-disclosure-panel voice-agent-insights-panel">
                  <div
                    className="voice-agent-insights-tabs panel-edge-nav panel-edge-nav--inline"
                    role="tablist"
                    aria-label={t(locale, 'transcription.voiceWidget.insights.tablist')}
                  >
                    <div
                      className={`panel-edge-nav-row ${insightTab === 'history' ? 'panel-edge-nav-row-active' : ''}`.trim()}
                    >
                      <button
                        type="button"
                        role="tab"
                      ...
                      >
                        ...
                        <span className="panel-edge-nav-label">
                          <strong className="panel-edge-nav-title">
                            {t(locale, 'transcription.voiceWidget.insights.historyTab')}
                          </strong>
                        </span>
                      </button>
                    </div>
                    <div
                      className={`panel-edge-nav-row ${insightTab === 'learning' ? 'panel-edge-nav-row-active' : ''}`.trim()}
                    >
                      <button
                        type="button"
                        role="tab"
                      ...
                      >
                        ...
                        <span className="panel-edge-nav-label">
                          <strong className="panel-edge-nav-title">
                            {t(locale, 'transcription.voiceWidget.insights.learningTab')}
                          </strong>
                        </span>
                      </button>
                    </div>
                  ...
                  {insightTab === 'history' ? (
                    session.entries.length > 0 ? (
                      <ul
                        className="voice-agent-history-list"
                        aria-label={t(locale, 'transcription.voiceWidget.insights.historyAria')}
                      >
                        {session.entries
                          .slice(-8)
                          .reverse()
                          .map((entry, index) => (
                            <li
                              ...
                              className="voice-agent-history-entry"
                            >
                              <span
                                className={`voice-agent-history-type voice-agent-history-type-${entry.intent.type}`}
                              >
                              ...
                              <span
                                ...
                                title={entry.sttText}
                                {...(entry.intent.type === 'dictation' &&
                                dictationPreviewTextProps?.dir
                                  ? { dir: dictationPreviewTextProps.dir }
                                  : {})}
                                {...(entry.intent.type === 'dictation' &&
                                dictationPreviewTextProps?.style
                                  ? { style: dictationPreviewTextProps.style }
                                  : {})}
                              >
                                {entry.sttText.length > 24
                                  ? `${entry.sttText.slice(0, 24)}…`
                                  : entry.sttText}
                              </span>
                              {entry.confidence > 0 && (
                      ...
                      <div className="voice-agent-learning-empty">
                        {t(locale, 'transcription.voiceWidget.empty.history')}
                      </div>
                    )
                  ) : learningLogEntries.length === 0 ? (
                    <div className="voice-agent-learning-empty">
                      {t(locale, 'transcription.voiceWidget.empty.learning')}
                    </div>
                  ) : (
                    <div className="voice-agent-learning-panel">
                      <div className="voice-agent-learning-panel-header">
                        <span>
                          {tf(locale, 'transcription.voiceWidget.learning.header', {
                            count: learningLogEntries.length,
                          })}
                        </span>
                        <button
                          type="button"
                        ...
                        {learningLogEntries
                          ...
                          .map((entry, index) => (
                            <li
                              ...
                              className="voice-agent-learning-entry"
                            >
                              <span className="voice-agent-learning-phrase" title={entry.phrase}>
                                {entry.phrase.length > 18
                                  ? `${entry.phrase.slice(0, 18)}…`
                                  : entry.phrase}
                              </span>
                              ...
                              <span
                                className={`voice-agent-learning-reason voice-agent-learning-reason-${entry.reason}`}
                              >
