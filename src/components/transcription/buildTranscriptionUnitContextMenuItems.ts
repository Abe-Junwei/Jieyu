...
export function buildTranscriptionUnitContextMenuItems(
  input: BuildTranscriptionUnitContextMenuItemsInput,
): ContextMenuItem[] {
  const {
    ctxMenu,
    locale,
  ...
  const restrictSkippedSegmentMenuActions =
    isSegmentUnitContext && isTranscriptionLayer && isSkipProcessing;
  /** 多选：选区内任一语段为跳过处理时，禁止会「包含」跳过段的批量合并。 */
  const selectionIncludesSkippedForMerge =
    multiCount > 1 &&
    Array.from(selectedUnitIds).some(
      (uid) => resolveSkipProcessingState?.(uid, ctxMenu.layerId, targetKind) === true,
    );
  ...
  const showSelfCertainty =
    Boolean(onSetUnitSelfCertaintyFromMenu) &&
    !isTranslation &&
    !restrictSkippedSegmentMenuActions &&
    !isWaveformSurface;
  const showSkipToggle =
    Boolean(onToggleSkipProcessingFromMenu) && isSegmentUnitContext && isTranscriptionLayer;
  const showSpeakerBlock =
    isTranscriptionLayer && !restrictSkippedSegmentMenuActions && !isWaveformSurface;
      ...
      ? [
          {
            ...
            danger: true,
            onClick: () => {
              runDeleteSelection(id, selectedUnitIds, targetKind, ctxMenu.layerId);
            },
          },
          ...(showMultiBatchMerge
            ? [
                {
                  ...
                  onClick: () => {
                    runMergeSelection(selectedUnitIds, targetKind, ctxMenu.layerId);
                  },
                },
              ]
            : []),
          ...(showSelectBeforeAfter
            ? [
                {
                  label: messages.selectBeforeAll,
                  shortcut: '⇧Home',
                  onClick: () => {
                    runSelectBefore(id);
                  },
      ...
      : [
          {
            ...
            danger: true,
            onClick: () => {
              runDeleteOne(id, targetKind, ctxMenu.layerId);
            },
          },
            ...
            ? [
                {
                  ...
                  shortcut: '⌘⇧M',
                  onClick: () => {
                    runMergePrev(id, targetKind, ctxMenu.layerId);
                  },
                },
                {
                  ...
                  shortcut: '⌘M',
                  onClick: () => {
                    runMergeNext(id, targetKind, ctxMenu.layerId);
                  },
                },
              ]
            : []),
            ...
            ? [
                {
                  ...
                  shortcut: '⌘⇧S',
                  onClick: () => {
                    runSplitAtTime(id, ctxMenu.splitTime, targetKind, ctxMenu.layerId);
                  },
                },
              ]
            : []),
            ...
            ? [
                {
                  ...
                  onClick: () => {
                    onToggleSkipProcessingFromMenu!(id, targetKind, ctxMenu.layerId);
                  },
                },
              ]
            : []),
  ...
  if (showSelfCertainty && onSetUnitSelfCertaintyFromMenu) {
    ...
    const scIds = selfCertaintyTargetUnitIds.length > 0 ? selfCertaintyTargetUnitIds : targetIds;
    items.push({
      ...
      children: [
        {
          ...
          onClick: () => {
            setSelfCertaintyFromMenu(scIds, targetKind, 'certain', ctxMenu.layerId);
          },
        },
        {
          label: t(locale, 'transcription.unit.selfCertainty.uncertain'),
          onClick: () => {
            setSelfCertaintyFromMenu(scIds, targetKind, 'uncertain', ctxMenu.layerId);
          },
        },
        {
          label: t(locale, 'transcription.unit.selfCertainty.notUnderstood'),
          onClick: () => {
            setSelfCertaintyFromMenu(scIds, targetKind, 'not_understood', ctxMenu.layerId);
          },
        },
        {
          label: t(locale, 'transcription.unit.selfCertainty.clear'),
          onClick: () => {
            setSelfCertaintyFromMenu(scIds, targetKind, undefined, ctxMenu.layerId);
          },
        },
      ],
    });
  ...
  if (showSpeakerBlock) {
    ...
    for (const speaker of recentSpeakerOptions) {
      speakerManageItems.push({
        ...
        onClick: () => {
          onAssignSpeakerFromMenu(targetIds, targetKind, speaker.id);
        },
      });
    }
    ...
    speakerManageItems.push({
      ...
      onClick: () => {
        onAssignSpeakerFromMenu(targetIds, targetKind, undefined);
      },
    });
    speakerManageItems.push({
  ...
  if (displayStyleControl && !isWaveformSurface) {
    const ctxLayer = [...transcriptionLayers, ...translationLayers].find(
