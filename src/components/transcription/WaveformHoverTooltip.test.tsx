...
describe('WaveformHoverTooltip', () => {
  const defaultProps = {
    ...
    y: 50,
    units: [
      makeUnit({ id: 'utt-1', startTime: 0, endTime: 10 }),
      makeUnit({ id: 'utt-2', startTime: 10, endTime: 20 }),
      makeUnit({ id: 'utt-3', startTime: 20, endTime: 30 }),
    ],
    getUnitTextForLayer: vi.fn(() => '测试文本'),
  ...
  it('shows text preview when hovering over an unit', () => {
    const props = {
      ...defaultProps,
      time: 5, // falls within utt-1 (0-10s)
      getUnitTextForLayer: vi.fn(() => '这是测试文本'),
    };
    const { getByText } = render(<WaveformHoverTooltip {...props} />);
    expect(getByText('这是测试文本')).toBeTruthy();
  });

  it('does not show text preview when no unit at time', () => {
    const props = {
      ...defaultProps,
      time: 35, // outside all units
    };
  ...
  it('truncates long text preview to 28 characters', () => {
    ...
    const props = {
      ...defaultProps,
      time: 5,
      getUnitTextForLayer: vi.fn(() => longerText),
    };
    const { container } = render(<WaveformHoverTooltip {...props} />);
  ...
  it('binary search finds unit at exact start time', () => {
    const props = {
      ...defaultProps,
      time: 0, // exact start of utt-1
      getUnitTextForLayer: vi.fn(() => '起始文本'),
    };
    const { getByText } = render(<WaveformHoverTooltip {...props} />);
    expect(getByText('起始文本')).toBeTruthy();
  });

  it('binary search finds unit at exact end time', () => {
    const props = {
      ...defaultProps,
      time: 10, // exact end of utt-1, which is start of utt-2
      getUnitTextForLayer: vi.fn((utt: LayerUnitDocType) => {
  ...
  it('binary search finds unit in middle of range', () => {
    const props = {
      ...defaultProps,
      time: 15, // middle of utt-2 (10-20)
      getUnitTextForLayer: vi.fn(() => '中间文本'),
    };
    const { getByText } = render(<WaveformHoverTooltip {...props} />);
  ...
  it('handles single unit', () => {
    const props = {
      ...defaultProps,
      units: [makeUnit({ id: 'utt-1', startTime: 0, endTime: 60 })],
      time: 30,
      getUnitTextForLayer: vi.fn(() => '单文本'),
    };
    const { getByText } = render(<WaveformHoverTooltip {...props} />);
    expect(getByText('单文本')).toBeTruthy();
  });

  it('does not show text preview when getUnitTextForLayer returns null', () => {
    const props = {
      ...defaultProps,
      time: 5,
      getUnitTextForLayer: vi.fn(() => null),
    };
    const { container } = render(<WaveformHoverTooltip {...props} />);
    expect(container.querySelector('.waveform-hover-tooltip-text')).toBeNull();
  });

  it('does not show text preview when getUnitTextForLayer returns undefined', () => {
    const props = {
      ...defaultProps,
      time: 5,
      getUnitTextForLayer: vi.fn(() => undefined),
    };
    const { container } = render(<WaveformHoverTooltip {...props} />);
    expect(container.querySelector('.waveform-hover-tooltip-text')).toBeNull();
  });

  it('does not show text preview when getUnitTextForLayer returns empty string', () => {
    const props = {
      ...defaultProps,
      time: 5,
  ...
  it('calls getUnitTextForLayer with correct unit', () => {
    const getUnitTextForLayer = vi.fn(() => '文本');
    const props = {
      ...defaultProps,
      time: 5,
      getUnitTextForLayer,
    ...
    expect(getUnitTextForLayer).toHaveBeenCalledWith(
