    ...
    <EmbeddedPanelShell
      ...
      footer={
        <div className="ai-chat-prompt-lab-action-row ai-chat-prompt-lab-action-row-footer">
          <PanelButton
            className="ai-chat-prompt-lab-action-btn"
            disabled={
              templateTitleInput.trim().length === 0 || templateContentInput.trim().length === 0
            }
            onClick={onSaveTemplate}
          >
            {editingTemplateId ? messages.update : messages.save}
          </PanelButton>
          <PanelButton
            variant="primary"
            className="ai-chat-prompt-lab-action-btn ai-chat-prompt-lab-action-btn-primary"
            disabled={templateContentInput.trim().length === 0}
            onClick={onInjectAndClose}
          >
            {messages.injectToInput}
          </PanelButton>
        </div>
      }
    >
      ...
      <PanelSection
        className="ai-chat-prompt-lab-section ai-chat-prompt-lab-library-surface"
        ...
        {promptTemplates.length === 0 ? (
          ...
          promptTemplates.map((item) => (
            <div key={item.id} className="ai-chat-prompt-lab-template-row">
              <span
                className="small-text ai-chat-prompt-lab-item-title ai-chat-prompt-lab-item-title-ellipsis"
                title={item.content}
              >
                {item.title}
              </span>
              <PanelButton
                className="ai-chat-prompt-lab-mini-btn"
                onClick={() => onInjectTemplate(item.content)}
              >
                {messages.inject}
              </PanelButton>
              <PanelButton
              ...
              <PanelButton
                variant="danger"
                className="ai-chat-prompt-lab-mini-btn ai-chat-prompt-lab-mini-btn-danger"
      ...
      >
        <label htmlFor={`${fieldIdPrefix}-title`} className="layer-action-dialog-field-label">
        ...
        />
        <label htmlFor={`${fieldIdPrefix}-content`} className="layer-action-dialog-field-label">
        ...
        <div className="ai-chat-prompt-lab-token-row">
          {['selected_text', 'current_unit', 'lexicon_summary', 'project_stage', 'current_row'].map(
            (token) => (
              <PanelButton
                key={token}
                className="ai-chat-prompt-lab-token-btn"
                onClick={() => onAppendPromptVariable(token)}
              >{`{{${token}}}`}</PanelButton>
            ),
          )}
        </div>
