...
describe('AiChatCard input submit', () => {
  ...
  it('shows decision explanation message and reason in decision panel', () => {
    ...
    expect(
      within(view.container).getByText(/User directive blocks destructive action\./),
    ).toBeTruthy();
    expect(
      within(view.container).getByText((_, element) =>
        Boolean(
          element &&
  ...
  it('tracks recommendation exposure and exact adoption after Tab accepts suggestion', async () => {
    ...
    await waitFor(() => {
      expect(onTrackAiRecommendationEvent).toHaveBeenCalledWith(
  ...
  it('tracks a fresh shown event when the same recommendation becomes visible again', async () => {
    ...
    await waitFor(() => {
      expect(
        onTrackAiRecommendationEvent.mock.calls.filter(([event]) => event?.type === 'shown'),
      ).toHaveLength(1);
    });
  ...
  it('renders follow-up chips from the latest local-query frame and sends the selected follow-up', () => {
    ...
    const followUpButton = within(view.container).getByRole('button', {
      name: /按说话人分别统计|Break down by speaker/i,
    });
  ...
  it('shows stop button while assistant is streaming without persistent hint', () => {
    ...
    const view = render(
      <AiAssistantHubContext.Provider
        value={makeContextValue({ onSendAiMessage, aiIsStreaming: true })}
      >
        <AiChatCard embedded />
      </AiAssistantHubContext.Provider>,
    );
    ...
    const stopButton = within(view.container).getByRole('button', {
      name: /停止|Stop/i,
  ...
  it('shows blocked reason and disables send button while high-risk tool is pending', () => {
    ...
    expect(
      within(view.container).getByText(/待确认的高风险操作|high-risk action is pending/i),
    ).toBeTruthy();
    expect(
      within(view.container).getAllByText(/删除操作确认|destructive action/i).length,
    ).toBeGreaterThan(0);
    const sendButton = within(view.container).getByRole('button', {
      name: /发送|Send/i,
    }) as HTMLButtonElement;
  ...
  it('shows user-friendly pending target label instead of full internal id', () => {
    ...
    const alertToggle = within(view.container).getByRole('button', {
      name: /展开|Expand|收起|Hide/i,
    });
    ...
    expect(
      within(view.container).getByRole('button', { name: /确认删除|Confirm Delete/i }),
    ).toBeTruthy();
  });
  ...
  it('does not dispatch resume input while ai stream is active', () => {
    ...
    const resumeButton = within(view.container).getByRole('button', {
      name: /Resume|继续执行/i,
    }) as HTMLButtonElement;
  ...
  it('renders RAG quick scenario shortcuts and injects the selected-text template into input', () => {
    ...
    expect(
      within(view.container).queryByRole('button', { name: /RAG 快捷场景|RAG Quick Scenarios/i }),
    ).toBeNull();
    fireEvent.click(
      within(view.container).getByRole('button', { name: /RAG 问答模板|RAG QA Template/i }),
    );
  ...
  it('renders all RAG templates directly without More menu and injects the terminology template', () => {
    ...
    fireEvent.click(
      within(view.container).getByRole('button', {
        name: /RAG 术语查证模板|RAG Terminology Template/i,
      }),
    );
  ...
  it('renders citation action buttons in fixed priority order', () => {
    ...
    const copyIndex = actionButtons.findIndex((btn) =>
      /复制|Copy/i.test(btn.getAttribute('aria-label') ?? ''),
    );
    const uttIndex = actionButtons.findIndex(
      (btn) => (btn.textContent?.trim() ?? '') === '句段参考',
    );
    const noteIndex = actionButtons.findIndex(
      (btn) => (btn.textContent?.trim() ?? '') === '笔记参考',
    );
    const pdfIndex = actionButtons.findIndex(
      (btn) => (btn.textContent?.trim() ?? '') === '文档参考',
    );
  ...
  it('hides legacy unit id labels and shows friendly citation text', () => {
          ...
          aiMessages: [
            {
              ...
              citations: [
                {
                  type: 'unit',
                  refId: 'utt_1773986765082_joj08x',
                  label: 'utt:utt_1773986765082_joj08x',
                },
              ],
            },
          ],
    ...
    expect(
      within(view.container).getByRole('button', { name: /语段参照|句段参考|Unit ref|Unit Ref/i }),
    ).toBeTruthy();
  ...
  it('shows model generated text for llm assistant replies', () => {
    ...
    expect(
      within(view.container).getByText(/DeepSeek-Chat\s+生成|DeepSeek-Chat\s+Generated/i),
    ).toBeTruthy();
  });
  ...
  it('opens replay details for a decision log requestId', async () => {
    const openReplayBundleByRequestIdSpy = vi
      .spyOn(replayUtils, 'openReplayBundleByRequestId')
      .mockResolvedValue({
        bundle: {
    ...
    const decisionPanelToggle = within(view.container).getByRole('button', {
      name: /AI 决策|AI Decisions/i,
    });
    ...
    fireEvent.click(
      within(view.container).getByRole('button', {
        name: /查看\s*回放\s*\/\s*对比|Replay\s*\/\s*Compare/i,
      }),
    );
    ...
    await waitFor(
      () => {
        expect(
          within(view.container).getByRole('button', {
            name: /展开详情|Show detail|收起详情|Hide detail/i,
          }),
        ).toBeTruthy();
      },
      { timeout: 3000 },
    );
    const replayDetailToggle = within(view.container).getByRole('button', {
      name: /展开详情|Show detail|收起详情|Hide detail/i,
    });
    ...
    await waitFor(
      () => {
        ...
        expect(
          within(view.container).getByText(/Golden 快照预览|Golden Snapshot Preview/i),
        ).toBeTruthy();
      },
      { timeout: 3000 },
    );
  ...
  it('opens replay from latest vertical workflow detail when requestId is present', async () => {
    const dispatchAppGlobalToastSpy = vi
      .spyOn(appGlobalToast, 'dispatchAppGlobalToast')
    ...
    expect(dispatchAppGlobalToastSpy).toHaveBeenCalledWith({
      message: expect.stringMatching(
        /已复制 Request ID: toolre\.\.\.1234|Request ID copied: toolre\.\.\.1234/i,
      ),
      variant: 'success',
      autoDismissMs: 1400,
    });
    ...
    fireEvent.click(
      within(view.container).getByRole('button', {
        name: /查看回放\s*\/\s*对比|Replay\s*\/\s*Compare/i,
      }),
    );
    ...
    await waitFor(
      () => {
        expect(openReplayBundleByRequestIdSpy).toHaveBeenCalledTimes(1);
        expect(openReplayBundleByRequestIdSpy).toHaveBeenCalledWith(
          'toolreq_vertical_very_long_id_1234',
          null,
          expect.any(Boolean),
        );
      },
    ...
    expect(
      within(view.container)
        .getByRole('button', { name: /AI 决策|AI Decisions/i })
        .getAttribute('aria-expanded'),
    ).toBe('true');
    await waitFor(
      () => {
        const targetDecisionItem = view.container.querySelector(
          '[data-ai-decision-request-id="toolreq_vertical_very_long_id_1234"]',
        );
  ...
  it('keeps vertical workflow replay action disabled when requestId is missing', async () => {
    ...
    fireEvent.click(within(view.container).getByRole('button', { name: /查看详情|Show detail/i }));
    const replayButton = within(view.container).getByRole('button', {
      name: /查看回放\s*\/\s*对比|Replay\s*\/\s*Compare/i,
    });
  ...
  it('exports golden snapshot from the decision log entry', async () => {
    ...
    fireEvent.click(within(view.container).getByRole('button', { name: /AI 决策|AI Decisions/i }));
    fireEvent.click(
      within(view.container).getByRole('button', { name: /导出快照|Export Snapshot/i }),
    );
    ...
    await waitFor(() => {
      ...
      expect(
        within(view.container).getByRole('button', { name: /已导出快照|Snapshot Exported/i }),
      ).toBeTruthy();
    });
  });
  ...
  it('imports a golden snapshot and renders diff panel against live replay bundle', async () => {
    ...
    await waitFor(() => {
      expect(
        document.querySelector('.ai-chat-replay-panel-file-input[type="file"][accept=".json"]'),
      ).not.toBeNull();
    });
    const fileInput = document.querySelector(
      '.ai-chat-replay-panel-file-input[type="file"][accept=".json"]',
    ) as HTMLInputElement;
  ...
  describe('ADR-0028 same-session keyboard send vs assistant dialogue', () => {
    ...
    it('blocks Enter submit while voice confirmation is active', () => {
      ...
      expect(onSendAiMessage).not.toHaveBeenCalled();
      expect(
        within(view.container).getByText(/voice assistant is waiting|语音助手正在等待/i),
      ).toBeTruthy();
    });
