...
import { resolveOrthographyCatalogGroupKey } from '~/hooks/orthography/useOrthographyPicker';

export function getOrthographyCatalogBadgeInfo(
  locale: Locale,
  orthography: Pick<OrthographyDocType, 'catalogMetadata'>,
) {
  const groupKey = resolveOrthographyCatalogGroupKey(orthography);
  const className =
    groupKey === 'reviewed-primary'
