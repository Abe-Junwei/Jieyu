...
function makeEmbeddingContextValue(
  overrides: Partial<EmbeddingContextValue> = {},
...
describe('AiAnalysisPanel embedding integration', () => {
  ...
  function enableExternalProviderForHealthCheck(
    ...
    fireEvent.click(
      within(container).getByLabelText(/启用外部 provider|Enable external provider/i),
    );
  ...
  it('does not render pending high-risk preview in analysis panel after hub decoupling', () => {
    ...
    expect(
      screen.queryByText(/高风险工具调用待确认|High-risk tool call pending confirmation/i),
    ).toBeNull();
  ...
  it('renders embedding fallback warning when provided', () => {
    const baseContext = makeContextValue();
    const embeddingCtx = makeEmbeddingContextValue({
      aiEmbeddingWarning:
        'Running fallback embedding (model unavailable). Retrieval quality may degrade.',
    });
  ...
  it('renders dedicated acoustic tab content and supports jumping to hotspots', async () => {
            ...
            frames: [
              {
                ...
                spectralRolloffHz: 2120,
                zeroCrossingRate: 0.036,
                spectralFlatness: 0.182,
                loudnessDb: -19.8,
                mfccCoefficients: [11.8, -3.9, 1.2],
                formantF1Hz: 490,
                formantF2Hz: 1500,
                formantReliability: 0.52,
                normalizedF0: 0.18,
                normalizedIntensity: 0.22,
              },
              {
                timeSec: 2.04,
                relativeTimeSec: 0.84,
                timeRatio: 0.38,
                f0Hz: 225,
                intensityDb: -14.6,
                reliability: 0.7,
                spectralCentroidHz: 1120,
                spectralRolloffHz: 2320,
                zeroCrossingRate: 0.042,
                spectralFlatness: 0.204,
                loudnessDb: -18.4,
                mfccCoefficients: [12.0, -3.5, 1.5],
                formantF1Hz: 520,
                formantF2Hz: 1640,
                formantReliability: 0.58,
                normalizedF0: 0.34,
                normalizedIntensity: 0.48,
              },
              {
                timeSec: 2.08,
                relativeTimeSec: 0.88,
                timeRatio: 0.4,
                f0Hz: 241,
                intensityDb: -13.2,
                reliability: 0.76,
                spectralCentroidHz: 1264,
                spectralRolloffHz: 2710,
                zeroCrossingRate: 0.052,
                spectralFlatness: 0.231,
                loudnessDb: -17.4,
                mfccCoefficients: [12.4, -3.2, 1.9],
                formantF1Hz: 560,
                formantF2Hz: 1760,
                formantReliability: 0.62,
                normalizedF0: 0.54,
                normalizedIntensity: 0.64,
              },
              {
                timeSec: 2.12,
                relativeTimeSec: 0.92,
                timeRatio: 0.42,
                f0Hz: 256,
                intensityDb: -12.6,
                reliability: 0.81,
                spectralCentroidHz: 1310,
                spectralRolloffHz: 2830,
                zeroCrossingRate: 0.056,
                spectralFlatness: 0.252,
                loudnessDb: -16.8,
                mfccCoefficients: [12.9, -2.9, 2.2],
                formantF1Hz: 590,
                formantF2Hz: 1860,
                formantReliability: 0.66,
                normalizedF0: 0.72,
                normalizedIntensity: 0.76,
              },
              {
                timeSec: 2.16,
                relativeTimeSec: 0.96,
                timeRatio: 0.44,
                f0Hz: 272,
                intensityDb: -12.1,
                reliability: 0.86,
                spectralCentroidHz: 1360,
                spectralRolloffHz: 2920,
                zeroCrossingRate: 0.06,
                spectralFlatness: 0.267,
                loudnessDb: -16.0,
                mfccCoefficients: [13.2, -2.5, 2.4],
                formantF1Hz: 640,
                formantF2Hz: 1940,
                formantReliability: 0.7,
                normalizedF0: 0.92,
                normalizedIntensity: 0.88,
              },
            ],
            toneBins: [
              {
    ...
    expect(within(container).getAllByText(/^声学摘要$|^Acoustic Summary$/i).length).toBeGreaterThan(
      0,
    );
    ...
    expect(
      within(container).getByRole('button', { name: /跳到关联热点|Jump to linked hotspot/i }),
    ).toBeTruthy();
    expect(
      within(container).getByRole('button', { name: /跳到最强热点|Jump to top hotspot/i }),
    ).toBeTruthy();
  ...
  it('restores acoustic navigation interactions after runtime recovers from error', () => {
    ...
    const hotspotButtons = within(container).getAllByRole('button', {
      name: /音高断裂|Pitch break/i,
    });
    const selectedHotspotButton = hotspotButtons.find(
      (node) => node.getAttribute('aria-pressed') === 'true',
    );
    expect(selectedHotspotButton).toBeTruthy();

    fireEvent.click(
      within(container).getByRole('button', { name: /跳到选区起点|Jump to selection start/i }),
    );
  ...
  it('applies preset updates only after clicking apply', () => {
    ...
    const presetSelect = container.querySelector(
      '.transcription-analysis-acoustic-preset-select',
    ...
    expect(onChangeAcousticConfig).toHaveBeenCalledWith(
  ...
  it('resets acoustic draft and applies reset after confirmation click', () => {
    ...
    fireEvent.click(
      within(container).getByRole('button', { name: /重置参数草稿|Reset parameter draft/i }),
    );
  ...
  it('clamps numeric acoustic config inputs in draft and applies once', () => {
    ...
    const pitchFloorInput = within(container).getByLabelText(
      /基频下限|Pitch floor/i,
    ) as HTMLInputElement;
    ...
    fireEvent.click(within(container).getByRole('button', { name: /应用参数|Apply parameters/i }));
    expect(onChangeAcousticConfig).toHaveBeenLastCalledWith(
      { pitchFloorHz: 500 },
      { replace: true },
    );
  ...
  it('enables batch selection export scope and disables PitchTier in batch mode', async () => {
    ...
    const pitchTierBtn = within(container).getByRole('button', {
      name: /导出 PitchTier|Export PitchTier/i,
    }) as HTMLButtonElement;
    expect(pitchTierBtn.disabled).toBe(true);
    expect(
      within(container).getByText(
        /已选 3 个，导出 2 个，跳过 1 个|3 selected, 2 exportable, 1 skipped/i,
      ),
    ).toBeTruthy();
  ...
  it('saves provider runtime config and triggers provider state refresh callback', () => {
    ...
    const endpointInput = within(container).getByLabelText(
      /外部 endpoint|External endpoint/i,
    ) as HTMLInputElement;
    ...
    const enableCheckbox = within(container).getByLabelText(
      /启用外部 provider|Enable external provider/i,
    ) as HTMLInputElement;
    ...
    expect(window.localStorage.getItem(ACOUSTIC_PROVIDER_STORAGE_KEYS.externalEnabled)).toBe(
      'true',
    );
    expect(window.localStorage.getItem(ACOUSTIC_PROVIDER_STORAGE_KEYS.externalEndpoint)).toBe(
      'https://provider.example.dev/analyze',
    );
    expect(window.localStorage.getItem(ACOUSTIC_PROVIDER_STORAGE_KEYS.externalTimeoutMs)).toBe(
      '4200',
    );
  ...
  it('shows timeout provider health status when probe exceeds timeout', async () => {
    ...
    expect(
      await within(container).findByText(/^请求超时$|^Timed out$/i, {}, { timeout: 2000 }),
    ).toBeTruthy();
  ...
  it('supports auto provider preference and maps it to null', () => {
    ...
    fireEvent.change(providerSelect as HTMLSelectElement, {
      target: { value: 'enhanced-provider' },
    });
