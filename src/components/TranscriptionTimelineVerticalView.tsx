...
const EMPTY_SPEAKER_VISUAL_BY_UNIT_ID = Object.freeze({}) as Record<
  string,
...
export function TranscriptionTimelineVerticalView({
  ...
  const [internalVerticalPaneFocus, setInternalVerticalPaneFocus] =
    useState<TranscriptionVerticalPaneFocusState>(() => ({
      ...DEFAULT_TRANSCRIPTION_VERTICAL_PANE_FOCUS,
    }));
  const verticalPaneFocus =
    verticalPaneFocusProp != null && updateVerticalPaneFocus != null
      ? verticalPaneFocusProp
      : internalVerticalPaneFocus;
  const patchVerticalPaneFocus = useCallback(
  ...
  const [pairedReadingBundleFilterRootId, setPairedReadingBundleFilterRootId] = useState<
    string | null
  >(null);
  ...
  const [layerAction, setLayerAction] = useState<{
    action: PairedReadingLayerActionType;
    layerId: string | undefined;
  } | null>(null);
  const [pairedReadingColumnLeftGrow, setPairedReadingColumnLeftGrow] = useState(
    readStoredPairedReadingColumnLeftGrow,
  );
  const [pairedReadingEditorHeightByGroup, setPairedReadingEditorHeightByGroup] = useState<
  ...
  const [isPairedReadingColumnSplitDragging, setIsPairedReadingColumnSplitDragging] =
    useState(false);
  ...
  const measurePairedReadingSplitLayout = useCallback(() => {
    ...
    const clearSplitterLineTopVar = () => {
      const innerEl = host?.querySelector(
        '.timeline-paired-reading-split-host-inner',
      ) as HTMLElement | null;
      if (innerEl?.style.getPropertyValue('--timeline-paired-reading-global-splitter-line-top')) {
    ...
    if (!host || compactMode !== 'both' || isNarrowPairedReadingLayout) {
      ...
      if (host?.style.getPropertyValue('--paired-reading-column-split-center-x')) {
    ...
    const src = group?.querySelector(
      '.timeline-paired-reading-source-column',
    ) as HTMLElement | null;
    const tgt = group?.querySelector(
      '.timeline-paired-reading-target-column',
    ) as HTMLElement | null;
    if (!group || !src || !tgt) {
      clearSplitterLineTopVar();
      if (host.style.getPropertyValue('--paired-reading-column-split-center-x')) {
    ...
    const centerVar = `${gapCenterFromHostPaddingLeft}px`;
    if (host.style.getPropertyValue('--paired-reading-column-split-center-x') !== centerVar) {
      host.style.setProperty('--paired-reading-column-split-center-x', centerVar);
    }
    ...
    if (inner && group) {
      ...
      const rowGap =
        parseFloat(innerStyle.rowGap || innerStyle.columnGap || innerStyle.gap || '8') || 8;
      ...
      const lineTopPx = Math.max(
        0,
        Math.round(layoutTop + PAIRED_READING_GLOBAL_SPLITTER_LINE_EXTRA_OFFSET_PX),
      );
      /* split-host-inner 顶 → 分割条命中区顶；无吸顶层头时 fromHeader 为 0，仅靠首组 offset */
      const lineTopVar = `${lineTopPx}px`;
      if (
        inner.style.getPropertyValue('--timeline-paired-reading-global-splitter-line-top') !==
        lineTopVar
      ) {
        inner.style.setProperty('--timeline-paired-reading-global-splitter-line-top', lineTopVar);
      }
    }
  ...
  const handlePairedReadingColumnSplitPointerDown = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>) => {
      ...
      const applyPairedReadingColumnSplitClientX = (clientX: number) => {
        ...
        if (m && m.trackPx > m.gapPx + 4) {
          raw =
            ((clientX - paddingLeftEdge - m.trackStartFromHostPaddingLeft - m.gapPx / 2) /
              (m.trackPx - m.gapPx)) *
            100;
        } else {
      ...
      const finish = (ev: PointerEvent) => {
        ...
        try {
          localStorage.setItem(
            PAIRED_READING_COLUMN_LEFT_GROW_KEY,
            String(pairedReadingColumnLeftGrowRef.current),
          );
        } catch {
          /* ignore */
        }
    ...
    [
      compactMode,
      isNarrowPairedReadingLayout,
      measurePairedReadingSplitLayout,
      resetPairedReadingColumnsToEqualWidth,
    ],
  );

  const [saveStatusByCellKey, setSaveStatusByCellKey] = useState<
    Record<string, NonNullable<TimelineDraftSaveStatus>>
  >({});
  const setPairedReadingCellSaveStatus = useCallback(
    (cellKey: string, status?: NonNullable<TimelineDraftSaveStatus>) => {
  ...
  const runPairedReadingSaveWithStatus = useCallback(
    async (cellKey: string, saveTask: () => Promise<void>) => {
      ...
      try {
        ...
        const blocked =
          err instanceof Error && err.message === 'PAIRED_READING_SEGMENT_PERSIST_BLOCKED';
        if (!blocked) {
  ...
  const canOpenTranslationCreate = effectiveAllLayersOrdered.some(
    (item) => item.layerType === 'transcription',
  );
  ...
  const pairedReadingUsesSegmentSourceRows = useMemo(
    () => transcriptionLayers.some((l) => layerUsesOwnSegments(l, defaultTranscriptionLayerId)),
  ...
  const groups = useMemo(() => {
    ...
    return buildVerticalReadingGroups({
      ...
      getSourceText: (unit) => {
        const layerId =
          (typeof unit.layerId === 'string' && unit.layerId.trim()) || sourceLayer?.id;
  ...
  useLayoutEffect(() => {
    ...
    const host = pairedReadingSplitHostRef.current;
    if (!host) return;
    let resizeObserverRafId: number | null = null;
    const scheduleMeasureFromResizeObserver = () => {
    ...
    const ro =
      typeof ResizeObserver !== 'undefined'
        ? new ResizeObserver(() => {
            scheduleMeasureFromResizeObserver();
          })
        : null;
    ro?.observe(host);
    const headerEl = host.querySelector('.timeline-paired-reading-header') as HTMLElement | null;
  ...
  const persistGroupTranslation = useCallback(
    async (
      ...
      await Promise.all(
        anchorUnitIds.map((unitId) => saveUnitLayerText(unitId, value, persistLayer.id)),
      );
    },
    [
      defaultTranscriptionLayerId,
  ...
  const persistPairedReadingTargetTranslation = useCallback(
    async (
      persistLayer: LayerDocType,
      ...
      const segId =
        typeof targetItem.translationSegmentId === 'string'
          ? targetItem.translationSegmentId.trim()
          : '';
  ...
  const persistSourceText = useCallback(
    async (unitId: string, value: string, layerId?: string) => {
  ...
  const [pairedReadingResizeFontPreviewByLayerId, setPairedReadingResizeFontPreviewByLayerId] =
    useState<Record<string, number>>({});

  const handlePairedReadingEditorLaneChange = useCallback(
    (groupKey: string, nextHeight: number) => {
  ...
  const handlePairedReadingEditorResizeEnd = useCallback(
    (groupKey: string, finalHeight: number) => {
      ...
      const applyFont = (layer: LayerDocType, height: number) => {
        const pol = resolveOrthographyRenderPolicy(
          layer.languageId,
          orthographies,
          layer.orthographyId,
        );
  ...
  const handlePairedReadingEditorResizePreview = useCallback(
    (layerKey: string, previewHeight: number) => {
      ...
      for (const tl of groupTranslationLayers) {
        const tgtPol = resolveOrthographyRenderPolicy(
          tl.languageId,
          orthographies,
          tl.orthographyId,
        );
  ...
  const showBundleChips =
    orderedDistinctBundleKeys.length > 1 && pairedReadingBundleFilterRootId == null;
  ...
  const isPairedReadingEditorResizing =
    typeof resizingPairedReadingEditorId === 'string' &&
  ...
  const pairedReadingShellStyle = useMemo(
    ...
    [pairedReadingColumnLeftGrow, defaultPairedReadingEditorMinHeight],
  );
  ...
  useEffect(() => {
    ...
    const syncTranslationLayer =
      translationLayers.find((l) => l.id === focusedLayerRowId) ?? targetLayer;
    const targetCellIdForSync =
      focusedIsTranslation && syncTranslationLayer
        ? resolvePairedReadingSyncedTargetCellId({
            group: matchedGroup,
  ...
  useLayoutEffect(() => {
    ...
    for (const node of host.querySelectorAll('[data-paired-reading-group-id]')) {
      if (
        ...
        node.getAttribute('data-paired-reading-group-id') === matchedGroup.id
      ) {
        targetEl = node;
        break;
      }
    ...
    >
      <div className="timeline-paired-reading-workspace">
        <div className="timeline-paired-reading-toolbar">
          <div
            className="timeline-paired-reading-mode-toggle"
            role="group"
            aria-label={t(locale, 'transcription.pairedReading.columnMode')}
          >
            <button
              type="button"
          ...
          <div
            className="timeline-paired-reading-header-actions"
            role="group"
            aria-label={t(locale, 'transcription.pairedReading.columnMode')}
          >
            {sourceLayer ? (
              <button
                ...
                onContextMenu={(event) =>
                  openPairedReadingMenuAtPointer(event, pairedReadingHeaderMenuItems.source)
                }
              >
                {sourceHeaderContent}
              </button>
            ...
            {headerTargetLayers.map((layer, index) => {
              ...
              const layerHeaderContent = resolvePairedReadingHeaderContentForLayer(
                layer,
                layerHeaderLabel,
              );
              const targetHeaderTestId =
                index === 0
              ...
              return (
                <button
                  ...
                  onContextMenu={(event) =>
                    openPairedReadingMenuAtPointer(
                      event,
                      buildPairedReadingLayerHeaderMenuItems(layer, layerHeaderContent),
                    )
                  }
                >
                  {layerHeaderContent}
          ...
          {pairedReadingLayerStyleMenuItems.length > 0 ? (
            <button
              ...
              onClick={(event) =>
                togglePairedReadingMenuFromButton(event, pairedReadingLayerStyleMenuItems)
              }
            >
