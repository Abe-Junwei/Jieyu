...
interface TranscriptionTimelineVerticalViewGroupListProps {
  ...
  setPairedReadingCellSaveStatus: (
    cellKey: string,
    status?: NonNullable<TimelineDraftSaveStatus>,
  ) => void;
  ...
  resolveNoteIndicatorTarget?:
    | ((unitId: string, layerId?: string, scope?: 'timeline' | 'waveform') => NoteIndicator)
    | undefined;
  resolveSelfCertaintyForUnit?:
    | ((unitId: string, layerId?: string) => UnitSelfCertainty | undefined)
    | undefined;
  resolveSelfCertaintyAmbiguityForUnit?:
    | ((unitId: string, layerId?: string) => boolean)
    | undefined;
  handleNoteClick?:
    | ((unitId: string, layerId: string | undefined, event: React.MouseEvent) => void)
    | undefined;
  ...
  navigateUnitFromInput?:
    | ((e: React.KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>, direction: -1 | 1) => void)
    | undefined;
  ...
  return (
    <>
      {visibleGroups.map((group, groupIndex) => {
        ...
        const { primaryColumnItems, secondaryColumnItems } =
          partitionPairedReadingSourceItemsForDualTranscriptionColumns({
        ...
        const targetEmptyReason =
          groupTranslationLayers.length === 0 && !usesSecondaryTranscriptionColumn
            ? resolveVerticalReadingGroupEmptyReason(
                group,
                translationLayers,
                transcriptionLayers,
        ...
        const baseReuseGroupTargetItems = !(
          ...
          group.targetItems.every(
            (item) => normalizePairedReadingPlainText(item.text ?? '').length === 0,
          )
        );
        const anchorForUi = resolveVerticalReadingGroupAnchorForUi(
          group,
          contextMenuSourceUnitId,
          activeUnitId,
        );
        const derivedActive =
          activeUnitId != null && group.sourceItems.some((item) => item.unitId === activeUnitId);
        const isGroupActive =
          activeVerticalReadingGroupId === group.id ||
          (activeVerticalReadingGroupId == null && derivedActive);
        ...
        const pairedReadingEditorHeight =
          pairedReadingEditorHeightByGroup[pairedReadingEditorGroupKey] ??
          defaultPairedReadingEditorMinHeight;
        ...
        const pairedReadingEditorResizingThisGroup =
          resizingPairedReadingEditorId === pairedReadingEditorGroupKey;
        const isTargetHeaderActive =
          pairedReadingTargetSide === 'target' ||
          (pairedReadingTargetSide == null &&
            translationLayers.some((l) => l.id === focusedLayerRowId));
          ...
          >
            ...
            <div className="timeline-paired-reading-source-column">
              {primaryColumnItems.map((pItem, pi) => {
                ...
                const renderOne = (
                  item: PairedReadingSourceItem,
                  rowKind: 'primary' | 'dependent',
                ) => {
                  ...
                  const focusCellId =
                    rowKind === 'dependent'
                      ...
                      : `source:${item.unitId}`;
                  const isSourceCardActive =
                    item.unitId === activeUnitId || activeVerticalReadingCellId === focusCellId;
                  ...
                  const isSourceDraftEmpty =
                    normalizePairedReadingPlainText(sourceDraft).trim().length === 0;
                  const sourceSelfCertainty = resolveSelfCertaintyForUnit?.(
                    item.unitId,
                    sourceLayerId || undefined,
                  );
                  const sourceSelfCertaintyAmbiguous =
                    !sourceSelfCertainty &&
                    resolveSelfCertaintyAmbiguityForUnit?.(
                      item.unitId,
                      sourceLayerId || undefined,
                    ) === true;
                  const sourceNoteIndicator =
                    resolveNoteIndicatorTarget?.(item.unitId, sourceLayerId || undefined) ?? null;
                  ...
                  const hasSourceBadge = Boolean(
                    sourceSelfCertainty || sourceSelfCertaintyAmbiguous || sourceNoteIndicator,
                  );
                  const sourceItemLayer = transcriptionLayers.find(
                    (layer) => layer.id === sourceLayerId,
                  );
                  const sourcePreviewFont =
                    pairedReadingEditorResizingThisGroup && sourceLayerId
                      ? pairedReadingResizeFontPreviewByLayerId[sourceLayerId]
                      : undefined;
                  const sourceTypography = sourceItemLayer
                    ? buildOrthographyPreviewTextProps(
                        resolveOrthographyRenderPolicy(
                          sourceItemLayer.languageId,
                          orthographies,
                          sourceItemLayer.orthographyId,
                        ),
                  ...
                  const sourceRowTitle = sourceItemLayer
                    ...
                    : t(locale, 'transcription.pairedReading.sourceHeader');
                  const sourceRailAriaLabel = tf(
                    locale,
                    'transcription.pairedReading.rowRailFocusLayer',
                    { layer: sourceRowTitle },
                  );
                  return (
                    <div
                      key={focusCellId}
                    ...
                    >
                      <button
                        ...
                        onContextMenu={(event) => {
                          openPairedReadingMenuAtPointer(
                            event,
                            buildPairedReadingLayerHeaderMenuItems(sourceItemLayer, sourceRowTitle),
                          );
                        }}
                      >
                        {renderPairedReadingRailLaneBody({
                      ...
                      <TimelineLaneDraftEditorCell
                        ...
                        wrapperClassName={[
                          ...
                          hasSourceBadge
                            ? 'timeline-paired-reading-source-surface-has-self-certainty'
                            : '',
                          hasSourceBadge
                            ? 'timeline-paired-reading-source-surface-has-side-badges'
                            : '',
                        ]
                          .filter(Boolean)
                          .join(' ')}
                        ...
                        onBlur={(event) => {
                          ...
                          if (!sourceLayerId) return;
                          clearAutoSaveTimer(
                            timelinePairedReadingSourceDraftAutoSaveKey(sourceLayerId, item.unitId),
                          );
                          if (value !== initialSourceText) {
                            void runPairedReadingSaveWithStatus(sourceCellKey, async () => {
                              await persistSourceText(item.unitId, value, sourceLayerId);
                        ...
                        onKeyDown={(event) => {
                          ...
                          setUnitDrafts((prev) => ({
                            ...prev,
                            [sourceDraftKey]: initialSourceText,
                          }));
                          setPairedReadingCellSaveStatus(sourceCellKey);
                        ...
                        onContextMenu={(event) => {
                          ...
                          handleAnnotationContextMenu(
                            item.unitId,
                            unitToView(unitDoc, sourceLayerId),
                            sourceLayerId,
                            event,
                          );
                        }}
                ...
                groupTranslationLayers.map((tLayer) => {
                  ...
                  const layerTargetItems: PairedReadingTargetItem[] = shouldReuseGroupTargetItems
                    ...
                    : (() => {
                        ...
                        return buildVerticalReadingTargetItemsFromRawText(
                          primarySourceUnit.id,
                          plain,
                        );
                      })();
                  ...
                  const layerNoteIndicator = anchorForUi.unitId
                    ...
                    : null;
                  const canRecordLayerAudio =
                    Boolean(startRecordingForUnit) &&
                    (tLayer.acceptsAudio === true ||
                      tLayer.modality === 'mixed' ||
                      tLayer.modality === 'audio');
                  ...
                  const buildTranslationAudioControlsForAnchor = (
                    ...
                    const isRowRecording =
                      recording &&
                      recordingUnitId === audioScopeId &&
                      recordingLayerId === tLayer.id;
                    ...
                    return (
                      <TimelineTranslationAudioControls
                        ...
                        {...(media && deleteVoiceTranslation
                          ? {
                              onDeleteRecording: () => {
                                void deleteVoiceTranslation(voiceDoc, tLayer);
                              },
                            }
                          : {})}
                  ...
                  const pairedReadingAutoSaveKey =
                    timelinePairedReadingTargetMergedDraftAutoSaveKey(tLayer.id, group.id);
                    ...
                    <div key={tLayer.id} className="timeline-paired-reading-target-layer-stack">
                      {layerPerSeg
                        ? layerTargetItems.map((targetItem, ti) => {
                            ...
                            const itemInitial = normalizePairedReadingPlainText(
                              targetItem.text || '',
                            );
                            ...
                            const isItemDraftEmpty =
                              normalizePairedReadingPlainText(itemDraft).trim().length === 0;
                            const isThisTargetRowActive =
                              isGroupActive &&
                              pairedReadingTargetSide === 'target' &&
                              activeVerticalReadingCellId ===
                                `target:${group.id}:${tLayer.id}:${targetItem.id}`;
                            const segAutoSaveKey =
                              timelinePairedReadingTargetSegmentDraftAutoSaveKey(
                                tLayer.id,
                                group.id,
                                targetItem.id,
                              ...
                              layerTargetItems
                                .map((item) => {
                                  if (item.id === targetItem.id)
                                    return normalizePairedReadingPlainText(nextValue);
                                  const otherDraftKey = `${layerDraftKeyBase}:${item.id}`;
                                  return normalizePairedReadingPlainText(
                                    translationDrafts[otherDraftKey] ?? item.text ?? '',
                                  );
                                })
                                .join('\n');
                            const rowSegmentId = targetItem.translationSegmentId?.trim();
                            const rowAnchorSegForAudio = rowSegmentId
                              ? (translationSegmentsForAudio.find((s) => s.id === rowSegmentId) ??
                                audioAnchorSeg)
                              : audioAnchorSeg;
                            return (
                              <div
                                key={`${tLayer.id}-${targetItem.id}`}
                                className="timeline-paired-reading-editor-row timeline-paired-reading-editor-row-target"
                              >
                                <button
                                  type="button"
                                ...
                                {isAudioOnlyTranslationLayer ? (
                                  <div
                                    className={[
                                      ...
                                      isThisTargetRowActive
                                        ? 'timeline-paired-reading-target-surface-active'
                                        : '',
                                      layerNoteIndicator
                                        ? 'timeline-paired-reading-target-surface-has-side-badges'
                                        : '',
                                    ]
                                      .filter(Boolean)
                                      .join(' ')}
                                    ...
                                    onContextMenu={(event) => {
                                      ...
                                      const menuSourceId =
                                        ...
                                        group.sourceItems.some(
                                          (si) => si.unitId === contextMenuSourceUnitId,
                                        )
                                          ? contextMenuSourceUnitId
                                          : primaryUnitId;
                                      const menuUnitDoc = menuSourceId
                                        ? unitByIdForSpeaker.get(menuSourceId)
                                        : undefined;
                                      ...
                                      handleAnnotationContextMenu(
                                        menuSourceId,
                                        unitToView(menuUnitDoc, tLayer.id),
                                        tLayer.id,
                                        event,
                                      );
                                    }}
                                  >
                                    ...
                                    <div className="timeline-translation-audio-card timeline-paired-reading-target-audio-only-inner">
                                      {buildTranslationAudioControlsForAnchor(
                                        rowAnchorSegForAudio ?? audioAnchorSeg,
                                      )}
                                    </div>
                                  </div>
                                ) : (
                                  <TimelineLaneDraftEditorCell
                                    ...
                                    wrapperClassName={[
                                      'timeline-paired-reading-target-surface',
                                      isItemDraftEmpty
                                        ? 'timeline-paired-reading-target-surface-empty'
                                        : '',
                                      isThisTargetRowActive
                                        ? 'timeline-paired-reading-target-surface-active'
                                        : '',
                                    ...
                                    inputClassName={[
                                      ...
                                      isItemDraftEmpty
                                        ? 'timeline-paired-reading-target-input-empty'
                                        : '',
                                    ]
                                      .filter(Boolean)
                                      .join(' ')}
                                    ...
                                    {...(ti === 0 && layerAudioControls
                                      ? { tools: layerAudioControls }
                                      : {})}
                                    ...
                                    onFocus={() => {
                                      patchVerticalPaneFocus({
                                        ...
                                        pairedReadingTargetSide: 'target',
                                        contextMenuSourceUnitId:
                                          anchorForUi.unitId || primaryUnitId || null,
                                      });
                                      focusedTranslationDraftKeyRef.current = itemDraftKey;
                                      onFocusLayer(tLayer.id);
                                    }}
                                    onChange={(event) => {
                                      const value = normalizePairedReadingPlainText(
                                        event.target.value,
                                      );
                                      patchVerticalPaneFocus({
                                        activeVerticalReadingGroupId: group.id,
                                      ...
                                      if (value !== itemInitial) {
                                        ...
                                        scheduleAutoSave(segAutoSaveKey, async () => {
                                          await runPairedReadingSaveWithStatus(
                                            itemCellKey,
                                            async () => {
                                              await persistPairedReadingTargetTranslation(
                                                tLayer,
                                    ...
                                    onBlur={(event) => {
                                      ...
                                      clearAutoSaveTimer(segAutoSaveKey);
                                      if (value !== itemInitial) {
                                        void runPairedReadingSaveWithStatus(
                                          itemCellKey,
                                          async () => {
                                            await persistPairedReadingTargetTranslation(
                                              tLayer,
                        ...
                        : (() => {
                            ...
                            const initialTargetText = layerTargetItems
                              .map((item) => item.text)
                              .join('\n');
                            ...
                            const isTargetDraftEmpty =
                              normalizePairedReadingPlainText(draft).trim().length === 0;
                            const isMergedTargetRowActive =
                              isGroupActive &&
                              pairedReadingTargetSide === 'target' &&
                              activeVerticalReadingCellId ===
                                `target:${group.id}:${tLayer.id}:editor`;
                            return (
                              <div className="timeline-paired-reading-editor-row timeline-paired-reading-editor-row-target">
                                ...
                                {isAudioOnlyTranslationLayer ? (
                                  ...
                                  <TimelineLaneDraftEditorCell
                                    ...
                                    {...(layerTypography.dir ? { dir: layerTypography.dir } : {})}
                                    inputStyle={layerTypography.style}
                                    {...(targetSaveStatus !== undefined
                                      ? { saveStatus: targetSaveStatus }
                                      : {})}
                                    overlay={renderPairedReadingOverlay({
                                      locale,
                                      certainty: undefined,
