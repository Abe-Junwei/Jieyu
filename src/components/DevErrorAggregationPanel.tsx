...
import {
  getDevErrorAggregationPanelMessages,
...
function formatEntryLabel(
  entry: ErrorAggregationEntry,
  messages: DevErrorAggregationPanelMessages,
): string {
...
export function DevErrorAggregationPanel() {
  ...
  const [entries, setEntries] = useState<ErrorAggregationEntry[]>(() =>
    getStructuredErrorAggregation(),
  );
  ...
  return (
    <details className="dev-error-aggregation-panel" dir={uiTextDirection} {...panelStyleProps}>
      ...
      <div className="dev-error-aggregation-panel-list">
        {entries.slice(0, 20).map((entry) => (
          <div
            key={`${entry.category}:${entry.action}:${entry.i18nKey ?? ''}:${entry.recoverable}`}
          >
            <strong>{entry.count}x</strong> {formatEntryLabel(entry, messages)}
          </div>
        ))}
      </div>
    </details>
