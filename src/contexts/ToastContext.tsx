  ...
  const rawMessage =
    translatedMessage !== undefined && translatedMessage.length > 0
      ...
      : state.message !== undefined && state.message.length > 0
        ? state.message
        : t(locale, 'transcription.toast.saveFailed');
    ...
    (state: SaveState) => {
      ...
      const message =
        state.kind === 'done'
          ? state.message !== undefined && state.message.length > 0
            ? state.message
            : t(locale, 'transcription.toast.saved')
          : state.kind === 'error'
            ? state.message !== undefined && state.message.length > 0
              ? state.message
              : t(locale, 'transcription.toast.saveFailed')
    ...
    <ToastContext.Provider value={value}>
      ...
      {current && (
        <div className="transcription-toast-container" role="status" aria-live="polite">
          <div
            className={`transcription-toast toast-${current.variant} ${current.exiting === true ? 'transcription-toast-exiting' : ''}`}
            onClick={dismiss}
