...
function makeUnit(
  overrides: Partial<TimelineUnitView> &
    Pick<
      TimelineUnitView,
      'id' | 'kind' | 'mediaId' | 'layerId' | 'startTime' | 'endTime' | 'text'
    >,
): TimelineUnitView {
  return {
    id: overrides.id,
    ...
    text: overrides.text,
    ...(typeof overrides.parentUnitId === 'string' && overrides.parentUnitId.length > 0
      ? { parentUnitId: overrides.parentUnitId }
      : {}),
    ...(typeof overrides.textId === 'string' && overrides.textId.length > 0
      ? { textId: overrides.textId }
      : {}),
    ...(typeof overrides.speakerId === 'string' && overrides.speakerId.length > 0
...
describe('transcriptionAiSelectionResolver', () => {
  it('builds deduped owner unit candidates from the read model', () => {
    ...
    const getUnitDocById = vi.fn((id: string) =>
      id === 'utt-1' ? makeUnitDoc('utt-1', 'media-1') : undefined,
    );
  ...
  it('falls back from direct selection to segment owner resolution', () => {
    const directUnit = makeUnit({
      ...
      endTime: 6,
      text: 'shadow',
      parentUnitId: 'utt-owner',
    });
    const candidates = [makeUnitDoc('utt-owner', 'media-1'), makeUnitDoc('utt-other', 'media-1')];
