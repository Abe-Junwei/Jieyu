...
const { mockBridgeTextForLayerTarget, mockResolveFallbackSourceOrthographyId } = vi.hoisted(() => ({
...
function createBaseInput(overrides: Partial<HookInput> = {}): HookInput {
  ...
  return {
    ...
    aiPanelMode: 'auto',
    setAiPanelMode: vi.fn() as unknown as Dispatch<
      SetStateAction<NonNullable<AiPanelContextValue['aiPanelMode']>>
    >,
...
describe('useTranscriptionAssistantController', () => {
      ...
      useTranscriptionAssistantController(
        createBaseInput({
          ...
          layers: [
            ...
            {
              ...makeLayer('layer-tr', 'translation'),
  ...
  it('publishes AI panel context with derived ready-state stats', async () => {
    ...
    await waitFor(() => {
      expect(setAiPanelContext).toHaveBeenCalledWith(
      ...
      useTranscriptionAssistantController(
        createBaseInput({
          ...
          layers: [
            { ...makeLayer('layer-main'), orthographyId: 'orth-source' } as LayerDocType,
            {
              ...makeLayer('layer-seg', 'translation'),
              orthographyId: 'orth-target',
            } as LayerDocType,
          ],
  ...
  it('persists dictation to host child translation layer when selected/default layers are not provided', async () => {
    ...
    const layerLinks = [
      ...
      },
      {
        id: 'link-fr',
        layerId: 'layer-tr-fr',
        transcriptionLayerKey: 'layer-fr',
        hostTranscriptionLayerId: 'layer-fr',
        isPreferred: true,
        createdAt: '2026-03-30T00:00:00.000Z',
      },
    ...
    const { result } = renderHook(() =>
      useTranscriptionAssistantController(hookInputWithoutDefaultTranscription),
    );
  ...
  it('builds a continuous dictation pipeline for unit-based voice dictation', async () => {
    ...
    const getUnitTextForLayer = vi.fn((unit: LayerUnitDocType) =>
      unit.id === 'utt-1' ? '' : '已有内容',
    );
    const saveUnitText = vi.fn(async () => undefined);
    const selectUnit = vi.fn();
    ...
    await result.current.voiceDictationPipeline?.callbacks.fillSegment(
      'utt-1',
      'transcription',
      'voice text',
    );
    ...
    await result.current.voiceDictationPipeline?.callbacks.navigateTo('utt-2');
    expect(selectUnit).toHaveBeenCalledWith('utt-2');
  });
  ...
  it('skips LLM voice intent resolution when AI chat is disabled', async () => {
    ...
    await expect(
      result.current.handleResolveVoiceIntentWithLlm({
        text: '播放',
