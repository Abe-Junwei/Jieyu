...
describe('OrthographyManagerPage', () => {
  beforeEach(() => {
    ...
    mockListLanguageCatalogEntries.mockImplementation(
      async ({ locale }: { locale: 'zh-CN' | 'en-US' }) => [
        {
          id: 'eng',
          entryKind: 'built-in',
    ...
    mockUpdateOrthography.mockImplementation(async (input: Record<string, unknown>) => ({
      ...
      ...((input.exemplarCharacters as OrthographyDocType['exemplarCharacters'] | undefined)
        ? {
            exemplarCharacters:
              input.exemplarCharacters as OrthographyDocType['exemplarCharacters'],
          }
        : {}),
      ...
      catalogMetadata: (input.catalogMetadata as
        | OrthographyDocType['catalogMetadata']
        | undefined) ?? {
        catalogSource: 'built-in-reviewed',
        reviewStatus: 'verified-primary',
        priority: 'primary',
  ...
  it('loads the orthography manager, selects the requested orthography, and registers side-pane summary', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText('Bridge Orthography · Latn · practical').length).toBeGreaterThan(
        0,
      );
    });
    ...
    await waitFor(() => {
      ...
      bridgeLinks.forEach((link) => {
        expect(link.getAttribute('href')).toBe(
          '/assets/orthography-bridges?targetOrthographyId=orth-source',
        );
      });
    });
    expect(screen.getByTestId('side-pane-title').textContent).toBe('正字法管理器');
    await waitFor(() => {
      expect(screen.getByTestId('side-pane-content').textContent).toContain(
        'Bridge Orthography · Latn · practical',
      );
    });
  });
  ...
  it('saves edited orthography metadata from the workspace panel', async () => {
    ...
    fireEvent.change(screen.getByRole('combobox', { name: '审校状态' }), {
      target: { value: 'verified-secondary' },
    });
    fireEvent.change(screen.getByRole('combobox', { name: '目录优先级' }), {
      target: { value: 'secondary' },
    });
    fireEvent.click(screen.getAllByRole('button', { name: '保存元数据' })[0]!);

    await waitFor(() => {
      expect(mockUpdateOrthography).toHaveBeenCalledWith(
    ...
    expect(
      screen.getAllByText('Updated Bridge Orthography · Latn · practical').length,
    ).toBeGreaterThan(0);
  ...
  it('commits a new language after the previous name has been cleared and replaced', async () => {
    ...
    await waitFor(() => {
      ...
      const lastCall = (
        lastCallIndex >= 0 ? mockUpdateOrthography.mock.calls[lastCallIndex]?.[0] : undefined
  ...
  it('saves additional localized name labels from the workspace panel', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('例如 fra、deu、zh-Hant'), {
      target: { value: 'fra' },
    });
    fireEvent.change(screen.getByPlaceholderText('输入该标签对应的名称'), {
      target: { value: 'Orthographe passerelle' },
    });
    fireEvent.click(screen.getAllByRole('button', { name: '保存元数据' })[0]!);
  ...
  it('keeps unsaved draft edits when the locale changes and only relocalizes the language display', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByDisplayValue('Dirty Bridge Orthography')).toBeTruthy();
      expect((screen.getByRole('combobox', { name: 'Language' }) as HTMLInputElement).value).toBe(
        'English',
      );
    });
  ...
  it('shows custom language asset id separately from language code and saves the asset id', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        (screen.getAllByRole('button', { name: '保存元数据' })[0] as HTMLButtonElement).disabled,
      ).toBe(false);
    });
  ...
  it('shows sidebar entry context and blocks orthography switching while the draft is dirty until confirmed', async () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter
          initialEntries={[
            '/assets/orthographies?orthographyId=orth-source&fromLayerId=layer_trc_bridge',
          ]}
        >
          <LocaleProvider locale="zh-CN">
            <AppSidePaneProvider>
    ...
    expect(
      await screen.findByText('当前入口来自转写侧栏；桥接规则维护已迁移到独立的正字法桥接工作台。'),
    ).toBeTruthy();
    await waitFor(() => {
      expect(screen.getByTestId('side-pane-content').textContent).toContain(
        'Bridge Orthography · Latn · practical',
    ...
    bridgeLinks.forEach((link) => {
      expect(link.getAttribute('href')).toBe(
        '/assets/orthography-bridges?targetOrthographyId=orth-source&fromLayerId=layer_trc_bridge',
      );
    });

    fireEvent.change(await screen.findByLabelText('英文名称'), {
      target: { value: 'Dirty Bridge Orthography' },
    });
  ...
  it('blocks bridge workspace navigation while the draft is dirty until confirmed', async () => {
    ...
    const detailBridgeLink = screen
      .getAllByRole('link', { name: '打开正字法桥接工作台' })
      .find((link) => {
        const href = link.getAttribute('href');
        return typeof href === 'string' && href.includes('targetOrthographyId=orth-source');
      });
