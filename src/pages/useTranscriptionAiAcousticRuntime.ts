...
import {
  ...
  type AcousticPromptSummary,
} from './transcriptionAcousticSummary';
import {
...
export function useTranscriptionAiAcousticRuntime(
  ...
  const [acousticRuntimeStatus, setAcousticRuntimeStatus] = useState<AcousticRuntimeStatus>({
    state: 'idle',
  });
  ...
  useEffect(() => {
    ...
    const configPartial =
      input.configOverride && Object.keys(input.configOverride).length > 0
    ...
    service
      .analyzeMedia({
        ...
        ...(input.providerPreference !== null &&
        input.providerPreference !== undefined &&
        input.providerPreference.length > 0
          ? { providerId: input.providerPreference }
          : {}),
        ...
        onProgress: (progress) => {
          ...
          const ratioDeltaReached =
            progress.ratio - previous.ratio >= PROGRESS_UPDATE_MIN_RATIO_DELTA;
      ...
      .then((result) => {
        ...
        setAcousticRuntimeStatus((previous) => ({
          ...
          phase: 'done',
          progressRatio: 1,
          ...(typeof previous.processedFrames === 'number'
            ? { processedFrames: previous.processedFrames }
            : {}),
          ...(typeof previous.totalFrames === 'number'
    ...
    input.selectedTimelineMediaId,
    input.configOverride,
    input.providerPreference,
    service,
  ]);

  const acousticSummary = useMemo(
    () =>
      buildAcousticPromptSummary(acousticAnalysis, input.selectionStartSec, input.selectionEndSec),
    [acousticAnalysis, input.selectionEndSec, input.selectionStartSec],
  );

  const acousticDetail = useMemo(
    () =>
      buildAcousticPanelDetail(acousticAnalysis, input.selectionStartSec, input.selectionEndSec),
    [acousticAnalysis, input.selectionEndSec, input.selectionStartSec],
  );
  ...
  const acousticBatchBuildResult = useMemo(
    () => buildAcousticPanelBatchBuildResult(acousticAnalysis, input.batchSelectionRanges),
