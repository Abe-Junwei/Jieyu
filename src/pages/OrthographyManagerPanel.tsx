...
export function OrthographyManagerPanel({
  ...
  const activeSearchSuggestionId =
  ...
  return (
      ...
      footer={
        selectedOrthography ? (
          <>
            ...
            >
              {saving
                ? t(locale, 'workspace.orthography.saving')
                : t(locale, 'workspace.orthography.saveButton')}
            </PanelButton>
          </>
        ) : null
      }
    >
      {fromLayerId ? (
        <p className="om-context-note orthography-builder-workspace-note">
          {t(locale, 'workspace.orthography.fromLayerHint')}
        </p>
      ) : null}
      ...
      {selectedOrthography ? (
        <>
          <section
            className="ws-summary-card om-summary la-panel-section"
            aria-labelledby="om-summary-title"
          >
            <div className="ws-summary-header">
              <div className="ws-summary-copy">
                <h2 id="om-summary-title" className="ws-summary-title">
            ...
            <div className="ws-summary-facts">
              <article className="ws-summary-fact">
                <span className="ws-summary-fact-label">
                  {t(locale, 'workspace.orthography.languageAssetIdLabel')}
                </span>
                <strong className="ws-summary-fact-value">
                  {selectedOrthography.languageId ?? t(locale, 'workspace.orthography.notSet')}
                </strong>
              ...
              <article className="ws-summary-fact">
                ...
                <strong className="ws-summary-fact-value">
                  {draft?.scriptTag ||
                    selectedOrthography.scriptTag ||
                    t(locale, 'workspace.orthography.notSet')}
                </strong>
          ...
          {isDirty ? (
            <PanelNote className="om-state om-state-warning">
              {t(locale, 'workspace.orthography.unsavedHint')}
            </PanelNote>
          ) : null}
        </>
      ) : null}
      ...
      <section
        className="om-browser la-panel-section"
        aria-label={t(locale, 'workspace.orthography.listTitle')}
      >
        <div className="om-browser-header">
          <h3 className="om-browser-title">{t(locale, 'workspace.orthography.listTitle')}</h3>
          <p className="panel-section__description">
            {t(locale, 'workspace.orthography.listDescription')}
          </p>
        </div>
        <div className="om-toolbar orthography-builder-group-body">
            ...
            <input
              ...
              aria-activedescendant={
                hasVisibleSearchSuggestions
                  ...
                  : activeIndex >= 0
                    ? `om-item-${filteredOrthographies[activeIndex]?.id ?? ''}`
                    : undefined
              }
            />
            <div
            ...
            >
              {hasVisibleSearchSuggestions
                ? searchSuggestions.map((suggestion, index) => (
                    ...
                    >
                      <span className="om-search-suggestion-label">
                        {formatLanguageCatalogSearchSuggestion(suggestion, locale)}
                      </span>
                    </div>
                  ))
                : null}
          ...
          {projectLanguageIds.length > 0 ? (
            <div
              className="om-filter-toggle"
              role="radiogroup"
              aria-label={t(locale, 'workspace.orthography.filterProjectOnly')}
            >
              <PanelButton
                role="radio"
        ...
        {!projectLanguageIds.length && showUnscopedIdleState ? (
          <div className="om-callout orthography-builder-hint">
            <PanelNote className="om-state om-state-warning">
              {t(locale, 'workspace.orthography.unscopedPrompt')}
            </PanelNote>
        ...
        {loading ? (
          <PanelNote className="om-state">{t(locale, 'workspace.orthography.loading')}</PanelNote>
        ) : null}
        {!loading && error ? (
          <PanelNote variant="danger" className="om-state om-state-error">
            {t(locale, 'workspace.orthography.errorPrefix').replace('{message}', error)}
          </PanelNote>
        ) : null}
        {!loading &&
        !error &&
        ...
        filteredOrthographies.length === 0 ? (
          <PanelNote className="om-state om-state-warning">
            {t(locale, 'workspace.orthography.searchNoResults')}
          </PanelNote>
        ) : null}
        {!loading &&
        !error &&
        ...
        <div
          className="om-list la-list-scroll"
          role="list"
          ref={listRef}
          aria-label={t(locale, 'workspace.orthography.listTitle')}
        >
          ...
          {draft ? (
            <div className="om-form-stack orthography-builder-panel orthography-builder-panel-compact">
              <section className="om-subsection orthography-builder-group orthography-builder-group-divided">
                <h3 className="panel-title-primary om-subsection-title orthography-builder-group-title">
                  {builderMessages.identitySectionTitle}
                </h3>
                <div className="om-form-grid">
                  ...
                  <div className="orthography-builder-script-type-row om-form-pair om-form-span-two-thirds">
                    ...
                    <FormField label={builderMessages.typeLabel}>
                      <select
                        ...
                        value={draft.type}
                        onChange={(event) =>
                          onDraftChange('type', event.target.value as OrthographyDraft['type'])
                        }
                      >
                  ...
                  <FormField
                    label={builderMessages.advancedDirectionLabel}
                    className="om-form-span-third"
                  >
                    <select
                      className="input panel-input"
                      ...
                      onChange={(event) =>
                        onDraftChange(
                          'direction',
                          event.target.value as OrthographyDraft['direction'],
                        )
                      }
                    >
                  ...
                  <FormField label={builderMessages.nameZhLabel} className="om-form-span-half">
                    <input
                      ...
                      onChange={(event) => onDraftChange('namePrimary', event.target.value)}
                      placeholder={builderMessages.nameZhPlaceholder}
                    />
                  </FormField>
                  <FormField label={builderMessages.nameEnLabel} className="om-form-span-half">
                    <input
                      className="input panel-input"
                      type="text"
                      value={draft.nameEnglishFallback}
                      onChange={(event) => onDraftChange('nameEnglishFallback', event.target.value)}
                      placeholder={builderMessages.nameEnPlaceholder}
                    />
                  </FormField>
                  <FormField
                    label={builderMessages.abbreviationLabel}
                    className="om-form-span-third"
                  >
                    <input
                      className="input panel-input"
                      ...
                      onChange={(event) => onDraftChange('abbreviation', event.target.value)}
                      placeholder={builderMessages.abbreviationPlaceholder}
                    />
                  </FormField>
                  <FormField
                    label={builderMessages.advancedLocaleLabel}
                    className="om-form-span-third"
                  >
                    <input
                      className="input panel-input"
                      ...
                      onChange={(event) => onDraftChange('localeTag', event.target.value)}
                      placeholder={builderMessages.localePlaceholder}
                    />
                  </FormField>
                  <FormField
                    label={builderMessages.advancedRegionLabel}
                    className="om-form-span-third"
                  >
                    <input
                      className="input panel-input"
                      ...
                      onChange={(event) => onDraftChange('regionTag', event.target.value)}
                      placeholder={builderMessages.regionPlaceholder}
                    />
                  </FormField>
                  <FormField
                    label={builderMessages.advancedVariantLabel}
                    className="om-form-span-half"
                  >
                    <input
                      className="input panel-input"
                      type="text"
                      value={draft.variantTag}
                      onChange={(event) => onDraftChange('variantTag', event.target.value)}
                      placeholder={builderMessages.variantPlaceholder}
                    />
                  </FormField>
                </div>
              </section>
              ...
              <section className="om-subsection orthography-builder-group orthography-builder-group-divided">
                <h3 className="panel-title-primary om-subsection-title orthography-builder-group-title">
                  {builderMessages.renderSectionTitle}
                </h3>
                <div className="om-form-grid">
                  <FormField label={builderMessages.primaryFontLabel} className="om-form-span-half">
                    <input
                      className="input panel-input"
                      type="text"
                      value={draft.primaryFonts}
                      onChange={(event) => onDraftChange('primaryFonts', event.target.value)}
                      placeholder={builderMessages.primaryFontPlaceholder}
                    />
                  </FormField>
                  <FormField
                    label={builderMessages.fallbackFontLabel}
                    className="om-form-span-half"
                  >
                    <input
                      className="input panel-input"
                      type="text"
                      value={draft.fallbackFonts}
                      onChange={(event) => onDraftChange('fallbackFonts', event.target.value)}
                      placeholder={builderMessages.fallbackFontPlaceholder}
                    />
                  </FormField>
                  <FormField
                    label={t(locale, 'workspace.orthography.monoFontLabel')}
                    className="om-form-span-third"
                  >
                    <input
                      className="input panel-input"
                      type="text"
                      value={draft.monoFonts}
                      onChange={(event) => onDraftChange('monoFonts', event.target.value)}
                      placeholder={builderMessages.fallbackFontPlaceholder}
                    />
                  </FormField>
                  <FormField
                    label={t(locale, 'workspace.orthography.lineHeightScaleLabel')}
                    className="om-form-span-third"
                  >
                    <input
                      className="input panel-input"
                      type="text"
                      value={draft.lineHeightScale}
                      onChange={(event) => onDraftChange('lineHeightScale', event.target.value)}
                    />
                  </FormField>
                  <FormField
                    label={t(locale, 'workspace.orthography.sizeAdjustLabel')}
                    className="om-form-span-third"
                  >
                    <input
                      className="input panel-input"
                      type="text"
                      value={draft.sizeAdjust}
                      onChange={(event) => onDraftChange('sizeAdjust', event.target.value)}
                    />
                  </FormField>
                </div>
                <div className="om-checkbox-row">
                  <label className="orthography-builder-checkbox">
                    <input
                      type="checkbox"
                      checked={draft.bidiIsolate}
                      onChange={(event) => onDraftChange('bidiIsolate', event.target.checked)}
                    />
                  ...
                  <label className="orthography-builder-checkbox">
                    <input
                      type="checkbox"
                      checked={draft.preferDirAttribute}
                      onChange={(event) =>
                        onDraftChange('preferDirAttribute', event.target.checked)
                      }
                    />
              ...
              <section className="om-subsection orthography-builder-group orthography-builder-group-divided">
                <h3 className="panel-title-primary om-subsection-title orthography-builder-group-title">
                  {builderMessages.localizedNamesSectionTitle}
                </h3>
                <div className="om-array-list">
                  {draft.localizedNameEntries.map((entry, index) => (
                    <div key={`${entry.languageTag}-${index}`} className="om-array-row">
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            onDraftChange(
                              ...
                              draft.localizedNameEntries.map((current, currentIndex) =>
                                currentIndex === index
                                  ? { ...current, languageTag: event.target.value }
                                  : current,
                              ),
                            )
                          }
                      ...
                      <FormField
                        label={t(locale, 'workspace.orthography.localizedNameValueLabel')}
                        className="om-array-value"
                      >
                        <input
                          className="input panel-input"
                          type="text"
                      ...
                      <PanelButton
                        ...
                        onClick={() =>
                          onDraftChange(
                            'localizedNameEntries',
                            draft.localizedNameEntries.filter(
                ...
                <PanelButton
                  ...
                  onClick={() =>
                    onDraftChange('localizedNameEntries', [
                      ...draft.localizedNameEntries,
                      { languageTag: '', label: '' },
                    ])
                  }
                >
                ...
                <h3 className="panel-title-primary om-subsection-title om-subsection-heading-follow orthography-builder-group-title">
                  {builderMessages.examplesSectionTitle}
                </h3>
                <div className="om-form-grid">
                  <FormField label={builderMessages.exemplarLabel} className="om-form-span-half">
                    <textarea
                      className="input panel-input om-textarea"
                      value={draft.exemplarMain}
                      onChange={(event) => onDraftChange('exemplarMain', event.target.value)}
                      placeholder={builderMessages.exemplarPlaceholder}
                    />
                  </FormField>
                  <FormField
                    label={t(locale, 'workspace.orthography.auxiliaryExemplarLabel')}
                    className="om-form-span-half"
                  >
                    <textarea
                      className="input panel-input om-textarea"
                      value={draft.exemplarAuxiliary}
                      onChange={(event) => onDraftChange('exemplarAuxiliary', event.target.value)}
                      placeholder={builderMessages.exemplarPlaceholder}
                    />
                  </FormField>
                  <FormField
                    label={t(locale, 'workspace.orthography.numberExemplarLabel')}
                    className="om-form-span-third"
                  >
                    <input
                      className="input panel-input"
                      type="text"
                      value={draft.exemplarNumbers}
                      onChange={(event) => onDraftChange('exemplarNumbers', event.target.value)}
                      placeholder={builderMessages.exemplarPlaceholder}
                    />
                  </FormField>
                  <FormField
                    label={t(locale, 'workspace.orthography.punctuationExemplarLabel')}
                    className="om-form-span-third"
                  >
                    <input
                      className="input panel-input"
                      type="text"
                      value={draft.exemplarPunctuation}
                      onChange={(event) => onDraftChange('exemplarPunctuation', event.target.value)}
                      placeholder={builderMessages.exemplarPlaceholder}
                    />
                  </FormField>
                  <FormField
                    label={t(locale, 'workspace.orthography.indexExemplarLabel')}
                    className="om-form-span-third"
                  >
                    <input
                      className="input panel-input"
                      type="text"
                      value={draft.exemplarIndex}
                      onChange={(event) => onDraftChange('exemplarIndex', event.target.value)}
                      placeholder={builderMessages.exemplarPlaceholder}
                    />
                  </FormField>
                </div>
              ...
              <details className="om-advanced orthography-builder-advanced-group">
                <summary className="om-advanced-summary">
                  <span className="panel-title-secondary orthography-builder-group-title">
                    {builderMessages.advancedSectionTitle}
                  </span>
                  <span className="om-advanced-meta">
                    {draft.catalogReviewStatus
                      ? resolveCatalogReviewStatusLabel(locale, draft.catalogReviewStatus)
                      : null}
                    {draft.catalogPriority
                      ? ` · ${resolveCatalogPriorityLabel(locale, draft.catalogPriority)}`
                      : null}
                  </span>
                </summary>
                ...
                <div className="om-advanced-body orthography-builder-advanced-panel">
                  <div className="om-form-grid">
                    <FormField
                      label={t(locale, 'workspace.orthography.catalogReviewStatusLabel')}
                      className="om-form-span-half"
                    >
                      <select
                        className="input panel-input"
                        value={draft.catalogReviewStatus}
                        onChange={(event) =>
                          onDraftChange(
                            'catalogReviewStatus',
                            event.target.value as OrthographyDraft['catalogReviewStatus'],
                          )
                        }
                      >
                        <option value="">{t(locale, 'workspace.orthography.notSet')}</option>
                        <option value="needs-review">
                          {resolveCatalogReviewStatusLabel(locale, 'needs-review')}
                        </option>
                        <option value="verified-primary">
                          {resolveCatalogReviewStatusLabel(locale, 'verified-primary')}
                        </option>
                        <option value="verified-secondary">
                          {resolveCatalogReviewStatusLabel(locale, 'verified-secondary')}
                        </option>
                        <option value="historical">
                          {resolveCatalogReviewStatusLabel(locale, 'historical')}
                        </option>
                        <option value="legacy">
                          {resolveCatalogReviewStatusLabel(locale, 'legacy')}
                        </option>
                        <option value="experimental">
                          {resolveCatalogReviewStatusLabel(locale, 'experimental')}
                        </option>
                      </select>
                    </FormField>
                    <FormField
                      label={t(locale, 'workspace.orthography.catalogPriorityLabel')}
                      className="om-form-span-half"
                    >
                      <select
                        className="input panel-input"
                        value={draft.catalogPriority}
                        onChange={(event) =>
                          onDraftChange(
                            'catalogPriority',
                            event.target.value as OrthographyDraft['catalogPriority'],
                          )
                        }
                      >
                        <option value="">{t(locale, 'workspace.orthography.notSet')}</option>
                        <option value="primary">
                          {resolveCatalogPriorityLabel(locale, 'primary')}
                        </option>
                        <option value="secondary">
                          {resolveCatalogPriorityLabel(locale, 'secondary')}
                        </option>
                      </select>
                    </FormField>
                    <div className="om-basic-grid om-form-span-2">
                      <div>
                        <dt>{t(locale, 'workspace.orthography.catalogSourceLabel')}</dt>
                        <dd>
                          {resolveCatalogSourceLabel(
                            locale,
                            selectedOrthography.catalogMetadata?.catalogSource,
                          )}
                        </dd>
                      </div>
                      <div>
                        <dt>{t(locale, 'workspace.orthography.catalogLabel')}</dt>
                        <dd>{selectedBadgeLabel ?? t(locale, 'workspace.orthography.notSet')}</dd>
                      </div>
                      <div>
                        <dt>{t(locale, 'workspace.orthography.updatedAtLabel')}</dt>
                        <dd>
                          {selectedOrthography.updatedAt ??
                            t(locale, 'workspace.orthography.notSet')}
                        </dd>
                      </div>
                      <div>
                        <dt>{t(locale, 'workspace.orthography.catalogSourceNoteLabel')}</dt>
                        <dd>
                          {selectedOrthography.catalogMetadata?.source ??
                            t(locale, 'workspace.orthography.notSet')}
                        </dd>
                      </div>
                    </div>
                  ...
                  <div className="om-form-grid">
                    <FormField
                      label={t(locale, 'workspace.orthography.keyboardLayoutLabel')}
                      className="om-form-span-half"
                    >
                      <input
                        className="input panel-input"
                        ...
                        onChange={(event) => onDraftChange('keyboardLayout', event.target.value)}
                        placeholder={t(locale, 'workspace.orthography.keyboardLayoutPlaceholder')}
                      />
                    </FormField>
                    <FormField
                      label={t(locale, 'workspace.orthography.imeIdLabel')}
                      className="om-form-span-half"
                    >
                      <input
                        className="input panel-input"
                        type="text"
                        value={draft.imeId}
                        onChange={(event) => onDraftChange('imeId', event.target.value)}
                        placeholder={t(locale, 'workspace.orthography.imeIdPlaceholder')}
                      />
                    </FormField>
                    <FormField
                      label={t(locale, 'workspace.orthography.deadKeysLabel')}
                      className="om-form-span-2"
                    >
                      <textarea
                        className="input panel-input om-textarea"
                        value={draft.deadKeys}
                        onChange={(event) => onDraftChange('deadKeys', event.target.value)}
                        placeholder={t(locale, 'workspace.orthography.deadKeysPlaceholder')}
                      />
                    </FormField>
                  </div>
                  ...
                  <div className="om-form-grid">
                    <FormField
                      label={t(locale, 'workspace.orthography.normalizationFormLabel')}
                      className="om-form-span-half"
                    >
                      <select
                        className="input panel-input"
                        value={draft.normalizationForm}
                        onChange={(event) =>
                          onDraftChange(
                            'normalizationForm',
                            event.target.value as OrthographyDraft['normalizationForm'],
                          )
                        }
                      >
                        <option value="">{t(locale, 'workspace.orthography.notSet')}</option>
                    ...
                    <FormField
                      label={t(locale, 'workspace.orthography.collationBaseLabel')}
                      className="om-form-span-half"
                    >
                      <input
                        className="input panel-input"
                        type="text"
                        value={draft.collationBase}
                        onChange={(event) => onDraftChange('collationBase', event.target.value)}
                        placeholder={t(locale, 'workspace.orthography.collationBasePlaceholder')}
                      />
                    </FormField>
                  </div>
                  <div className="om-checkbox-row">
                    <label className="orthography-builder-checkbox">
                      <input
                        type="checkbox"
                        checked={draft.normalizationCaseSensitive}
                        onChange={(event) =>
                          onDraftChange('normalizationCaseSensitive', event.target.checked)
                        }
                      />
                    ...
                    <label className="orthography-builder-checkbox">
                      <input
                        type="checkbox"
                        checked={draft.normalizationStripDefaultIgnorables}
                        onChange={(event) =>
                          onDraftChange('normalizationStripDefaultIgnorables', event.target.checked)
                        }
                      />
                  ...
                  <div className="om-form-grid">
                    <FormField
                      label={t(locale, 'workspace.orthography.collationRulesLabel')}
                      className="om-form-span-half"
                    >
                      <textarea
                        className="input panel-input om-textarea om-codearea"
                        value={draft.collationRules}
                        onChange={(event) => onDraftChange('collationRules', event.target.value)}
                      />
                    </FormField>
                    <FormField
                      label={t(locale, 'workspace.orthography.conversionRulesLabel')}
                      className="om-form-span-half"
                    >
                      <textarea
                        className="input panel-input om-textarea om-codearea"
                        value={draft.conversionRulesJson}
                        onChange={(event) =>
                          onDraftChange('conversionRulesJson', event.target.value)
                        }
                        placeholder={t(locale, 'workspace.orthography.conversionRulesPlaceholder')}
                      />
                    </FormField>
                  </div>
                  ...
                  <div className="om-form-grid">
                    <FormField
                      label={t(locale, 'workspace.orthography.notesZhLabel')}
                      className="om-form-span-half"
                    >
                      <textarea
                        className="input panel-input om-textarea"
                        value={draft.notesZh}
                        onChange={(event) => onDraftChange('notesZh', event.target.value)}
                      />
                    </FormField>
                    <FormField
                      label={t(locale, 'workspace.orthography.notesEnLabel')}
                      className="om-form-span-half"
                    >
                      <textarea
                        className="input panel-input om-textarea"
                        value={draft.notesEn}
                        onChange={(event) => onDraftChange('notesEn', event.target.value)}
                      />
                    </FormField>
                  </div>
                </div>
