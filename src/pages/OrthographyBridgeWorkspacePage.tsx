...
export function OrthographyBridgeWorkspacePage({
  ...
  const [shellFooterState, setShellFooterState] =
    useState<OrthographyBridgeShellFooterState | null>(null);
      ...
      try {
        ...
        setError(
          loadError instanceof Error
            ? loadError.message
            : t(locale, 'workspace.orthographyBridge.errorFallback'),
        );
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  const handleSelectForKeyboard = useCallback(
    (id: string) => {
      const nextParams = new URLSearchParams(searchParamsRef.current);
    ...
    () => (
      <div className="app-side-pane-feature-stack">
        <section
          ...
          aria-label={t(locale, 'workspace.orthographyBridge.sidePaneCurrent')}
        >
          <div
          ...
          >
            <span className="app-side-pane-section-title">
              {t(locale, 'workspace.orthographyBridge.sidePaneCurrent')}
            </span>
          </div>
            ...
            {selectedOrthography ? (
              <>
                <span className="app-side-pane-feature-badge">
                  {selectedBadge?.label ?? t(locale, 'workspace.orthographyBridge.notSet')}
                </span>
                <p className="app-side-pane-feature-summary">
                  {formatOrthographyOptionLabel(selectedOrthography, locale)}
                </p>
                <p className="app-side-pane-feature-note">
                  {t(locale, 'workspace.orthographyBridge.sidePaneSelectedHint')}
                </p>
              </>
            ) : (
              <p className="app-side-pane-feature-note">
                {t(locale, 'workspace.orthographyBridge.sidePaneEmpty')}
              </p>
            )}
          </div>
        </section>
        ...
        <section
          className="app-side-pane-group"
          aria-label={t(locale, 'workspace.orthographyBridge.sidePaneQuickAccess')}
        >
          <div
            className="app-side-pane-group-toggle app-side-pane-group-toggle-static"
            role="presentation"
          >
            <span className="app-side-pane-section-title">
              {t(locale, 'workspace.orthographyBridge.sidePaneQuickAccess')}
            </span>
          </div>
          <div className="app-side-pane-nav app-side-pane-feature-nav">
            ...
            >
              {t(locale, 'workspace.orthographyBridge.openLanguageMetadata')}
            </LanguageAssetRouteLink>
            <OrthographyPanelLink className="side-pane-nav-link app-side-pane-feature-link">
              {t(locale, 'workspace.orthographyBridge.openOrthographyManager')}
  ...
  useRegisterAppSidePane({
    ...
    subtitle: selectedOrthography
      ? selectedOrthographyLabel
      : t(locale, 'workspace.orthographyBridge.sidePaneSubtitle'),
  ...
  return (
        ...
        shellFooterState ? (
          <>
            <button
              ...
              onClick={shellFooterState.onCancel}
              disabled={shellFooterState.saving}
            >
            ...
            <button
              type="button"
              className="btn"
              onClick={shellFooterState.onSave}
              disabled={shellFooterState.saving}
            >
    ...
    >
      <section
        className="ws-summary-card ob-summary la-panel-section"
        aria-labelledby="orthography-bridge-workspace-title"
      >
        <div className="ws-summary-header">
          <div className="ws-summary-copy">
            <h2 id="orthography-bridge-workspace-title" className="ws-summary-title">
              {selectedOrthographyLabel}
            </h2>
          </div>
          <div className="ob-summary-meta">
            {selectedBadge ? (
              ...
              <span>{t(locale, 'workspace.orthographyBridge.notSet')}</span>
            )}
          </div>
        </div>
        ...
        <div className="ws-summary-facts">
          <article className="ws-summary-fact">
            <span className="ws-summary-fact-label">
              {t(locale, 'workspace.orthographyBridge.detailTitle')}
            </span>
          ...
          <article className="ws-summary-fact">
            <span className="ws-summary-fact-label">
              {t(locale, 'workspace.orthography.languageLabel')}
            </span>
            <strong className="ws-summary-fact-value">{selectedLanguageLabel}</strong>
            <span className="ws-summary-fact-note">
              {selectedOrthography?.languageId ?? t(locale, 'workspace.orthographyBridge.notSet')}
            </span>
          </article>
          <article className="ws-summary-fact">
            <span className="ws-summary-fact-label">
            ...
            <span className="ws-summary-fact-note">
              {selectedOrthography?.id ?? t(locale, 'workspace.orthographyBridge.notSet')}
            </span>
          </article>
        </div>

        <div className="ob-summary-links">
          <LanguageAssetRouteLink to="/assets/language-metadata" className="btn btn-ghost">
            {t(locale, 'workspace.orthographyBridge.openLanguageMetadata')}
          </LanguageAssetRouteLink>
          <OrthographyPanelLink className="btn btn-ghost">
            {t(locale, 'workspace.orthographyBridge.openOrthographyManager')}
          </OrthographyPanelLink>
        </div>
      </section>
      ...
      <section
        className="ob-browser la-panel-section"
        aria-label={t(locale, 'workspace.orthographyBridge.listTitle')}
      >
        <div className="ob-browser-header">
          ...
          <p className="panel-section__description">
            {t(locale, 'workspace.orthographyBridge.listDescription')}
          </p>
        </div>
            ...
            aria-activedescendant={
              hasVisibleSearchSuggestions
                ? searchSuggestionActiveIndex >= 0 &&
                  searchSuggestionActiveIndex < searchSuggestions.length
                  ? `ob-search-suggestion-${searchSuggestionActiveIndex}`
                  : undefined
                : kbActiveIndex >= 0
                  ? `ob-item-${filteredOrthographies[kbActiveIndex]?.id ?? ''}`
                  : undefined
            }
          />
          <div
          ...
          >
            {hasVisibleSearchSuggestions
              ? searchSuggestions.map((suggestion, index) => (
                  ...
                  >
                    <span className="ob-search-suggestion-label">
        ...
        {projectLanguageIds.length > 0 ? (
          <div
            className="ob-filter-toggle"
            role="radiogroup"
            aria-label={t(locale, 'workspace.orthographyBridge.filterProjectOnly')}
          >
            <button
              type="button"
              role="radio"
              aria-checked={projectOnly}
              className={`btn${projectOnly ? ' btn-active' : ''}`}
              onClick={() => setProjectOnly(true)}
            >
              {t(locale, 'workspace.orthographyBridge.filterProjectOnly')}
            </button>
            <button
              type="button"
              role="radio"
        ...
        {!projectLanguageIds.length && showUnscopedIdleState ? (
          <div className="ob-empty-callout">
            <p className="ob-state ob-state-warning">
              {t(locale, 'workspace.orthographyBridge.unscopedPrompt')}
            </p>
        ...
        {loading ? (
          <p className="ob-state">{t(locale, 'workspace.orthographyBridge.loading')}</p>
        ) : null}
        {!loading && error ? (
          <p className="ob-state ob-state-error">
            {t(locale, 'workspace.orthographyBridge.errorPrefix').replace('{message}', error)}
          </p>
        ) : null}
        {!loading &&
        !error &&
        ...
        filteredOrthographies.length === 0 ? (
          <p className="ob-state ob-state-warning">
            {t(locale, 'workspace.orthographyBridge.searchNoResults')}
          </p>
        ) : null}
        {!loading &&
        !error &&
        ...
        <div
          className="ob-list la-list-scroll"
          role="list"
          ref={kbListRef}
          aria-label={t(locale, 'workspace.orthographyBridge.listTitle')}
        >
          {filteredOrthographies.map((orthography, index) => {
            ...
            return (
              ...
              >
                <span className="ob-list-item-label">
                  {formatOrthographyOptionLabel(orthography, locale)}
                </span>
                <span className="ob-list-item-meta">
                  {orthography.languageId ?? t(locale, 'workspace.orthographyBridge.notSet')}
                </span>
              </button>
            );
          })}
      ...
      <div className="ob-detail-column la-panel-section">
        <section
          className="ob-bridge la-panel-section"
          aria-label={t(locale, 'workspace.orthographyBridge.managerTitle')}
        >
          <div className="ob-bridge-header">
            <h3 className="ob-bridge-title">
              {t(locale, 'workspace.orthographyBridge.managerTitle')}
            </h3>
            <p className="panel-section__description">
              {t(locale, 'workspace.orthographyBridge.managerDescription')}
            </p>
          </div>
