...
import {
  ...
  createVoiceDictationPipeline,
  persistVoiceDictationToUnit,
  resolveVoiceDictationTarget,
} from './voiceDictationRuntime';
...
export function useTranscriptionAssistantController(
  ...
  const aiPanelContextValue = useMemo<AiPanelContextValue>(
    () => buildTranscriptionAssistantContextValue(input),
    [input],
  );

  useEffect(() => {
    input.setAiPanelContext(aiPanelContextValue);
  }, [aiPanelContextValue, input]);

  const handleResolveVoiceIntentWithLlm = useCallback(
    async ({
      text,
      mode,
      session,
      ...
      const { resolveVoiceIntentWithLlmUsingConfig } =
        await import('../services/VoiceIntentLlmResolver');
  ...
  const handleVoiceDictation = useCallback(
    (text: string) => {
      if (isSegmentTimelineUnit(input.selectedTimelineUnit)) {
        ...
        const selectedSegmentUnit = input.unitsOnCurrentMedia.find(
          (unit) => unit.id === selectedTimelineUnit.unitId,
        );
        if (isSkipProcessingUnit(selectedSegmentUnit)) {
          setSaveState({
            kind: 'error',
            message: t(locale, 'transcription.action.skipProcessingMarked'),
          });
          return;
        }
        fireAndForget(
          (async () => {
            ...
            await input.saveSegmentContentForLayer(
          ...
          {
            context: 'src/pages/useTranscriptionAssistantController.ts:L111',
            policy: 'user-visible',
          },
        );
        return;
      ...
      const persistAndAdvance = async (persist: () => Promise<void>) => {
        await persist();
        if (
          input.nextUnitIdForVoiceDictation === undefined ||
          input.nextUnitIdForVoiceDictation.length === 0
        )
          return;
      ...
      fireAndForget(
        ...
        {
          context: 'src/pages/useTranscriptionAssistantController.ts:L157',
          policy: 'user-visible',
        },
      );
    },
    [input, locale, setSaveState],
  );

  const voiceDictationPipeline = useMemo<
    UseTranscriptionAssistantControllerResult['voiceDictationPipeline']
  >(() => {
  ...
  const handleVoiceAnalysisResult = useCallback(
    async (unitId: string | null, analysisText: string) => {
      if (unitId === null || unitId.length === 0) {
        ...
        reportValidationError({
          ...
          setErrorState: ({ message: nextMessage, meta }) =>
            setSaveState({ kind: 'error', message: nextMessage, errorMeta: meta }),
        });
        ...
        return {
          ...
          message:
            error instanceof Error
              ? error.message
              : t(locale, 'transcription.assistant.voiceAnalysis.saveFailedFallback'),
        };
      }
    },
