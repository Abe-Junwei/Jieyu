...
import { useAiChatContextValue, type AiChatContextSource } from '../hooks/ai/useAiChatContextValue';
...
function normalizeAssistantSidebarObserverRecommendation(
  ...
  const { actionType, targetUnitId, targetForm, targetPos, targetConfidence, ...rest } = item;
...
export function useTranscriptionAssistantSidebarController(
  ...
  const observerRecommendations = useMemo(
    () =>
      (input.aiChatContextInput.observerRecommendations ?? []).map(
        normalizeAssistantSidebarObserverRecommendation,
      ),
