...
export interface UseTranscriptionSelectionContextControllerResult {
  ...
  selectedTimelineUnitForTime:
    | Pick<LayerUnitDocType, 'startTime' | 'endTime'>
    | Pick<LayerUnitDocType, 'startTime' | 'endTime'>
    | null;
...
export function useTranscriptionSelectionContextController(
  ...
  const selectedTimelineSegment = useMemo(() => {
    ...
    return (
      input.segmentsByLayer
        .get(selectedUnit.layerId)
        ?.find((segment) => segment.id === selectedUnit.unitId) ?? null
    );
  ...
  const nextUnitIdForVoiceDictation = useMemo(
    () =>
      resolveNextUnitIdForDictation({
  ...
  const independentLayerIds = useMemo(
    () =>
      new Set(
        input.layers
          .filter((layer) => layerUsesOwnSegments(layer, input.defaultTranscriptionLayerId))
          .map((layer) => layer.id),
      ),
  ...
  const segmentTimelineLayerIds = useMemo(
    () =>
      new Set(
        input.layers
          .filter((layer) =>
