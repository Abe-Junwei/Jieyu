...
function orderSegmentTargetsByTimeline(
...
function hasSemanticSegmentSelector(call: AiChatToolCall): boolean {
  ...
  const segmentPosition =
    typeof call.arguments.segmentPosition === 'string' ? call.arguments.segmentPosition.trim() : '';
  return (
...
function resolveSelectorTargets(
  ...
  if (call.arguments.segmentPosition === 'middle') {
    return orderedTargets.length > 0
      ? [orderedTargets[Math.floor((orderedTargets.length - 1) / 2)]!]
      : [];
  }
  if (call.arguments.segmentPosition === 'previous' || call.arguments.segmentPosition === 'next') {
    if (!selectedTargetId) return [];
    const anchorIndex = orderedTargets.findIndex(
      (item) => item.id === selectedTargetId || item.unitId === selectedTargetId,
    );
...
function resolveTargetSegments(
  ...
  if (requestedIds.length > 0) {
    ...
    return requestedIds
      .map((id) => byId.get(id))
      .filter((item): item is SegmentTargetDescriptor => Boolean(item));
  }
...
export function createTranscriptionAiToolRiskCheck({
  ...
  const resolvedSegmentTargets =
    Array.isArray(segmentTargets) && segmentTargets.length > 0
      ? segmentTargets
  ...
  return (call: AiChatToolCall): AiToolRiskCheckResult | null => {
    if (call.name === 'delete_layer') {
      ...
      if (layerId) {
        const exists =
          transcriptionLayers.some((layer) => layer.id === layerId) ||
        ...
        if (layerType && languageQuery) {
          ...
          const entry = code
            ? SUPPORTED_VOICE_LANGS.flatMap((group) => group.langs).find(
                (lang) => lang.code === code,
              )
            : undefined;
          if (entry) {
            entry.label
              .split(/\s*\/\s*/)
              .forEach((part) => matchTokens.push(part.trim().toLowerCase()));
          }
          const matched = pool.filter((layer) => {
            const fields = [
              layer.languageId,
              layer.key,
              ...listUniqueNonEmptyMultiLangLabels(layer.name),
            ]
              .filter(
    ...
    if (isNonDeleteSegmentWriteTool(call.name)) {
      ...
      if (targetSegments.length === 0) {
        ...
        if (requestedSegmentId.length > 0) {
          return {
            requiresConfirmation: false,
            riskSummary: tf(locale, 'transcription.aiTool.segment.segmentNotFound', {
    ...
    const targetUnitIds = new Set(
      targetSegments.filter((item) => item.kind === 'unit').map((item) => item.unitId ?? item.id),
    );
    ...
    const transcriptionPreviews = targetSegments
      ...
      .filter((item) => item.length > 0);
    const translationEntryCountWithContent = Array.from(translationTextByLayer.values()).reduce(
      (count, layerMap) => {
    ...
    if (targetSegments.length === 1) {
      ...
      const rowIndex =
        Math.max(
          0,
          sortedByTime.findIndex((item) => item.id === targetSegment.id),
        ) + 1;
      const transcriptionPreview = transcriptionPreviews[0]
        ? transcriptionPreviews[0]!.length > 18
          ? `${transcriptionPreviews[0]!.slice(0, 18)}...`
          : transcriptionPreviews[0]!
        : t(locale, 'transcription.ai.segment.noTranscriptionText');

      return {
        requiresConfirmation: true,
        riskSummary: tf(locale, 'transcription.ai.segment.deleteRiskSummary', {
          rowIndex,
          timeRange,
        }),
        impactPreview: [
          tf(locale, 'transcription.ai.segment.deleteImpactContent', {
            preview: transcriptionPreview,
          }),
          tf(locale, 'transcription.ai.segment.deleteImpactRelation', {
            count: translationEntryCountWithContent,
          }),
    ...
    return {
      ...
      impactPreview: [
        tf(locale, 'transcription.ai.segment.deleteBatchImpactScope', {
          count: targetSegments.length,
          preview:
            combinedPreview.length > 48 ? `${combinedPreview.slice(0, 48)}...` : combinedPreview,
        }),
        tf(locale, 'transcription.ai.segment.deleteBatchImpactRelation', {
          count: translationEntryCountWithContent,
        }),
        t(locale, 'transcription.ai.segment.deleteImpactUndo'),
      ],
