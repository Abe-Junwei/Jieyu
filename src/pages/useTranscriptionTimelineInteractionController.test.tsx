...
const { mockUpdateSegment } = vi.hoisted(() => ({
  mockUpdateSegment: vi.fn<(segmentId: string, payload: unknown) => Promise<void>>(
    async () => undefined,
  ),
}));
...
function createBaseInput(overrides: Partial<HookInput> = {}): HookInput {
  ...
  return {
    ...
    segmentsByLayer: new Map([
      [
        'layer-sub',
        [makeSegment('seg-1', 'layer-sub', 0, 2), makeSegment('seg-2', 'layer-sub', 3, 4)],
      ],
    ]),
...
describe('useTranscriptionTimelineInteractionController', () => {
  ...
  it('captures Alt pointerdown into sub-selection drag state', () => {
    ...
    expect(
      (waveCanvas as HTMLDivElement & { setPointerCapture: ReturnType<typeof vi.fn> })
        .setPointerCapture,
