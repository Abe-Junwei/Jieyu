...
export function useSpeakerActionRoutingController({
  ...
  const handleExportSpeakerSegmentsRouted = useCallback(
    (speakerKey: string) => {
      ...
      setSaveState({
        kind: 'done',
        message: formatSpeakerExportDone('segments', rows.length, t, tf),
      });
    },
    [
      activeSpeakerManagementLayer,
  ...
  const confirmSpeakerDialogRouted = useCallback(async () => {
    if (segmentSpeakerDialogState?.mode === 'clear') {
      ...
      try {
        ...
        setSaveState({
          kind: 'done',
          message: formatSpeakerClearResult('segments', cleared, t, tf),
        });
        setSegmentSpeakerDialogState(null);
      } catch (error) {
        if (undoPushed) await undo();
    ...
    (speakerId?: string) => {
      if (
        ...
        fireAndForget(applySpeakerToMixedSelection(speakerId), {
          context: 'src/pages/useSpeakerActionRoutingController.ts:L638',
          policy: 'user-visible',
        });
        return true;
      }
      if (selectedBatchSegmentsForSpeakerActions.length > 0) {
        fireAndForget(
          ...
          {
            context: 'src/pages/useSpeakerActionRoutingController.ts:L642',
            policy: 'user-visible',
          },
        );
        return true;
      ...
      if (selectedStandaloneUnitIdsForSpeakerActions.length > 0) {
        fireAndForget(
          ...
          {
            context: 'src/pages/useSpeakerActionRoutingController.ts:L646',
            policy: 'user-visible',
          },
        );
        return true;
