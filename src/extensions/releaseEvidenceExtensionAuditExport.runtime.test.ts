...
const DEFAULT_OUTPUT_RELATIVE_PATH =
  'docs/execution/audits/extension-capability-audit-export-v1.ndjson';

function resolveExportOutputPath(): string {
  const configured = String(
    process.env.RELEASE_EVIDENCE_EXTENSION_CAPABILITY_AUDIT_EXPORT ?? '',
  ).trim();
...
describe('release evidence extension audit export runtime', () => {
  it('writes extension capability audit ndjson from runtime invocation path', async () => {
    ...
    await expect(
      registry.invokeCapability(extensionId, 'read.transcription', { fail: false }),
    ).resolves.toEqual({ ok: true });
    await expect(
      registry.invokeCapability(extensionId, 'read.transcription', { fail: true }),
    ).rejects.toThrow('runtime export seed failure');

    const rows = registry
      .getCapabilityAuditTail(50)
      .filter((item) => item.extensionId === extensionId)
      .sort((left, right) => Number(left.at) - Number(right.at));
    expect(rows.length).toBeGreaterThanOrEqual(2);

    const normalizedLines = rows.map((row) =>
      JSON.stringify({
