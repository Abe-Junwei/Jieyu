...
const tmpDir = path.join(workspaceRoot, '.tmp', 'm12');
const vitestJsonPath = path.join(tmpDir, 'm12-multi-replica.vitest.json');
...
function buildReport({
  ...
  lines.push('# M12 多副本批量同步评审报告（自动生成）');
  lines.push('');
  ...
  lines.push('|---|---:|---:|---|');
  lines.push(`| batch_sync_contract_pass_rate | 100% | ${toRateLabel(batch.rate)} (${batch.passed}/${batch.total}) | ${batch.rate === 1 ? 'PASS' : 'FAIL'} |`);
  lines.push(`| quorum_contract_pass_rate | 100% | ${toRateLabel(quorum.rate)} (${quorum.passed}/${quorum.total}) | ${quorum.rate === 1 ? 'PASS' : 'FAIL'} |`);
  ...
  lines.push(`| multi_replica_consistency_pass_rate | 100% | ${toRateLabel(multiReplica.rate)} (${multiReplica.passed}/${multiReplica.total}) | ${multiReplica.rate === 1 ? 'PASS' : 'FAIL'} |`);
  lines.push('');
  ...
  lines.push('3. M12 要求批量同步、仲裁法定人数、回滚与多副本一致性契约均为 100%。');
  lines.push('');
...
async function main() {
  ...
  if (!report?.testResults) {
    console.error('[m12-multi-replica] Missing vitest json report.');
    process.exitCode = 1;
    return;
  }
  ...
  const batch = resolveTotals(assertions, '[batch]');
  const quorum = resolveTotals(assertions, '[quorum]');
  const rollback = resolveTotals(assertions, '[rollback]');
  const multiReplica = resolveTotals(assertions, '[multi-replica]');
  ...
  if (batch.rate < 1) {
    p0Findings.push(`批量同步契约通过率不达标：${toRateLabel(batch.rate)}。`);
  }
  if (quorum.rate < 1) {
    p0Findings.push(`法定人数契约通过率不达标：${toRateLabel(quorum.rate)}。`);
  }
  if (rollback.rate < 1) {
    p0Findings.push(`回滚契约通过率不达标：${toRateLabel(rollback.rate)}。`);
  }
  if (multiReplica.rate < 1) {
    p0Findings.push(`多副本一致性契约通过率不达标：${toRateLabel(multiReplica.rate)}。`);
  }

  if (run.status !== 0 || run.signal) {
    p0Findings.push('vitest 执行失败，多副本批量同步契约测试未完成。');
  }
  ...
  console.log(`[m12-multi-replica] Report written: ${path.relative(workspaceRoot, reportOutputPath)}`);
  console.log(`[m12-multi-replica] Gate decision: ${decision}`);
...
main().catch((error) => {
  console.error('[m12-multi-replica] Unexpected error:', error);
  process.exitCode = 1;
});
