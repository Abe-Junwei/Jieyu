...
export const architectureGuardRules = [
  ...
  // 冻结页面层直连 db 的文件数（基线 45，仅允许下降不允许上涨） | Freeze page→db import file count (baseline 45, must not increase)
  patternRule(/^src\/pages\/(?!.*\.test\.).*\.(ts|tsx)$/, {
    maxRegexMatchCounts: [
      {
        label: 'page→db direct imports (M3 baseline 45, frozen — new files must be 0)',
