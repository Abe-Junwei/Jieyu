...
- 高风险或 UI 觉察大改默认套 feature flag（`src/featureFlags.ts`），`false` 合并，自用 1 周后切默认 `true`；详见 §五.5 feature flag 工作流。
...
- 高风险或 UI 觉察大改：在 [src/featureFlags.ts](src/featureFlags.ts) 注册 `boolean | enum` flag。
