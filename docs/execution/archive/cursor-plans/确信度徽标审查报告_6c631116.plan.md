...
[`TranscriptionTimelineTextOnly.tsx`](src/pages/TranscriptionPage.TimelineContent.tsx)：`segment` 禁止 `realUtt?.selfCertainty` 回退 —— 与控制器一致，**利于防串层**。
...
- 时间轴：[`TranscriptionTimelineTextOnly.tsx`](src/pages/TranscriptionPage.TimelineContent.tsx)、[`useTimelineAnnotationHelpers.tsx`](src/hooks/useTimelineAnnotationHelpers.tsx)、[`TranscriptionOverlays.tsx`](src/components/TranscriptionOverlays.tsx)
