...
- **文件**：
  - `[src/components/TranscriptionTimelineMediaLanes.tsx](src/components/TranscriptionTimelineMediaLanes.tsx)` — 在 `layer.layerType === 'transcription'` 分支，对每一行像翻译层一样解析 `translationTextByLayer` / `translationAudioByLayer`（已存在 props）、`recording*`、`startRecordingForUnit` / `stopRecording` / `deleteVoiceTranslation`，并传入行组件。
  - **实现策略（二选一，推荐前者）**：
...
- **文档模式时间轴**：若 `[TranscriptionTimelineTextOnly.tsx](src/components/TranscriptionTimelineTextOnly.tsx)` 中翻译层已展示录音控件，转写层若有 `audio`/`mixed`，需同样接入（避免「媒体模式有、文档模式无」）。
...
- **位置**：[`TranscriptionTimelineMediaLanes`](src/components/TranscriptionTimelineMediaLanes.tsx) 转写分支仅用 [`TranscriptionTimelineMediaTranscriptionRow`](src/components/TranscriptionTimelineMediaTranscriptionRow.tsx)，未传录音 props。
