...
**审查日期**：2026-05-04  
**审查范围**：`src/` 全目录（1634 文件，~478k 行）+ `tests/` + `scripts/`  
**审查轮次**：6 轮 18 维度  
**执行摘要**：项目具备工业级的架构门禁与工程治理体系，整体质量极高；主要技术债集中在**循环依赖**、**模块膨胀**和**测试配置边界**三个方面。
...
| 循环依赖 | 🔴 严重 | 32 个循环链 |
| CSS 体系 | ✅ 通过 | 全部 12 项 CSS 门禁通过 |
| 构建预算 | ✅ 通过 | 所有 chunk 在预算内 |
| 安全审计 | ✅ 通过 | 生产依赖 0 漏洞 |
| 单元测试 | ⚠️ 通过+配置瑕疵 | 541/544 通过，1 个误跑 |
| E2E 测试 | ✅ 通过 | Playwright 三引擎覆盖 |
...
**深度评价**：`scripts/check-architecture-guard.mjs` 的 **879 行配置规则**是本项目工程化水平的核心体现：
- **Orchestrator 壳模式**：`TranscriptionPage.Orchestrator.tsx` 被限制为 100 行、0 hooks，强制其为纯装载壳。
...
```
db/index.ts → dbIntegrityProbe.ts → engine.ts 
    → migrations/m18LinguisticUnitCutover.ts 
    → migrations/timelineUnitMapping.ts 
    → utils/transcriptionFormatters.ts 
    → [utils/langMapping.ts | utils/multiLangLabels.ts] 
    → db/index.ts
```

**根因**：`engine.ts` 模块顶层同步导入 migration 文件进行 schema 升级，而 migration 依赖 utils，utils 又回指 `db/index.ts`。

**风险**：Dexie 初始化时若模块求值顺序异常，可能导致 migration 在 db 单例就绪前执行。

**修复建议**：
1. 将 `utils/transcriptionFormatters.ts` 中依赖 `db/index.ts` 的部分提取为 `*.types.ts`，migration 仅导入类型/纯函数。
2. 或 `engine.ts` 改用动态导入 `import('./migrations/...')` 延迟加载 migration。

#### 🔴 高风险：跨层 Toast 循环（1 个核心循环，拖出 14 个衍生循环）

```
ToastContext.tsx → hooks/transcriptionTypes.ts → db/index.ts 
    → db/io.ts → hooks/useBackupReminder.ts → ToastContext.tsx
```

**精准定位**：
- `src/db/io.ts:57`：`import { markBackupCompleted } from '../hooks/useBackupReminder';`
- `src/hooks/useBackupReminder.ts:2`：`import { useToast } from '../contexts/ToastContext';`

**根因**：**数据层（db/io.ts）直接依赖 Hook 层**，违反 M3 依赖方向。db 应处于最底层，不得依赖 hooks/contexts。

**风险**：这是全仓最大的循环锚点，以此为起点拖累了 `VoiceAgentService → GlobalContextService → EmbeddingSearchService → ...` 的巨型循环网。

**修复建议**：
- `db/io.ts` 不得导入 `useBackupReminder`。将 `markBackupCompleted` 解耦为纯函数或事件机制，让 IO 层触发事件，上层 hook 消费。
- `ToastContext` 对 `transcriptionTypes` 的依赖应降级为 type-only，并将 `SaveState` 类型下沉到 `types/`。

#### 🟡 中风险：Voice 服务同域循环（10 个循环）

`VoiceAgentService.ts` / `VoiceInputService.ts` 被横向拆分为 10+ 子文件，互相 import。

**修复建议**：子文件合并为私有方法，或建立单向依赖（子文件不回指主文件，类型通过独立 `.types.ts` 共享）。

#### 🟡 中风险：AI hooks 循环网（13+ 个循环）

`useAiChat.ts` ↔ `useAiChat.confirmExecution.ts` 等 10+ 子文件密集循环。

**评估**：目前均为 `type-only import`，TypeScript 编译期可处理，但极易恶化为值导入循环。

**修复建议**：立即提取 `hooks/useAiChat.types.ts`，所有子文件从 types 导入，不从 `useAiChat.ts` 回指。

#### 🟢 低风险：UI 层轻循环（5 个）

- `AiPanelContext.tsx ↔ AiAnalysisPanel.tsx`：Context 从组件导入 type。
- `TranscriptionPage.TimelineContent.tsx ↔ TranscriptionTimelineWorkspaceHost.tsx`：Pages 内部循环。
- `extensionRuntime.ts ↔ extensionTrustGovernance.ts`
- i18n/collaboration 循环（1 个）
...
**`any` 密度扫描**：全仓非测试代码中仅发现 **4 处 `any`**：

| 位置 | 代码 | 评估 |
|------|------|------|
| `src/pages/useReadyWorkspaceLayoutDerivations.ts` | `deferredAiRuntimeForSidebar: { aiChat: any }` | 🟡 应为精确类型 |
| `src/db/io.ts` | `dbInstance.dexie.transaction as (...args: any[]) => Promise<void>` | 🟡 Dexie 类型断言，可接受 |
| `src/hooks/useAiChat.streamCompletion.ts` | `metadata?: any` (×2) | 🟡 AI 流元数据，建议用 `unknown` + 运行时校验 |

**结论**：any 密度极低（~4 / 47万行），类型系统健康度极高。
...
**迁移策略**：
- 当前活跃迁移：M18（127行，语言单位切主）、M41（118行，自确性去污染）、M42（17行，TrackEntity 文档ID）。
...
```
tests/e2e-sandbox/aiSessionSidecarSandboxAudit.spec.ts
  test.describe('F4 session sidecar sandbox audit...')
    ^ Error: test.describe() can only be called in a test file
```

**根因**：`tests/e2e-sandbox/*.spec.ts` 是 **Playwright E2E 测试**，使用了 `test.describe`（Playwright API），但被 Vitest 扫描并尝试执行，导致类型冲突。

**风险等级**：🟡 **配置瑕疵，非业务逻辑问题**。但 CI 中若 Vitest 全量运行会因此失败（除非已排除该目录）。

**修复建议**：在 `vitest.config.ts`（或 `vite.config.ts` 中的 `test` 配置）的 `exclude` 中加入 `tests/e2e-sandbox/**` 和 `tests/e2e/**`。
...
| E2E 文件 | 4 个 spec（494 行） |
| E2E-Sandbox 文件 | 1 个 spec（119 行） |
| 重试策略 | 1 次 |
...
**评价**：E2E 数量偏少（仅 5 个 spec），但覆盖了最关键的安全和 a11y 场景。对于 47 万行的应用，建议补充核心工作流 E2E（导入 → 转写 → AI 分析 → 导出）。
...
**文件规模**：`src/observability/` 共 1694 行。
...
全仓非测试代码中 **58 个文件**包含 `console.log/warn/error`。建议清理或替换为 `recordMetric` / `aiTrace`。
...
| # | 问题 | 位置 | 影响 |
|---|------|------|------|
| 1 | **DB 层循环依赖** | `db/index → engine → migration → utils → db` | 初始化时序风险 |
| 2 | **跨层 Toast 循环** | `db/io.ts` 导入 `useBackupReminder` | 违反 M3，拖累 14 个衍生循环 |
| 3 | **Vitest 误跑 Playwright** | `tests/e2e-sandbox/*.spec.ts` 未排除 | CI 中 1 个测试文件失败 |

### 🟡 P1：近期优化

| # | 问题 | 位置 | 建议 |
|---|------|------|------|
| 4 | **AiChatCard.tsx 膨胀** | `src/components/ai/AiChatCard.tsx` | 2079/2100 行，立即拆分 |
| 5 | **AI hooks type 循环** | `useAiChat.ts` ↔ 子文件 | 提取 `useAiChat.types.ts` |
| 6 | **Voice 服务同域循环** | `VoiceAgentService` / `VoiceInputService` | 合并子文件或单向化依赖 |
| 7 | **ImportExport chunk 过大** | `TranscriptionPage.ImportExport.archive.js` | 1.47MB，需拆包分析 |
| 8 | **Console 残留** | 58 个文件 | 清理或接入遥测 |

### 🟢 P2：持续观察

| # | 问题 | 位置 | 建议 |
|---|------|------|------|
| 9 | **内联样式债务** | 4 处 | 逐步迁移到 CSS 变量 |
| 10 | **hover-without-focus** | 6 处 CSS | a11y 优化 |
| 11 | **E2E 覆盖不足** | 仅 5 个 spec | 补充核心工作流 E2E |
| 12 | **TODO(2026Q2)** | `unitSelfCertainty.ts` | 按计划迁移 |

---

## 正向实践清单（值得保持和借鉴）

1. **Architecture Guard 规则体系**：879 行精细规则覆盖文件规模、hook 使用、导入方向、ADR 防护，是国内罕见的工业级架构守护。
...
7. **类型系统极致**：47 万行代码仅 4 处 `any`，`tsc --noEmit` 0 错误。
