...
| **P1** | 矩阵随动 | 以 [`timelineParityMatrix.ts`](../../../src/pages/timelineParityMatrix.ts) 的 **`TIMELINE_PARITY_MATRIX_VERSION`** 为准（历史：v6 起 `acoustic-strip-contract`；**v12** 起 `segment-range-gesture-single-surface`；**v13** 壳层 `layersCount` 单源；**v14** 纵向手势说明与 `useLasso.test.tsx` 锚点；**v15** 起 `phase-f-range-preview-ssot` 阶段 F·1 SSOT backlog 行；**当前 = 15**）。 |
...
**本规划范围内已落地**：阶段 **A0**（布局键）、**A**（[`timelineViewportTypes`](../../../src/hooks/timelineViewportTypes.ts)）、**C**（[`useTimelineViewport`](../../../src/hooks/useTimelineViewport.ts) + bridge）、**orchestrator-zoom-source**、**D**（stage `zoomControls` 对齐投影）、**矩阵**（v9–v15 演进见 [`timelineParityMatrix.ts`](../../../src/pages/timelineParityMatrix.ts)）、**B 完整合同**（`AcousticStripContract` / `acousticStrip`）、**P0 热点与 layout 守卫**、**缩放百分比写者收口至波形桥**、**P2**（`timelineMode` 占位/写路径 + import 单路径核对）、**P3（Hub + G3）**；**阶段 F 阶段 0（契约）**：矩阵行 **`segment-range-gesture-single-surface`** + 锚点 [`segmentRangeGestureParity.test.ts`](../../../src/pages/segmentRangeGestureParity.test.ts) + [`useLasso.test.tsx`](../../../src/hooks/useLasso.test.tsx)；**阶段 F·1（工程锚点，实现仍为 backlog）**：矩阵行 **`phase-f-range-preview-ssot`** + [`phaseFRangePreviewSsot.test.ts`](../../../src/pages/phaseFRangePreviewSsot.test.ts)。
...
- 移除 `playbackExtentSec` 钳制（播放覆盖完整媒体）。
...
- `timeMapping` 非 1:1 的语段 X 渲染（A8）另列。

### 阶段 F — 语段范围手势单表面（与声学解耦）
...
**子文档（分阶段落地方案）**：[`时间轴交互与壳层收敛落地方案-2026-04-21.md`](./时间轴交互与壳层收敛落地方案-2026-04-21.md)（阶段 A–F：extent 单源、视口帧、声学 chrome 映射、范围交互 writer、改时 preview、选集与空壳）。

**阶段 0（契约，已落地）**

- [`timelineParityMatrix.ts`](../../../src/pages/timelineParityMatrix.ts)：`TIMELINE_PARITY_MATRIX_VERSION` **12**；新增矩阵行 **`segment-range-gesture-single-surface`**（`waveform`/`textOnly`：`full`；`vertical`：`partial` + `verticalGapZh`）。
- 锚点：[`timelineParityMatrix.test.ts`](../../../src/pages/timelineParityMatrix.test.ts) + [`segmentRangeGestureParity.test.ts`](../../../src/pages/segmentRangeGestureParity.test.ts)；结构门禁：[`TranscriptionPage.structure.test.ts`](../../../src/pages/TranscriptionPage.structure.test.ts) 断言矩阵源码含该行 id 与版本常量。

**后续阶段（未在本批实现）**：抽出 `useSegmentRangeGesturePreview`（或等价名）、合并 `waveLassoRect`/`lassoRect`/`enableDragSelection` 视觉路径；详见规划评审记录与主规划 §5.7 第二条线。

**阶段 1（下一迭代，backlog）**

1. **单一 preview 状态**：波形空区、`RegionsPlugin`、tier 套索的拖建/框选 **预览** 收敛到一处 reducer（或等价 hook），避免多处 `useState` 分叉。
2. **与纵向协同**：延续 `tierTimelineLassoSuppressed` 与 `useLasso` 排除列表；阶段 1 不改变「纵向 partial」产品结论，只减少实现上的双管道风险。
3. **验收**：在 [`segmentRangeGestureParity.test.ts`](../../../src/pages/segmentRangeGestureParity.test.ts) / [`useLasso.test.tsx`](../../../src/hooks/useLasso.test.tsx) 基础上按需增补；手验与 [横纵时间轴宿主差异与整改清单-2026-04-21.md](./横纵时间轴宿主差异与整改清单-2026-04-21.md) §6.1 对齐。

---

## 5. 验收矩阵（已写入 [`timelineParityMatrix`](../../../src/pages/timelineParityMatrix.ts)；**`TIMELINE_PARITY_MATRIX_VERSION` 以该文件为准**，当前 **v15**）
...
| 阶段 F：语段范围拖建单一反馈面（契约） | `segment-range-gesture-single-surface` | `timelineParityMatrix.test.ts` + `segmentRangeGestureParity.test.ts` + `TranscriptionPage.structure.test.ts` + `useLasso.test.tsx`（矩阵行/版本门禁） |
| 阶段 F·1：预览状态 SSOT（backlog） | `phase-f-range-preview-ssot` | `phaseFRangePreviewSsot.test.ts` + `segmentRangeGestureParity.test.ts` + `timelineParityMatrix.test.ts`（parity 全 partial，实现待收敛） |
...
4. ~~**阶段 A0 与能力矩阵**~~：**已决** — 合同键已删；矩阵当前 **v15**（含 **`g3-draft-autosave-key-helpers`**、**`segment-range-gesture-single-surface`**、壳层 **`layersCount` 单源**、**`useLasso`** 锚点、**`phase-f-range-preview-ssot`** 阶段 F·1 SSOT 工程行）；版本号以 [`timelineParityMatrix.ts`](../../../src/pages/timelineParityMatrix.ts) 内常量为准。
