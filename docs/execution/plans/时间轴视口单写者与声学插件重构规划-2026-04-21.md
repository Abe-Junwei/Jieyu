...
| **P1** | 矩阵随动 | [`TIMELINE_PARITY_MATRIX_VERSION`](../../../src/pages/timelineParityMatrix.ts) **v6**：在 v5 基础上新增 **`acoustic-strip-contract`** 行。 |
...
## 5. 验收矩阵（已写入 [`timelineParityMatrix`](../../../src/pages/timelineParityMatrix.ts) **v10**）
...
| G3：草稿防抖 key + 侧栏译文壳 | `g3-draft-autosave-key-helpers` | `timelineDraftAutoSaveKeys.test.ts` + `TranscriptionTimelineTextTranslationItem.test.tsx` + `TranscriptionTimelineVerticalView.test.tsx` |
...
- [x] **parity-matrix-bump**：`TIMELINE_PARITY_MATRIX_VERSION` **10**；v9 含 **`g3-lane-draft-editor-cell-shared`**；v10 新增 **`g3-draft-autosave-key-helpers`**。
...
- [x] **g3-draft-autosave-keys**：**`timelineDraftAutoSaveKeys`**（含纵向 **pr-src / pr-seg / pr**）+ **`TranscriptionTimelineMediaTranscriptionRow`** / **`TranscriptionTimelineTextTranslationItem`** / **`TranscriptionTimelineVerticalViewGroupList`**；侧栏 **`bubbleClick`**；定向 `timelineDraftAutoSaveKeys.test.ts`、`TranscriptionTimelineVerticalView.test.tsx`。
...
4. ~~**阶段 A0 与能力矩阵**~~：**已决** — 合同键已删；矩阵当前 **v10**（含 **`g3-draft-autosave-key-helpers`**）。
