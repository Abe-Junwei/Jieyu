---
title: 多宿主翻译层-方案A-执行方案-2026-04-20
doc_type: execution-plan
status: active
owner: repo
last_reviewed: 2026-05-12
---
# 多宿主翻译层（方案 A）执行方案（重规划）
...
1. **承认「规范领先于实现」**：仓库当前仍存在 `layer_links` 与 `parentLayerId`/`parentLayerIds` **双轨中间态**；在硬切完成前须有**合并窗口前一次性盘点**与 **CI 禁止新增「翻译宿主 parent 读」**（见 §2.0）。
...
3. **读路径 P0**：`translationLayerTargetResolver`、`TranscriptionTimelineVerticalView`、`useLayerSegments` 等必须改为 **link 图**（优先经 §1.8 只读适配器）；G9a/G9b 拆分仍适用（§7 / §8 / §9）。
...
1. **能力边界确认**：仅“翻译层（translation）”支持多宿主；转写层（transcription）维持单宿主语义。
2. **关系模型确认**：`layer_links` 是**一等事实源（single source of truth）**。
3. **兼容策略确认**：无历史数据、无向后兼容、无 legacy 迁移负担，可采用破坏式重整。

> 由此推导：**翻译层（`translation`）—转写宿主**关系仅以 `layer_links` 表达；**不得**再将**翻译层**的 `parentLayerId` / `parentLayerIds` 作为该宿主关系的输入、输出或持久化载体（§1.3、§1.6）。**转写层（`transcription`）** 上 `parentLayerId` 若用于**层树内**挂靠（从属转写 → 根转写），与本方案范围**正交**，**保留**且不在本期迁出（§1.6）。
...
- 翻译层宿主集合 = `layer_links` 中 `layerId === translationLayer.id` 的全部 link。  
- **宿主侧引用（修订）**：以 **转写层 `Layer.id`** 为规范标识；持久化字段名若仍为 `transcriptionLayerKey`，须视为**待改名技术债**并一次性改为存 id（见 §1.5），禁止运行期「既像 key 又像 id」的双解释。  
- 主宿主通过 `isPreferred === true` 定义（同一翻译层最多一个 preferred）。

### 1.2 结构约束

- 翻译层至少有 1 个宿主 link。  
- **转写层**：不通过 `parentLayerId` 表达「**被翻译宿主**」语义（该语义仅 `layer_links`）；**允许**通过 `parentLayerId` 表达「**从属转写层 → 根转写层**」的层树挂靠（与 `buildLayerBundles` 一致，见 §1.6）。  
- 不允许出现“有 translation 层但无 link”的悬空状态（除事务中间态）。

### 1.3 数据与投影策略（无 legacy：硬单源）

- **翻译宿主（精确）**：对 `layerType === 'translation'`，不再在 `LayerDocType` 上保留 `parentLayerId` / `parentLayerIds` 作为**指向转写宿主**的载体；该关系仅由 `layer_links` 表达。  
...
| [TranscriptionTimelineVerticalView.tsx](../../../src/components/TranscriptionTimelineVerticalView.tsx) | `translationLayerAppliesToComparisonSourceTranscriptionIds` 等仍看单 `parentLayerId` | 宿主集合与 source 集合**交集** | 非主宿主对照漏译文列        |
| [useLayerSegments.ts](../../../src/hooks/useLayerSegments.ts)                                      | 翻译宿主解析用 `layer.parentLayerId`                                                    | preferred 与 link 一致   | 语段轨与主宿主策略漂移       |
...
| `layerParentIds.ts`（已删除；读路径见 [`useLayerSegments.ts`](../../../src/hooks/useLayerSegments.ts)） | 宿主读写仍绕 parent                                                                    | **删除**或收窄为与翻译宿主无关的工具  | 无 legacy 下禁止第二真相  |
...
**`layerParentIds.ts`（无 legacy 修订）**：该文件已从仓库删除（G13）；宿主/segment 读路径以 [`useLayerSegments.ts`](../../../src/hooks/useLayerSegments.ts) 与 `layer_links` 为准。随 `Layer` 上翻译宿主 parent 字段删除，**不得**再引入同名「第二真相」工具模块。
...
**`parentLayerIds`（精确）**：历史实现曾集中于已删除的 `layerParentIds.ts`；当前 segment / 宿主解析以 [`useLayerSegments.ts`](../../../src/hooks/useLayerSegments.ts) 与 `layer_links` 为准。随翻译宿主迁 link 后**与译文 `parentLayerId` 一并自翻译层移除**，**不**主张用于转写层多父（转写多父非本方案范围）。
...
- **`LayerOrderingService`**：`buildLayerBundles` / 拖拽重排后依赖翻译宿主时，须由调用方注入 **`layerLinks`** 或 §1.8 预计算索引；禁止在纯排序函数内隐式打开 Dexie（若项目已有 store 注入，须在模块头注释写明数据来源）。
...
- **一次性全仓盘点**（硬切合并前必做）：列出仍把「翻译宿主」绑定在 `parentLayerId`/`parentLayerIds` 或 tier 上的读/写点；合并后应为零（归档至 `docs/execution/audits/` 可选）。  
- **CI 门禁（硬切后常态）**：禁止新增「以 `translation.parentLayerId` 判定翻译宿主」的逻辑；可对 `translationLayerAppliesTo*`、`resolveHostAware*` 等路径设白名单直至清零。  
...
- [src/services/LayerConstraintService.ts](../../../src/services/LayerConstraintService.ts)  
  ...
  - **翻译宿主**相关边与可达性：**不再**以 `translation.parentLayerId` / `parentLayerIds` 推导；改走 link 图。  
...
- [src/hooks/useTranscriptionLayerActions.ts](../../../src/hooks/useTranscriptionLayerActions.ts)  
  ...
  - `performLayerDelete`：删除转写层时仅移除关联 link；译文 link 变空时按策略级联删翻译层。  
...
- **G9a（读路径 / P0）**：`translationLayerTargetResolver`、`TranscriptionTimelineVerticalView`、`useLayerSegments`、`useVoiceInteraction` 等依赖宿主选择的 selection 路径；优先保证**多宿主命中与 preferred 一致**。  
...
- [src/hooks/useImportExport.additionalTierHandlers.ts](../../../src/hooks/useImportExport.additionalTierHandlers.ts)  
  ...
  - 移除对 **翻译宿主** `parentLayerId` / `parentLayerIds`（及 tier 上「第二宿主」伪语义）的对比/修复依赖；**不**要求消除对 `transcription.parentLayerId` 的层树处理（§1.6）。
...
- 新增/改造 Vitest：
  - 翻译层多宿主创建：生成 N 条 link 且唯一 preferred。  
  - 删除转写层：只移除对应 link；剩余 link>0 时翻译层保留。  
...
1. 新建翻译层绑定英/法两条独立转写，指定英语为主宿主。
2. 左右对照在英法视角均能命中同一翻译层。
3. 删除英语转写后，翻译层仍保留并自动由法语宿主持有。
...
6. 事务原子性边界未定：创建翻译层 + 批量 link + preferred 指定必须在同一事务。
7. AI 工具动作语义未补：link/unlink 需要明确“增边/删边/切主”三种模式与幂等行为。
8. 导入导出损失语义提示未补：EAF 仅主链导出时，需在 UI/日志明示有损信息。
9. 读链路残留单宿主判断点需清零：**翻译宿主**路径上凡以 `translation.parentLayerId`/`parentLayerIds` 判定宿主的逻辑须改为 link 图查询；**转写层树**路径上依赖 `transcription.parentLayerId` 的 bundle/排序逻辑**不在**本条清零范围（§1.6）。
...
- **半套 SSOT 风险**：无 legacy 下以 **硬切完成** 为发布门槛；硬切完成前**禁止**对外宣称「翻译宿主已完全以 layer_links 为唯一事实源」。
...
- **G9a**：`translationLayerTargetResolver` / `TranscriptionTimelineVerticalView` / `useLayerSegments` 等读路径改为 link-only
...
| G6  | 事务原子性边界未定                                 | 将“创建翻译层 + 建立 link 集 + 写 preferred”封装到单个 Dexie `rw` 事务。                                                                                                      | [src/hooks/useTranscriptionLayerActions.ts](../../../src/hooks/useTranscriptionLayerActions.ts), [src/db/dexieTranscriptionGraphStores.ts](../../../src/db/dexieTranscriptionGraphStores.ts)                                                                                                                                                                                                                                                                                                                                                                          |
...
| G9  | 读链路残留单宿主判断点                               | **拆分为 G9a / G9b（见 §2.2）**；全仓清理**翻译宿主**路径上对 `translation.parent*` 的判定，统一改为 link 图或 §1.8 只读适配器（**不**清理 `transcription.parentLayerId` 层树语义）。                                                                        | G9a：`translationLayerTargetResolver`、`TranscriptionTimelineVerticalView`、`useLayerSegments` 等；G9b：`LayerConstraintService`、`useTranscriptionUnitActions.helpers` 等                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| G13 | `layerParentIds` 与 SSOT 叙事冲突              | **删除**或去翻译宿主语义；禁止作为过渡「第二真相」（§1.4）。                                                                                                                          | 源码已移除；读路径锚点 [`useLayerSegments.ts`](../../../src/hooks/useLayerSegments.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| G14 | 翻译宿主仍落在译文 Layer.parent*                   | 从 `layerType === 'translation'` 的持久化与类型中**移除**宿主用 `parentLayerId`/`parentLayerIds`；**保留** `transcription` 的层树 `parentLayerId`（§1.6）；注释与 ADR 与 §1.3 一致。    | [src/db/types.ts](../../../src/db/types.ts), `docs/adr/`（可选）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| T13   | [src/components/TranscriptionTimelineVerticalView.test.tsx](../../../src/components/TranscriptionTimelineVerticalView.test.tsx)           | `filters translation layers by host-link intersection`                         | G9      |
| T14   | [`src/services/EafConstraintParsing.test.ts`](../../../src/services/EafConstraintParsing.test.ts)（导入 `useLayerSegments` 的约束辅助） | `uses preferred host as timeline source for translation layer`                 | G9      |
