...
- **不依赖宿主媒体的**：例如**翻译层本地录音**（现已有 translation recording / blob 路径）、纯文本层编辑、扬声器标签等 —— 必须在**统一宿主**上继续可用；其数据源是 **层单元 + attachment blob**，不是「整段宿主音频」。
...
> **实现注**：下列表格为 **产品语言** 验收提纲；**工程锚点与 `parity` 三列**以 [`src/pages/timelineParityMatrix.ts`](./src/pages/timelineParityMatrix.ts) 为准（**`TIMELINE_PARITY_MATRIX_VERSION = 15`**，单测 [`timelineParityMatrix.test.ts`](./src/pages/timelineParityMatrix.test.ts)）。
...
- 2026-04-21：**P3 Project Hub 时间映射（矩阵 v8）**。`TIMELINE_PARITY_MATRIX_VERSION` **8**：新增 **`project-hub-time-mapping-modeless`**；Hub 逻辑时间导出/校准/预览不再依赖 **`exportTimelineModeLabel`**；`computeSemanticTimelineMappingPreview`；定向 `LeftRailProjectHub.test.tsx`、`timeMappingHubPreview.test.ts`。
...
- 2026-04-21：**文档与矩阵版本号同步**。概述与 §5.7 实现注中 **`TIMELINE_PARITY_MATRIX_VERSION`** 更新为与 [`timelineParityMatrix.ts`](./src/pages/timelineParityMatrix.ts) 一致的 **14**；第二条线工程补充追加 **v12–v14** 简述（阶段 F 契约、壳层单源、纵向手势/`useLasso` 锚点）。

- 2026-04-21：**矩阵 v15（阶段 F·1 SSOT 锚点行）**。新增 `phase-f-range-preview-ssot` 行（parity 全 partial）+ [`phaseFRangePreviewSsot.test.ts`](./src/pages/phaseFRangePreviewSsot.test.ts)；`TIMELINE_PARITY_MATRIX_VERSION` **15**；概述/§5.7 实现注同步为 **15**。
