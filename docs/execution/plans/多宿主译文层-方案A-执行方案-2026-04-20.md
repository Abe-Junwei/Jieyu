...
| `layerParentIds.ts`（已删除；读路径见 [`useLayerSegments.ts`](../../../src/hooks/useLayerSegments.ts)） | 宿主读写仍绕 parent                                                                    | **删除**或收窄为与译文宿主无关的工具  | 无 legacy 下禁止第二真相  |
...
**`layerParentIds.ts`（无 legacy 修订）**：该文件已从仓库删除（G13）；宿主/segment 读路径以 [`useLayerSegments.ts`](../../../src/hooks/useLayerSegments.ts) 与 `layer_links` 为准。随 `Layer` 上译文宿主 parent 字段删除，**不得**再引入同名「第二真相」工具模块。
...
**`parentLayerIds`（精确）**：历史实现曾集中于已删除的 `layerParentIds.ts`；当前 segment / 宿主解析以 [`useLayerSegments.ts`](../../../src/hooks/useLayerSegments.ts) 与 `layer_links` 为准。随译文宿主迁 link 后**与译文 `parentLayerId` 一并自译文层移除**，**不**主张用于转写层多父（转写多父非本方案范围）。
...
| G9  | 读链路残留单宿主判断点                               | **拆分为 G9a / G9b（见 §2.2）**；全仓清理**译文宿主**路径上对 `translation.parent*` 的判定，统一改为 link 图或 §1.8 只读适配器（**不**清理 `transcription.parentLayerId` 层树语义）。                                                                        | G9a：`translationLayerTargetResolver`、`TranscriptionTimelineVerticalView`、`useLayerSegments` 等；G9b：`LayerConstraintService`、`useTranscriptionUnitActions.helpers` 等                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| G13 | `layerParentIds` 与 SSOT 叙事冲突              | **删除**或去译文宿主语义；禁止作为过渡「第二真相」（§1.4）。                                                                                                                          | 源码已移除；读路径锚点 [`useLayerSegments.ts`](../../../src/hooks/useLayerSegments.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
