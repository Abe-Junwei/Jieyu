...
> **当前下一刀（2026-09-20）**：标注 M1、B4f、**B3c/d** 与 **B3e LIFT 出站**已落地。余量：**B7** 仍 blocked on ChatWindow 会话隔离。不排 C3d Word；语料 flag 仍默认 false。Dogfood ≠ 产品开放。详见 [后续路线图详细评估](../audits/后续路线图详细评估-2026-09-11.md)。状态图例：✅ 已关闭 · 🟡 部分落地 · ⬜ 未开始。
...
| **B3e** | 词典 LIFT 出站 | M | **【✅ 已落地】** `/lexicon` 将当前 `lexemes` 序列化为 SIL LIFT 0.13 并下载 `.lift`。lemma→`lexical-unit`，gloss/definition→`sense`，词形→entry `<variant>`（allomorph）。只出站、不写库、无导入。无新 flag。SDD：`lexicon-lift-export/`。不做义项树 / DMLex / 附件包；不从 `/corpus` 出词典包（R5） | 空库零下载；XML 含 version=0.13 与 nested sense id；定向 vitest | 是 |
...
| 2026-09-20 | **B3e LIFT 出站**：`/lexicon` 导出 SIL LIFT 0.13（lemma / sense / variant allomorph）；只出站不写库。无新 flag。SDD：`lexicon-lift-export/`。下一刀仍不排 B7 / C3d / flag 放量。 |
