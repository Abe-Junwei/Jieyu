...
Wave 3 — Workflow 编排 + 人在环写路径
  A12  StepKind + registry 元数据 + parallel readonly API（checklist v1 已有；send-turn 并行仍不开）
  A11  Preview UI → confirm → commitToolEffects 端到端 ← 下一刀（独立 PR / flag 默认 false）
...
> **Agent 轨下一刀**：见 §2.2.2 **Wave 3 → A11**（AgentUiEvent + Preview）。A12 主体（registry / StepKind / readonly batch API）已落地。状态图例：✅ 已关闭 · 🟡 部分落地 · ⬜ 未开始。
...
| 2026-09-02 | **Wave 3 A12 主体**：`WorkflowStepKind`、registry `reflectionHandlerId` / `outputSchemaId` / `maxReflectionRetries` / `stepKinds`、composed 步 ⊆ registry、`executeReadonlyToolBatch`（A10.6）、finalize 走 `dispatchVerticalWorkflowReflection` + `after_model`。send-turn 多工具仍串行；A12 仍 🟡 至 A13 接并行样本。下一刀 **A11**。 |
