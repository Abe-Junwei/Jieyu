...
| **B3c** | 词典额外义项与词形 | M | **【✅ 已落地】** `/lexicon` 编辑 `senses[1…]`（gloss + 可选 definition）与 `forms` transcription；空行丢弃；全部词形为空则去掉 `forms`。无新 flag、无新表。SDD：`lexicon-senses-forms/`。不做义项树 / variant-entry | extra sense + forms write→list readback | 是 |
