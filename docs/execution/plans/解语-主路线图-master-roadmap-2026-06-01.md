...
| **B3b** | 词典词条编辑表单 | L | **【🟡 本 PR】** `saveLexeme` 经 `saveLexiconEntry` 从 `/lexicon` 调用；lemma / 主 gloss / citation / language / notes；新建；空 lemma 不写。不接 ChatWindow；附件仍走 B8 flag。SDD：`lexicon-entry-edit/` | 写→reload→readback；不改 R8 键分轨 | 是 |
