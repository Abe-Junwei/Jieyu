...
> **当前下一刀（2026-09-11）**：标注 M1（B4c/d/e）已开放 `/annotation`。余量：C3a/b、B3b、B4f 二次分词；**B7** 仍 blocked on ChatWindow 会话隔离。不启动 EAF/TextGrid；语料 flag 仍默认 false。详见 [后续路线图详细评估](../audits/后续路线图详细评估-2026-09-11.md)。状态图例：✅ 已关闭 · 🟡 部分落地 · ⬜ 未开始。
...
| 2026-09-11 | **B4c/d/e 标注 M1 开放**：段播放 + `user_notes`/selfCertainty + AutoGloss 预览采纳。`annotationPageEnabled` 默认 true。SDD：`annotation-m1-open/`。下一刀 C3a/b 或 B3b；B4f 二次分词仍未做。 |
