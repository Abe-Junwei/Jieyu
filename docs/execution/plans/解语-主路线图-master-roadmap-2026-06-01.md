...
> **当前下一刀（2026-09-25）**：标注 M1、B4f、**B3c–B3n**（含 LIFT、义项树、同级排序、提升/降级、义项词类、词条类型、义项例证、词条发音与词源）已落地。余量：**B7** 仍 blocked on ChatWindow 会话隔离。不排 C3d Word / DMLex；语料 flag 仍默认 false。Dogfood ≠ 产品开放。详见 [后续路线图详细评估](../audits/后续路线图详细评估-2026-09-11.md)。状态图例：✅ 已关闭 · 🟡 部分落地 · ⬜ 未开始。
