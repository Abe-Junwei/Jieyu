...
> **当前下一刀（2026-09-11）**：先合入路线图校准 [#165](https://github.com/Abe-Junwei/Jieyu/pull/165)，再 rebase **B5c** [#154](https://github.com/Abe-Junwei/Jieyu/pull/154)。详细序、开放 vs dogfood、以及 **不要做** 的清单见 [后续路线图详细评估](../audits/后续路线图详细评估-2026-09-11.md)。状态图例：✅ 已关闭 · 🟡 部分落地 · ⬜ 未开始。
...
| **B4c** | 标注页段播放（键盘合同接线） | M | **【⬜ 未开始】** `annotationKeyboardMachine` 已产出 `playToggle`，未接媒体 player。复用转写只读 seek/playback runtime，**禁止**复制 Orchestrator / ChatWindow。评估：[后续路线图详细评估](../audits/后续路线图详细评估-2026-09-11.md) 轨 3 | Space 非输入态切播放；输入态仍插入空格；定向 vitest + e2e:chromium | 是 |
| **B4d** | 标注页 note / tag / selfCertainty | L | **【⬜ 未开始】** IGT 无这些字段 UI。与转写笔记分轨，禁止第二套真源。排在 B4c 之后、与 B3b 不同时开 | 写→reload→readback；R1–R8；不写 `layer_units` | 是 |
...
| **B5c** | 语料库 HTML 剪贴板 + 工作集 bundle（P1） | M | **【⬜ 未合入 main】** `main` 仅有 B5b plain/Markdown。实现与 SDD `corpus-library-html-bundle/` 在 [#154](https://github.com/Abe-Junwei/Jieyu/pull/154)（`cursor/b5c-corpus-html-bundle-b848`），相对当前 `main` **CONFLICTING**，须 rebase 后再合。不接 ChatWindow / EAF/TextGrid 新管线；沿用页面 flag 默认 false | HTML MIME + bundle golden；空选不写剪贴板；rebase 后 CI 绿 | 是（在 PR 分支） |
...
> **导出现状（2026-09-11 代码盘点）**：**已强** = 语言学交换格式 EAF/TextGrid/TRS/Flextext/Toolbox + 原生 JYT/JYM（均带 round-trip 导入）；**已有** = 声学选区 CSV/JSON、项目归档 bundle、AI 回答带引用纯文本复制、语料工作集 **plain/Markdown** 剪贴板（B5b）；**缺口** = 字幕(C3a)、表格(C3b)、学术 IGT LaTeX(C3c)、Word(C3d 待定)、语料 **HTML/bundle（B5c，未合入 main）**。所有导出切片须**先建内部统一读模型再序列化**，禁止反噬编辑数据模型。
...
| 2026-09-11 | **后续评估**：[详细评估](../audits/后续路线图详细评估-2026-09-11.md)。下一刀 = #165 → rebase B5c；新增 **B4c/B4d/B3b** 余量行。Dogfood ≠ 产品开放。B7 按 L 估。 |
