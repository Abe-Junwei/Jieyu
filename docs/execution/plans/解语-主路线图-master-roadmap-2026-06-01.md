...
> **当前下一刀（2026-09-11 代码核对）**：rebase 合入 **B5c** HTML/bundle（[#154](https://github.com/Abe-Junwei/Jieyu/pull/154) 相对 `main` CONFLICTING）。之后余量：B4/B5/B8 **flag dogfood**（默认仍 false）；A1 `any` 收口；A3 pin 持久化；C3 字幕/表格/IGT。**B7** 仍未开工（`corpusBridgeAdapter` 不存在；须 ChatWindow 会话隔离，不是行数门槛）。不启动 EAF/TextGrid 新管线。状态图例：✅ 已关闭 · 🟡 部分落地 · ⬜ 未开始。
...
| 2026-09-11 | **代码核对**：[审计](../audits/主路线图代码核对-2026-09-11.md)。作废「开放 B4/B5 占位 / 可选 B2」为下一刀；ChatWindow 766/800 门槛改为 127/220。A2/A3 改为 🟡；新增 **B5c** 行（#154 CONFLICTING）。B7 改为 adapter 未建。下一刀 rebase B5c。 |
