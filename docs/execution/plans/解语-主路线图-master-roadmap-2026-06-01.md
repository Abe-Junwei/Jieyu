...
> **当前下一刀（2026-09-26）**：标注 M1、**B4f–B4i**、**B3c–B3w**（含 LIFT、义项树、同级排序、提升/降级、义项词类、词条类型、义项例证、词条发音、词源、字面意义、参考文献、限制、概要定义、义项学名、义项人类学注释、义项语篇注释、义项百科注释与义项语法注释）已落地。B4i 在 IGT 行只读显示同一句段的翻译层文本。余量：**B7** 仍 blocked on ChatWindow 会话隔离。不排 C3d Word / DMLex / M2 typed relation；语料 flag 仍默认 false。Dogfood ≠ 产品开放。详见 [后续路线图详细评估](../audits/后续路线图详细评估-2026-09-11.md)。状态图例：✅ 已关闭 · 🟡 部分落地 · ⬜ 未开始。
...
| **B3m** | 词典词条发音 | S | **【✅ 已落地】** `/lexicon` 编辑词条 `pronunciation`（LIFT `<pronunciation>` 第一条 form 文本）。空白省略该键。出站 lang 为 `und-fonipa`。不存媒体、声调、CV、第二条发音。无 DMLex、无新 flag。SDD：`lexicon-lexeme-pronunciation/` | pronunciation write→概览 readback；`<pronunciation>` 往返；定向 vitest | 是 |
...
| **B3u** | 词典义项语篇注释 | S | **【✅ 已落地】** `/lexicon` 编辑义项 `discourseNote`（LIFT 义项 `<note type="discourse">` 第一条 form 文本）。空白省略该键。出站 lang 为 `und`，排在人类学注释之后、子义项之前。不读词条级同名 note。再次导入省略该 note 时注释随 `senses` 整段替换而消失。无 DMLex、无新 flag。SDD：`lexicon-sense-discourse-note/` | discourseNote write→义项列表 readback；人类学注释仍在；定向 vitest | 是 |
| **B3v** | 词典义项百科注释 | S | **【✅ 已落地】** `/lexicon` 编辑义项 `encyclopedicNote`（LIFT 义项 `<note type="encyclopedic">` 第一条 form 文本）。空白省略该键。出站 lang 为 `und`，排在语篇注释之后、子义项之前。不读词条级同名 note。再次导入省略该 note 时注释随 `senses` 整段替换而消失。无 DMLex、无新 flag。SDD：`lexicon-sense-encyclopedic-note/` | encyclopedicNote write→义项列表 readback；语篇注释仍在；定向 vitest | 是 |
| **B3w** | 词典义项语法注释 | S | **【✅ 已落地】** `/lexicon` 编辑义项 `grammarNote`（LIFT 义项 `<note type="grammar">` 第一条 form 文本）。空白省略该键。出站 lang 为 `und`，排在百科注释之后、子义项之前。不读词条级同名 note，也不读无 type 的 General Note。再次导入省略该 note 时注释随 `senses` 整段替换而消失。无 DMLex、无新 flag。SDD：`lexicon-sense-grammar-note/` | grammarNote write→义项列表 readback；百科注释仍在；定向 vitest | 是 |
...
| 2026-09-26 | **B3w 义项语法注释**：`/lexicon` 编辑义项 `grammarNote`，对应 LIFT 义项 `<note type="grammar">` 第一条 form 文本。空白省略。出站 lang 为 `und`，排在百科注释之后、子义项之前。不读词条级同名 note，也不读无 type 的 General Note。再次导入省略该 note 时注释随 `senses` 整段替换而消失。无 DMLex、无新 flag。SDD：`lexicon-sense-grammar-note/`。下一刀仍不排 B7 / C3d / DMLex / flag 放量。 |
| 2026-09-26 | **B3v 义项百科注释**：`/lexicon` 编辑义项 `encyclopedicNote`，对应 LIFT 义项 `<note type="encyclopedic">` 第一条 form 文本。空白省略。出站 lang 为 `und`，排在语篇注释之后、子义项之前。不读词条级同名 note。再次导入省略该 note 时注释随 `senses` 整段替换而消失。无 DMLex、无新 flag。SDD：`lexicon-sense-encyclopedic-note/`。下一刀仍不排 B7 / C3d / DMLex / flag 放量。 |
| 2026-09-25 | **B3u 义项语篇注释**：`/lexicon` 编辑义项 `discourseNote`，对应 LIFT 义项 `<note type="discourse">` 第一条 form 文本。空白省略。出站 lang 为 `und`，排在人类学注释之后、子义项之前。不读词条级同名 note。再次导入省略该 note 时注释随 `senses` 整段替换而消失。无 DMLex、无新 flag。SDD：`lexicon-sense-discourse-note/`。下一刀仍不排 B7 / C3d / DMLex / flag 放量。 |
...
| 2026-09-25 | **B3m 词条发音**：`/lexicon` 编辑词条 `pronunciation`，对应 LIFT `<pronunciation>` 第一条 form 文本。空白省略。出站 lang 为 `und-fonipa`。不存媒体、声调、CV、第二条发音。无 DMLex、无新 flag。SDD：`lexicon-lexeme-pronunciation/`。下一刀仍不排 B7 / C3d / DMLex / flag 放量。 |
