...
**状态（2026-05-01）**：**C1 部分落地** — 转写页专用语音摘要与 `sendToAiChat` 桥接迁至 `src/services/transcriptionVoiceInteractionWiring.ts`；`useVoiceInteraction` 变薄。**C2 部分落地** — Web Speech 类型、`getSpeechRecognitionCtor`、`isWebSpeechSupported`、`runAecDiagnostic` 迁至 `src/services/VoiceInputService.webSpeechSupport.ts`；`VoiceInputService.ts` 仍 re-export 公共 API。recording/vad/probes 方向的可选进一步切分仍为后续 PR。
