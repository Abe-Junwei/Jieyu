...
**统一排期入口**：阶段完成度与「下一步」的 **执行清单** 已并入《[AI智能体-战略规划与下一步-2026-05-07.md](./AI智能体-战略规划与下一步-2026-05-07.md)》**§2.5、§3.5**；本文仍以 **审查范围、架构读模型、发现分级、阶段设计与附录** 为深度真源。
...
**状态（2026-05-01）**：**C1 部分落地** — 转写页专用语音摘要与 `sendToAiChat` 桥接迁至 `src/services/transcriptionVoiceInteractionWiring.ts`；`useVoiceInteraction` 变薄。**C2 部分落地** — Web Speech 类型与 ctor/AEC 迁至 `VoiceInputService.webSpeechSupport.ts`；**STT fallback 链、链切片、商业未配置原因、失败文案**迁至 `VoiceInputService.fallbackChain.ts`；**Web Speech 结果/致命错误**迁至 `VoiceInputService.webSpeechEngine.ts`；**Web Speech 实例化、连续模式 onend、rec.start**迁至 `VoiceInputService.webSpeechSession.ts`；**WhisperX VAD 策略与 RecordingExecutor 绑定**迁至 `VoiceInputService.vadSync.ts`；**引擎切换 debounce**迁至 `VoiceInputService.engineSwitchCoordinator.ts`；**共享分析用 getUserMedia 流**迁至 `VoiceInputService.sharedAnalysisStream.ts`（`SttEngine` 仍从 `VoiceInputService` 导出）。recording/probes 与 `RecordingExecutor` 本体的可选进一步切分仍为后续 PR。
...
**状态（2026-05-02）**：**ADR-0029 已 `accepted`**（默认关、v1 `speechSynthesis`、不支持时弱提示）；`桌面端浏览器支持策略.md` 已互链；`voice-unified-chat-path-rollout.md` 已写入 E2E 必跑、release-stable（建议 N=2）与 D2-A 文档证据口径。release evidence 自动化 gate 未强制新增（拍板 D2-A）。
...
**产品 / release 拍板索引**（TTS、发布门禁、键入路由）：[`AI语音拍板项-TTS-发布门禁-键入路由-2026-05-02.md`](./AI语音拍板项-TTS-发布门禁-键入路由-2026-05-02.md)（链至 ADR-0029、rollout 与战略规划 §2.5/§3.5）。
...
历史文档 [`规划-语音智能体架构设计方案-2026-03-18.md`](../archive/historical-root-docs/规划-语音智能体架构设计方案-2026-03-18.md)（索引稿）与 [`…-legacy-snapshot-2026-05-07.md`](../archive/historical-root-docs/规划-语音智能体架构设计方案-2026-03-18-legacy-snapshot-2026-05-07.md)（全文快照）中的 **P0/P1/P2**（如早期「`TtsService` 独立产品化」设想）与本方案 **Phase D / Phase C** 语义上对应；**TTS 现行拍板**见 [ADR-0029](../../adr/0029-assistant-tts-web-speech-policy.md)。**当前事实源**以 `docs/architecture/`、`docs/adr/` 与代码为准。