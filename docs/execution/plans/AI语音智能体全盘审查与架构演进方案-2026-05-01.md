...
**状态（2026-05-01）**：**C1 部分落地** — 转写页专用语音摘要与 `sendToAiChat` 桥接迁至 `src/services/transcriptionVoiceInteractionWiring.ts`；`useVoiceInteraction` 变薄。**C2 部分落地** — Web Speech 类型与 ctor/AEC 迁至 `VoiceInputService.webSpeechSupport.ts`；**STT fallback 链与失败文案**迁至 `VoiceInputService.fallbackChain.ts`（`SttEngine` 类型别名仍从 `VoiceInputService` 导出）。recording/vad/probes 方向的可选进一步切分仍为后续 PR。
