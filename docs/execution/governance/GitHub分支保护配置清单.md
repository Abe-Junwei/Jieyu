...
若你本地具备 GitHub token，也可以优先用仓库脚本半自动下发 branch protection：

1. 先用 `npm run github:branch-protection:dry-run` 检查 owner/repo、目标分支和请求体。
2. 再在当前 shell 提供 `GITHUB_TOKEN`、`GH_TOKEN` 或 `GITHUB_PAT` 之一。
3. 最后运行 `npm run github:branch-protection:apply`。

说明：脚本默认会为 `dev` 与 `main` 同时下发 `docs-governance`、`quality`、`build-guard` 三个必选检查，并开启 stale review 清理、conversation resolution、禁止 force push、禁止删除分支与 admin enforcement。
...
1. 当前 `docs-governance` job 会独立执行 `npm run check:docs-governance` 与 `npm run report:docs-link-debt`。
2. 当前 `quality` job 内已经串行覆盖 `npm run typecheck`、`npm test`、`npm run report:architecture-hotspots`。
3. 当前 `build-guard` job 内已经覆盖 `npm run build:guard`。
...
若界面显示为带 workflow 前缀的形式，也选择对应 `CI` 工作流下的这三个检查。

## 与仓库内机制的对应关系

### `docs-governance`

由 [.github/workflows/ci.yml](.github/workflows/ci.yml) 中的 `docs-governance` job 提供，覆盖：

1. `npm run check:docs-governance`
2. `npm run report:docs-link-debt`

其中：

1. `check:docs-governance` 失败会直接阻塞合并。
2. `report:docs-link-debt` 用于暴露历史文档链接债务，不作为单独失败口径。
...
2. `check:docs-governance`
3. `check:segmentation-storage-boundary`
4. `check:tier-boundary-imports`
5. `check:tierid-diffusion`
6. `check:architecture-guard`
7. `vitest run`
...
3. 确认 PR 页面显示必需检查为 `docs-governance`、`quality` 与 `build-guard`。
4. 确认在 checks 未完成前，Merge 按钮不可用。
5. 确认若 PR discussion 未解决，Merge 仍被阻止。

## 自动化执行限制

当前仓库已经提供 branch protection 下发脚本，但是否能直接写入远端设置，仍取决于当前环境是否有：

1. 对目标仓库有权限的 `GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_PAT`
2. 对 `Abe-Junwei/Jieyu` 具备维护或管理员权限的 GitHub 账号

若当前 shell 无认证上下文，脚本只能做 dry-run，不能直接写入远端；这时按本文档在 GitHub 页面手工配置即可。