...
export class LinguisticService {
  ...
  static async saveTierDefinition(data: TierDefinitionDocType, source: AuditSource = 'human'): Promise<TierSaveResult> {
    ...
    const allDocs = await db.collections.tier_definitions.find().exec();
    const allTiers = allDocs.map((d) => d.toJSON()).filter((t) => t.textId === data.textId);
  ...
  static async removeTierDefinition(id: string, source: AuditSource = 'human'): Promise<{ errors: ConstraintViolation[] }> {
    ...
    const allDocs = await db.collections.tier_definitions.find().exec();
    const childTiers = allDocs.map((d) => d.toJSON()).filter((t) => t.parentTierId === id);
    ...
    // Cascade: delete all annotations belonging to this tier
    const annDocs = await db.collections.tier_annotations.find().exec();
    const tierAnns = annDocs.map((d) => d.toJSON()).filter((a) => a.tierId === id);
    for (const ann of tierAnns) {
  ...
  /** Internal: persist tier annotation + audit, no constraint validation. */
  private static async _persistTierAnnotation(data: TierAnnotationDocType, source: AuditSource): Promise<string> {
    const db = await getDb();
  ...
  static async saveTierAnnotation(data: TierAnnotationDocType, source: AuditSource = 'human'): Promise<TierSaveResult> {
    ...
    const annDocs = await db.collections.tier_annotations.find().exec();
    const existingAnns = annDocs.map((d) => d.toJSON()).filter((a) => tierIds.has(a.tierId));
    ...
    const id = await this._persistTierAnnotation(data, source);
    const warnings = violations.filter((v) => v.severity === 'warning');
    return { id, errors: [], warnings };
  }
  ...
  static async removeTierAnnotation(id: string, source: AuditSource = 'human'): Promise<void> {
    ...
    const allAnns = await db.collections.tier_annotations.find().exec();
    const children = allAnns.map((d) => d.toJSON()).filter((a) => a.parentAnnotationId === id);
  ...
  static async saveTierAnnotationsBatch(
    ...
    for (const ann of newAnnotations) {
      await this._persistTierAnnotation(ann, 'human');
    }
